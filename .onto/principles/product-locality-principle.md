...
공유 자산의 읽기 참조와 `ONTO_HOME` 격리는 별개다. §3 #5의 격리 대상은 설치에 포함된 리소스(.onto/roles/, authority/)이며, 주체자 수준 공유 데이터(`~/.onto/domains/`, `~/.onto/learnings/`)는 product-override 규칙으로 관리된다.

...
| 산출물 종류 | 본질 sink 위치 (예) | dogfood 의 역할 |
|---|---|---|
| govern queue / verdict | `.onto/govern/queue.ndjson` | dogfood off 시에도 queue 정상. dogfood 는 read + 자체 분석 mirror |
| self_apply 사후 보고 | `.onto/govern/self-apply-log/<session>/events.jsonl` | dogfood off 시에도 Principal 사후 검토 가능. dogfood 는 read + mirror |
| review record | `.onto/review/<session>/` | dogfood off 시에도 review 산출물 정상. dogfood 는 read + measurement.yaml 으로 집계 |
| learn promote 결과 | `.onto/learnings/*.md` + `_audit/` | dogfood off 시에도 학습 정상. dogfood 는 read + feedback-to-product pointer 만 mirror |
| reconstruct ontology | `.onto/reconstruct/<session>/` | dogfood off 시에도 reconstruct 정상. dogfood 는 read + mirror |

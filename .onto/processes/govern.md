...
|---|---|---|
| govern (queue / verdict) | `.onto/govern/queue.ndjson` (event-sourced ledger of all governance events: pending decision + decided verdict + manual_edit marker) | dogfood off 시에도 queue 정상. dogfood 는 read + mirror seat 에 집계 |
...
| **design** | **본질 sink 미정착** (`.onto/processes/design.md` 절차 자체 미정의). 임시 sink 는 `development-records/design/<date>-<topic>.md` (위계 밖 이력 패턴 — 본 §14.6 자체도 design 활동 산출물의 일례). 절차 정착 시점에 본 invariant 가 sink 위치 결정에 자동 적용. | (정착 후) dogfood 는 read + mirror |

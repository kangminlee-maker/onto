...
본 v0 는 **현 product 내부 규범** (authority/, .onto/processes/, .onto/commands/, design-principles/, .onto/roles/ 등) 의 변경 관리에만 적용한다. 다음은 범위 밖:
...
| `learning_to_principle_promotion` | promoted learning → principle | `~/.onto/learnings/{agent}.md` 의 항목 | `design-principles/*.md` 또는 `.onto/processes/*.md` 의 본문 | W-C-03 (미구현, §1.1/§1.2 "보류 중") |
...
| 본문 배치 | `design-principles/*.md` + `.onto/processes/*.md` | `authority/core-lexicon.yaml` 내부 |
...
§1.2 "보류 중" 해소. promoted learning (knowledge) 이 principle 본문 (`design-principles/` 또는 `.onto/processes/`) 으로 승격되는 경로. canonical term: `learning_to_principle_promotion`. W-D-05 가 확정한 도착지 (principle entity) 를 소비한다.

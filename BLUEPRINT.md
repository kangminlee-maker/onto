...
> 이 문서는 onto-review 시스템의 구조, 원리, 사용법을 기술합니다.
> 대상 독자: 비개발자 제품 전문가.
...
onto-review는 **Claude Code 플러그인**입니다. Claude Code(터미널에서 동작하는 AI 코딩 도구) 안에서 슬래시 명령어(`/onto-review`, `/onto-buildfromcode` 등)로 실행됩니다.

이 플러그인은 두 가지 기능을 제공합니다:

1. **검증(review)**: 문서, 코드, 설계안 등 범위가 확정된 대상을 8인 에이전트 패널이 다관점으로 검증
2. **구축(build)**: 코드베이스에서 온톨로지(도메인 지식의 구조화된 표현)를 자동으로 추출

### 왜 존재하는가

하나의 관점으로 검증하면 특정 측면의 문제를 놓칩니다. 예를 들어 논리적으로 정합하더라도 실제 사용 시 질의가 불가능할 수 있고, 구조적으로 완전하더라도 새 도메인 추가 시 깨질 수 있습니다. onto-review는 7개의 서로 다른 검증 축을 가진 에이전트가 독립적으로 평가하고, 8번째 에이전트(Philosopher)가 이를 종합하여 "세부에 매몰되지 않고 목적에 부합하는가"를 판정합니다.

### 핵심 설계 원칙

| 원칙 | 의미 | 이것이 없으면 |
|---|---|---|
| **다관점 독립 검증** | 7인이 서로의 결과를 모른 채 독립적으로 판단 | 선행 의견에 끌려가는 anchoring bias 발생 |
...
| **delta** | Explorer가 코드를 읽고 보고하는 도메인 사실 단위 (build 프로세스 전용) |
...
| **certainty** | 사실의 확실성 등급. deterministic(코드에서 확인) ~ not-in-code(코드 외부 정보 필요) |
...
Philosopher는 7인과 근본적으로 다른 역할을 합니다.

**7인 검증 에이전트와의 차이**:
- 7인: 각자의 축에서 기준 충족 여부를 판단
- Philosopher: 7인의 판단 결과를 시스템의 "상위 목적"에 비추어 재해석

**하는 일**:
1. 7인의 결과를 합의/모순/간과된 전제/새로운 관점으로 분류
2. 만장일치인 항목에도 "합의의 논리적 근거"를 별도 검증 (전원 동의해도 근거가 부실하면 지적)
3. 세부 기준 충족에 매몰되어 본래 목적과 어긋나는 경우를 포착
4. 7인이 모두 놓친 관점을 새로 제시
...
### 3.3 Explorer — 코드 탐색자 (build 전용)

build 프로세스에서만 존재하는 에이전트입니다.

**하는 일**: 코드를 직접 읽고, 도메인 사실을 서술합니다.
**하지 않는 일**: 온톨로지적 해석 (예: "이것은 Aggregate이다"). 이것은 검증 에이전트가 합니다.
...
이 분리가 존재하는 이유: Explorer가 해석까지 하면, 7인 검증 에이전트의 독립적 판단이 Explorer의 해석에 anchoring됩니다.
...
2. Team 생성 + Round 1 (7인 독립 리뷰)
   ...
   - 8인 teammate 생성 (7인 검증 + Philosopher)
   - 7인이 독립적으로 검증. 서로의 결과를 모름
...
- **만장일치도 검증**: 8인 전원이 동의해도, 그 합의의 논리적 근거를 별도로 확인

---

### 4.2 개별 질문 (question)

**명령어**: `/onto-{dimension} {질문}` (예: `/onto-logic 이 설계에 모순이 있나요?`)
**입력**: 특정 관점으로 답변받고 싶은 질문
**출력**: 해당 에이전트 1인의 답변 + 학습 기록

8인 전원을 동원할 필요 없이, 특정 축의 전문가 1인에게 직접 질문합니다.
에이전트는 자신의 역할 정의 + 방법론 학습 + 도메인 문서 + 도메인 학습을 로드한 후 답변합니다.

---

### 4.3 코드 기반 온톨로지 구축 (build)

**명령어**: `/onto-buildfromcode {경로|URL}`
**입력**: 코드베이스 (경로 미지정 시 프로젝트 전체)
**출력**: `raw.yml` — 코드에서 추출한 온톨로지

이 프로세스가 review와 구조적으로 다른 이유:
- review: 범위가 확정됨 → 7인이 독립 병렬로 각자 평가하면 됨
...
Phase 0: Schema Negotiation
   온톨로지의 "틀"을 사용자와 결정
   5가지 선택지: Axiom 기반 / Action-Centric / Knowledge Graph / Domain-Driven / 커스텀
   → schema.yml 저장
...
| 등급 | 의미 | 예시 | 판정 주체 |
|---|---|---|---|
| `deterministic` | 코드에서 직접 관찰. 코드가 바뀌지 않으면 이 사실도 안 바뀜 | "Payment 클래스에 status 필드가 있다" | Explorer |
| `code-embedded-policy` | 코드에 로직은 있으나 "왜"의 근거가 코드에 없음 | "최소 결제 금액이 500원으로 하드코딩됨 — 왜 500인지는 코드에 없음" | 검증 에이전트 |
| `inferred` | 합리적 추론이지만 코드에서 직접 확인 불가 | "이 두 서비스는 이벤트로 통신하는 것으로 추정됨" | 검증 에이전트 |
| `not-in-code` | 코드에서 결정 불가. 다른 소스 필요 | "이 기능의 사용자 시나리오" | 검증 에이전트 |

이 분류가 존재하는 이유: 사용자가 "어디까지 확정이고 어디부터 결정이 필요한지" 파악하기 위함. `not-in-code`는 Phase 3에서 사용자 결정 대상으로 제시됩니다.
...
- **TeamCreate**: 팀을 생성합니다. team_name은 `{프로세스}-{세션ID}` 형식 (예: `onto-review-20260325-a3f7b2c1`)
- **SendMessage**: teammate 간 메시지를 전송합니다
- **TeamDelete**: 팀을 삭제합니다

**세션 ID**: `{YYYYMMDD}-{hash8}` 형식. 날짜로 시간 순서를 파악하고, 8자리 랜덤 해시로 충돌을 방지합니다.

**세션 디렉토리**: `{project}/.onto-review/{프로세스}/{세션ID}/` — 리뷰 결과 파일이 저장되는 곳
...
│   ├── review.md             #   팀 리뷰 (8인 패널)
│   ├── question.md           #   개별 질문 (1인)
│   ├── build.md              #   코드 기반 온톨로지 구축 (적분형 탐색)
...
├── commands/                 # 명령어 정의 (13개)
│   ├── onto-review.md
│   ├── onto-buildfromcode.md
│   ├── onto-transform.md
│   ├── onto-onboard.md
│   ├── onto-promotelearnings.md
│   ├── onto-logic.md
│   ├── onto-structure.md
│   ├── onto-dependency.md
│   ├── onto-semantics.md
...
**문제**: 8인의 리뷰 원문(약 64,500자)이 team lead의 컨텍스트에 한꺼번에 유입되면, LLM이 일부 메시지만 인식합니다. 실제로 7인 중 3인의 결과만 인식된 사례가 있습니다.

**원인**: Agent Teams의 메시지 전달(transport)은 성공했으나, team lead LLM이 컨텍스트 한계로 모든 메시지를 처리하지 못함.

**해결**: 파일 기반 전달. reviewer가 결과를 파일로 저장하고, team lead에게는 파일 경로(약 500자)만 보고합니다. Philosopher가 파일을 직접 읽어 원문을 처리합니다. 이렇게 하면 team lead 컨텍스트에 원문이 적재되지 않고, "원문 그대로 전달" 규칙도 구조적으로 보장됩니다.

### Issue 1-B: 세션 간 에이전트 누수

**문제**: team_name이 고정(`onto-review`)이었을 때, 다른 세션에서 생성된 에이전트가 현재 세션의 team lead에게 메시지를 보내는 현상. 다른 프로젝트의 리뷰 내용이 현재 세션에 유입됨.

**원인**: `~/.claude/teams/onto-review/` 디렉토리가 파일 시스템에 공유되어, 같은 team_name을 사용하는 팀이 여러 세션에서 동시에 존재.

**해결**: 세션 ID 기반 team_name (`onto-review-20260325-a3f7b2c1`). 세션마다 고유한 이름을 사용하여 격리.
...
```
/onto-buildfromcode              # 프로젝트 전체
/onto-buildfromcode src/         # 특정 디렉토리
/onto-buildfromcode {GitHub URL} # 외부 레포
```
...
1. **에이전트 구성 결정**: 7개의 검증 축(논리, 구조, 의존성, 의미, 활용, 진화, 포괄성) + 1인 메타 검증(Philosopher). 이 8인 구성은 온톨로지 검증 이론에서 도출된 것으로, 축을 추가/제거할 수 있으나 각 축이 다른 축과 독립적이어야 합니다.

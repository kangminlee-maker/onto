...
### 질문 3. 이 작업은 품질 저하 없이 완전히 고정 가능한가
...
## LLM/runtime interface 원칙

LLM이 읽는 입력은 두 층으로 분리해 설계하는 것이 좋다.
...
이 입력은 artifact-first, deterministic, contract-boundary로 고정할 수 있어야 한다.

### 2. 자율 탐색 입력 (SelfDirectedExplorationInputs)

LLM이 문제 해결 중 스스로 확보하는 입력이다.
...
runtime은 첫 번째 층만 제공해야 한다.
두 번째 층에서 무엇을 더 읽고 조사할지는 `LLM`이 스스로 판단해야 한다.
...
- interface는 자유 텍스트 conversation이 아니라 `artifact-first handoff`로 구성한다.
- packet은 `smallest sufficient bundle`이어야 한다.
- primary target은 embed할 수 있지만, supporting context는 원칙적으로 ref로 넘긴다.
- giant duplicated payload를 만드는 방식보다 lightweight handoff가 낫다.
- packet에 명시되지 않은 reference chain을 LLM이 암묵적으로 무한 재귀 탐색하도록 두면 안 된다.

### declared boundary 원칙

declared boundary는 단순히 "무엇을 넘길 것인가"가 아니라 아래를 함께 정하는 것이다.

1. 어떤 요소를 선언하는가
2. 어떤 방식으로 제시하는가
3. 어떤 방식으로 통제하는가

실무적으로는 아래 4개 seat로 분리하는 것이 좋다.
...
- 추가 탐색 시 provenance 남기기

같은 제약을 prompt, host, MCP, 분리 서버 등 서로 다른 강도로 적용할 수 있다.

이때 중요한 규칙:

- 여러 제약이 충돌하면 **가장 강한 deny가 승리**한다.
- 경계 때문에 필요한 근거를 확보할 수 없으면 억지 결론을 내지 말고 `degraded` 또는 `fail-close`로 끝내야 한다.

## 무엇을 probabilistic하게 다룰 것인가

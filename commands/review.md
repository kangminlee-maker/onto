...
CLI executor 경로(`--codex`, `--claude` 등)는 lens 진행을 LLM 토큰 없이 주체자에게 보여주기 위해 **runtime이 자동으로 watcher pane을 띄운다**:
...
| 3 | Codex CLI | **주체자 동의 필수** — 추가 과금 발생 고지 후 승인 시에만 실행 | 추가 과금 |
...
- "Codex는 언제 사용?" → Tier 2까지 모두 실패 + 주체자 명시적 동의 시에만

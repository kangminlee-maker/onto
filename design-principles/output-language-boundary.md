...
- Agent prompt 상단의 표준 `[Language Policy]` boilerplate (processes/*.md 전수 주입) — 방어적 redundancy. 현재 reconstruct/process inline English instruction 로 충분
- Review lens/synthesize output 의 **body vs user-visible summary 구조적 분리** — `design-principles/output-language-boundary.md` §3.3 dual-purpose case. `ReviewRecord` 스키마와 `materialize-review-prompt-packets.ts` 까지 영향. 독립 설계 세션 필요

**현재 상태의 함의**: 원칙 + registry + canonical render seat + 모든 프롬프트 위반 수정 + CI lint 활성 + review contract 정렬 — **기계적 강제 + 계약 표면 일치 모두 확보**. 기여자가 새 위반을 도입하면 CI 에서 즉시 차단하고, contract 를 읽는 기여자는 원칙을 즉시 인지한다. 남은 작업은 모두 "확장" 성격 (실제 번역 layer, 구조적 output 분리).

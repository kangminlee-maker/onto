...
**확립된 것**:
- 본 원칙 문서 (rank 4)
- `authority/external-render-points.yaml` seat (minimal stub)
- `CLAUDE.md` Authority 위계 등록

**미확립 — 후속 설계 세션에서 정식화 필요**:
- Canonical render 함수 `src/core-runtime/translate/render-for-user.ts`
- Agent prompt 상단의 표준 `[Language Policy]` boilerplate
- CI lint gate (`scripts/lint-output-language-boundary.ts`) — 등록되지 않은 `{output_language}` 참조 차단
- 기존 위반 수정 (`processes/reconstruct.md` line 447/531/567 의 `Respond in {output_language}`)

**현재 상태의 함의**: 본 원칙은 **등록된 규범이지만 기계적 강제는 없음**. 기여자는 본 문서를 읽고 자발적으로 준수해야 한다. CI 강제는 위 "미확립" 항목들이 후속 PR 로 도입된 뒤 활성화된다.

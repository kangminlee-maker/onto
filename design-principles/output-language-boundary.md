...
- Agent prompt 상단의 표준 `[Language Policy]` boilerplate (processes/*.md 전수 주입) — 방어적 redundancy
...
**현재 상태의 함의**: 원칙 + registry + canonical render seat + 모든 프롬프트 위반 수정 + CI lint 활성 — 기계적 강제 확보. 기여자가 새 위반을 도입하면 CI 에서 즉시 차단. 남은 작업은 "확장" 성격 (실제 번역 layer, review contract 재설계).

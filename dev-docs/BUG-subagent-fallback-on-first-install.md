...
onto 플러그인을 처음 설치한 환경에서 `/onto:review`, `/onto:build` 등을 실행하면, Agent Teams(TeamCreate)를 시도하지 않고 항상 subagent(Agent tool) fallback으로 실행된다. 사용자가 "Agent Teams를 사용하라"고 명시적으로 지시해야 그 다음부터 Agent Teams로 전환된다.
...
- 사용자가 수동으로 개입해야 Agent Teams 활성화 → 일관성 없는 사용 경험

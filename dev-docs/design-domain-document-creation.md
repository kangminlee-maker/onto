...
| **config.yml** | 프로젝트별 onto 설정 파일. `{project}/.onto/config.yml`에 위치. `domains:` 선언이 source of truth (디렉토리 존재는 구현, config.yml이 권위). 정의는 `design-per-session-domain-selection.md` Section 1 참조 | `domains: [SE, ontology]` |
...
- **전제조건**: SEED 마커가 포함된 섹션이 0개 (모든 마커가 제거된 상태)
- **동작**: `drafts/{domain}/` → `domains/{domain}/`로 이동 + config.yml `domains:` 자동 추가
...
| # | File | Update |
|---|------|--------|
| 1 | `process.md` | Domain Documents 섹션에 drafts/ 경로 추가, 피드백 루프 규칙 추가 |
| 2 | `process.md` | Domain Determination Rules에 drafts/ 리뷰 경로 추가 |
| 3 | `processes/review.md` | seed 리뷰 대상 유형 추가, Context Gathering에 drafts/ 분기 |
| 4 | `commands/` | `/onto:create-domain`, `/onto:feedback`, `/onto:promote-domain` 커맨드 파일 생성 |
| 5 | `README.md` | 커맨드 목록, 디렉토리 구조에 drafts/ 추가 |
| 6 | `dev-docs/BLUEPRINT.md` | 설계 반영 |
| 7 | `setup-domains.sh` | drafts/ 경로 인식 추가 (설치 시 drafts/ 건드리지 않음) |

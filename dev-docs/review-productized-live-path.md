...
`processes/review.md`는 prompt-backed reference execution의 source와 절차를 담되,
...
세부 contract는 `dev-docs/review-execution-preparation-artifacts.md`를 따른다.
...
세부 contract는 `dev-docs/review-record-contract.md`를 따른다.
...
기존 `processes/review.md`의 아래 항목은 source material로만 남는다.
...
1. `processes/review.md`를 이 live path에 맞는 reference execution 문서로 더 축소한다

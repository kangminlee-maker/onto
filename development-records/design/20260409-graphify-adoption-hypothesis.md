...
| artifact purpose | **onto build의 canonical architecture design document**. Two-Layer Architecture는 사용자(=주체자) confirmed principle. graphify adoption 부분은 여전히 backlog hypothesis (build authority owner re-review 필요) |
...
**목적**: ontology가 *실제로 작동하는 코드 혹은 something*이어야 한다는 사용자(=주체자) 요구를 충족.
...
**목적**: ontology가 *코드/something이 변경될 때 함께 진화*해야 한다는 사용자(=주체자) 요구를 충족.
...
**Option β 채택** (사용자(=주체자) 승인 2026-04-10): `raw-ground-truth.yml` + `raw-inference.yml` 별도 파일.
...
| **User domain knowledge** | 사용자(=주체자)가 "이 inference는 실제로 맞다"고 직접 확인 | 불가능 (user input only) |
...
- "이건 사실이고 이건 추측입니다"가 사용자(=주체자)에게 명확해야 함
...
사용자(=주체자) 결정 (2026-04-10)으로 v7에서 다음 항목들이 resolved:
...
| 2차 SYN-D2 (priority reranking unresolved) | disagreement | §13.6 Authority-Resolved + v7 사용자(=주체자) 결정 | resolved |
...
### 21.1 사용자(=주체자) 결정 timeline

**2026-04-10 — 핵심 framing 진술**:

1. **첫 framing**: 사용자(=주체자)가 BT-E4 (semantic similarity)에 대한 반응에서 build의 본질을 재정의:

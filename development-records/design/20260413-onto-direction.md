...
**learning**: onto의 모든 활동 과정에서 발생하는 단축 정보. "이 종류 작업에서는 이런 패턴이 통한다"는 실용 발견물. 원칙이 아니며, 활용하면 시간·비용을 줄인다. lifecycle_status로 seed → provisional → promoted 경로를 거친다.

**knowledge**: promoted 단계의 learning을 지칭하는 별칭. 축적된 knowledge가 §1.0 측면 3(기제)의 비교우위 누적을 이룬다.

**principle**: 규범적 결정으로 확정된 상위 층. learning 승격 경로와 무관하며(knowledge → principle 경로는 보류 중), `authority/`·`design-principles/`·`processes/`에 존재한다. 모든 onto 활동이 준수해야 할 제약.
...
보류 중. 이론적으로 promoted learning이 일정 조건에서 principle로 승격 가능하나, 구체 기준은 향후 govern 개발 시 결정.
...
1. 기존 `20260412-harness-transition-roadmap.md` 검토: 본 §1에 반영되지 않은 아이디어 식별 후 필요한 것만 본 문서에 통합. 이후 기존 파일 삭제
2. 기존 `20260412-sprintkit-reuse-mapping.md` 검토: 본 §1 축 B와의 정합 확인
3. 작업 목록 초안 작성: 각 축별 task-level 분해 (to-do list)
4. 축 D 선행 확인 (lexicon provisional lifecycle 성문화 위치, 인용 금지 검증 도구)

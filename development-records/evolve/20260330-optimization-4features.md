...
> 5차 리뷰(20260330-e7c5cf6e) 기반. 사용자(=주체자) 승인 완료 (2026-03-30).
...
리뷰 대상의 복잡도에 비례하여 에이전트 수를 조절. 시스템이 판단하고 사용자(=주체자)가 결정.
...
위험도 높음: 구현 직전 최종 검증, 안전장치 설계, 기존 사용자(=주체자) 영향 변경
위험도 낮음: 탐색적 질문, 초기 방향, 내부 문서, 이미 리뷰 거친 후속 확인
→ 수용 가능: 경량 가능 / 수용 불가: 전원 필요

**경량 가능 판단 시:**
사용자(=주체자)에게 사유와 함께 선택지를 제시한다.
...
사용자(=주체자)에게는 선택한 에이전트와 그 이유를 함께 제시한다.
```
...
에이전트 API 에러 발생 시 사용자(=주체자) 개입 없이 팀 리드가 자동으로 감지하고 재시도.
...
| `processes/learn/promote.md` | Step 3 하단에 process.md Error Handling Rules 참조 + promote 고유 사항만 기술 |
...
### 프로세스: processes/learn/health.md
...
| File | Change |
|------|--------|
| `processes/learn/health.md` | 신규 생성 |
| `commands/learn/health.md` | 신규 생성 |

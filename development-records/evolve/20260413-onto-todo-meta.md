...
본 문서가 이전 review 결함을 어디서 어떻게 해소했는지 추적 가능하도록 매핑한다.
...
### v4 → v5 반영 (session `20260413-5f709c73`)

| Review 지적 | v5 반영 위치 |
|---|---|
| coverage: M-04 schema lifecycle/status 부재 | M-04 필드 스키마에 `lifecycle_status` 추가 (active/deferred/done/deprecated/archived) |
| coverage: M-04 schema provenance/origin 부재 | M-04 필드 스키마에 `provenance` 추가 (backlog source ID 또는 M-03 gap ID) |
| structure: Migration continuity 필드의 canonical schema seat 부재 | M-04 스키마에 `deprecated_aliases`, `previous_id`, `rewrite_trace` 추가. Change-type별 M-04 reopen mandatory로 업그레이드 |
...
**PR Extraction Unit + Dedup Rule** (v4 추가, v5 절차화):

- **Extraction unit**: 기본 **1 PR = 1 includable item**. 예외: PR 본문에 명시적 체크리스트(`- [ ]`)가 있고 각 항목이 **다른 축·활동**에 속하면 체크리스트 항목별 분리

**Dedup 절차** (v5 — 실행 가능한 단일 경로):

1. **Similarity input basis**: 각 item을 아래 3개 필드로 구성된 문자열로 정규화
...
3. **Similarity 판정**:
   ...
   - < 60% → 별개 item
4. **Tie-break rule** (dedup 후보 중 1개 선택):
   - 최신 updatedAt 우선
   - 동일 updatedAt 시 **PR > memory entry > design record** (실행 단계가 더 구체한 것)
   - 여전히 동일하면 가장 많은 label·comment 보유한 것
5. **Evidence seat** (각 dedup 결정 기록):
   - `dedup_basis`: similarity 점수 + 공통 영역
   - `supersedes`: 제거된 item의 source ID 목록
   - `kept_because`: tie-break 사유 한 줄
6. **Ambiguous-case fallback**: 위 절차로도 판정 불가한 경우 separate items로 보존. M-03 disposition에서 재판단하고 `dedup_deferred: true` 태그.

**Source 후보**:
- `MEMORY.md` 관련 entry (roles refactor v3 backlog, Build 8th Review Backlog, Principal Stage 3 Backlog, Design Spec v4 잔여 등)
- Open PR (#20, #21, #22, #7)
- `development-records/evolve/` 산출물
...
| 필드 | 의미 |
|---|---|
| ID | `W-{axis}-{nn}` 형식. axis ∈ {A, B, C, D}. nn은 axis 내 고유 번호 |
| 제목 | 한 줄 요약 |
...
| **lifecycle_status** (v5 추가) | active / deferred / done / deprecated / archived. 초기값 active. M-08 protocol이 전이 관리 |
| **provenance** (v5 추가) | 이 work item의 origin 참조. backlog source ID (M-00) 또는 M-03 gap ID. 둘 이상이면 list |
...
**경로 α 실행 순서** (v5에서 M-04 schema 완결):
...
| 항목 | 규칙 |
|---|---|
| **ID continuity** | 축 변경 시 기존 `W-{old_axis_token}-{nn}`을 `W-{new_axis_token}-{nn}`으로 rename. axis token ∈ {A, B, C, D}. 예: `W-A-01` → `W-B-01`. number는 유지. 새 axis 내 번호 충돌 시 가장 큰 nn+1로 reissue |
...
| **Mapping artifact seat** | `development-records/evolve/expansion-migration-{YYYYMMDD}.md`에 변환 테이블 저장 (old_id → new_id, 변경 유형, 이유) |
...
|---|---|---|
| **활동 추가** | M-01 (inventory) → **M-04** (schema에 activity-지시 필드 반영) → M-03 (disposition) → M-05 (dep) → M-06 → M-07 | — |
| **활동 삭제** | M-01 → **M-04** → M-03 → M-05 → M-06 → M-07 + 해당 **활동에 속한 work items** (activity 필드 기준)를 deprecated로 전환 | — |
| **활동 병합** | 삭제+추가 조합. **M-04 mandatory**. Migration Contract의 `deps rewrite` + `previous_id` + `deprecated_aliases` 기록 필수 | — |
| **축 추가·변경** | **M-04** (축별 섹션 재조직 + `W-{axis}-{nn}` ID 규약 갱신) → M-05 (dep graph 대폭 갱신) → M-06 (ID continuity 적용 후 재생성) → M-07 | M-03 (disposition 재분류 시) |
| **Schema 변경** | **M-04** (versioning 증가 + 새 필드 정의) → M-06 (기존 item migration) → M-07 | M-03 (분류 기준 바뀐 경우) |
| **Canonicality 기준 변경** | **M-04** (기준 문서화) → M-03 (재분류) → M-06 (거부된 item 재검토) → M-07 | — |
...
Principal 확정 시:
1. 본 v5 commit + push
2. v5를 한 번 더 9-lens review (재검증 5차)
3. 통과 시 M-00 착수
4. `.onto/review/` 하위 1~4차 review session들 모두 reference로 보존 (audit trail)

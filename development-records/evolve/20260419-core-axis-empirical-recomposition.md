...
v1~v4 분석은 (coverage, depth retention, items-lost) 3 metric 으로 "Pareto front" 주장. 그러나 이 3 metric 만으로는 k=9 (full) 가 모든 k<9 를 dominate (더 많은 lens = 더 많은 coverage). 본 proposal 의 실질 근거는 **4 축 cost-constrained Pareto**: `coverage × depth × items-lost × cost (LLM call count)`. Cost 를 포함해야 "품질 vs 비용" trade-off 의 의미 있는 front 가 성립. 이후 본 proposal 과 CHANGELOG / SSOT 전수에 `cost-constrained` qualifier 를 사용.

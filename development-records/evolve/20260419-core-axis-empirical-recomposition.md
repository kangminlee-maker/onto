...
revision_history:
  - "2026-04-19: initial proposal — v5 empirical analysis (`20260419-lens-contribution-analysis.md`) 기반 core-axis 재구성 권장. Option P' (6 lens) 를 cost-constrained Pareto-optimal 로 채택"
  - "2026-04-19 (post PR #131 1차 리뷰): semantics F-S1/F-S3 흡수 — (a) 이름 `core-axis` 의미 shift (meta-level axis → empirical core lens) 를 §1.1 에 explicit. (b) 'Pareto-optimal' → 'cost-constrained Pareto-optimal' 로 정정 (4th 축 cost 포함 이유 기술)"
purpose: |
  ...
  K=3~9 전수 비교에서 **Option P' (6 lens) 가 cost-constrained Pareto-optimal** — coverage
...
### 1.1.1 이름 `core-axis` 의 의미 shift (post 1차 리뷰)

PR #127 의 rename 원칙은 "이름이 선정 근거를 직접 전달" 이었다. 당시 '선정 근거' 를 "meta-level 4 axis core" 로 추론. v0.2.1 에서 선정 근거가 empirical Pareto-optimal core lens set 으로 전환되었으나, 이름 원칙 자체는 유지된다: `axis` 의 의미가 **"meta-level 판단 축" → "empirical core lens"** 로 shift 한 것이며, `core-axis` 는 여전히 "core 여섯 축" 으로 의미 정합. 이름 재rename 은 불필요 — 본 §1.1.1 가 의미 shift 의 explicit SSOT.

### 1.1.2 Pareto-optimal 의 정확한 정의 (post 1차 리뷰)

v1~v4 분석은 (coverage, depth retention, items-lost) 3 metric 으로 "Pareto front" 주장. 그러나 이 3 metric 만으로는 k=9 (full) 가 모든 k<9 를 dominate (더 많은 lens = 더 많은 coverage). 본 proposal 의 실질 근거는 **4 축 Pareto (with cost as 4th axis)**: `coverage × depth × items-lost × cost (LLM call count)`. Cost 를 포함해야 "품질 vs 비용" trade-off 의 의미 있는 front 가 성립.

**용어 choice**: 본 proposal 과 CHANGELOG / SSOT 전수에 `cost-constrained Pareto-optimal` 을 사용. 엄밀한 수학 용어 (strictly: "Pareto-optimal under coverage × depth × items-lost × cost" 또는 "4-axis Pareto-optimal") 의 약어로 이해한다. 주체자 정정 우선 시 엄밀 form 으로 rewrite 가능.
...
### 3.2 Cost-constrained Pareto frontier

Frontier 는 4-axis (coverage × depth × items-lost × cost) 하에서 정의.
§1.1.2 에 qualifier 이유 기술 — cost 없이는 k=9 가 모든 k<9 을 dominate.
...
| **6** | — | **cost-constrained Pareto-optimal** |
| 7 | k=8 when accepting +1 cost | 마진 감소 |
| 8 | k=9 when requiring axiology | 17 session lost |
| 9 | reference | Full 9 lens = 100% / 100% / 0 (cost 9) |

**k=6 이 유일 cost-constrained Pareto-optimal** (4-axis 에서 dominated by none). K=7, 8 은 cost 대비 marginal gain 감소 구간, k=9 는 full (cost 최대).
...
| **6 (Option P')** | {axiology, coverage, evolution, logic, semantics, structure} — **cost-constrained Pareto-optimal** |
| 7 | + dependency |
...
1. **Cost-constrained Pareto-optimal**: k=3~9 중 coverage × depth × items-lost × cost 4-axis 에서 dominated 아님 (cost 포함 이유는 §1.1.2 참조)

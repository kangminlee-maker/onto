---
as_of: 2026-04-19
status: canonical (pending-lexicon-sync)
...
related_authorities:
  ...
  - "design-principles/project-locality-principle.md (rename → product-locality — §9)"
...
| **강제력 — 소비 경계 (consumption)** | **hard**. `project-locality-principle.md` §2.2 강제 — **promoted Learning 만** consult pool 진입. seed/candidate/provisional 단계 Experience·Learning 은 consumption 대상 아님 (creation·promotion 의 입력으로만 사용) |
...
{product}/.onto/
  ...
  ├── domains/{X}/ (override)           ← project-locality §2.3 domain override
...
| 6 | `promote_and_generalize` (target=methodology) | compound | **definition only** | v1 불허. Principal manual 만 |
| 7 | `canonicalize` | tier | **definition only** | Tier 1·2 → Tier 3. snapshot copy |
| 8 | `norm_update` (domain) | Tier 1 → Tier 2 | **active** | promote.md Step 7 권고 |
| 9 | `medium_norm_update` | Tier 1 → Tier 2 | **definition only** | — |
| 10 | `ontology_update` | Tier 1 → Tier 2 | **definition only** | — |
| 11 | `re_classification` | scope 내 | **definition only** | medium ↔ domain primary 변경 |
| 12 | `retire` | lifecycle 종료 | **definition only** | M-08 부분 구현 |
...
| zone | 좌표 | learn 자동 write | govern 강제 |
|---|---|---|---|
| `{product}/.onto/experiences/` | T1 Exp, product (cell 8 Exp) | ✅ | ❌ |
| `{product}/.onto/learnings/` | T1 Learn, product (cell 8 Learn) | ✅ | ❌ |
| `~/.onto/mediums/{X}/experiences/` | T1 Exp, medium (cell 9 Exp) | ⏸ definition only | ❌ |
| `~/.onto/mediums/{X}/learnings/` | T1 Learn, medium (cell 9 Learn) | ✅ (promote_and_generalize) | ❌ |
| `~/.onto/domains/{X}/experiences/` | T1 Exp, domain (cell 10 Exp) | ⏸ definition only | ❌ |
| `~/.onto/domains/{X}/learnings/` | T1 Learn, domain (cell 10 Learn) | ✅ (promote_and_generalize) | ❌ |
...
| `~/.onto/domains/{X}/{8 doc}` | T2, domain (cell 7) | ✅ (`norm_update` — promote.md Step 7) | ❌ |
| `{product}/.onto/principles/` (일반 product) | T3, product (cell 1) | ⏸ patch only (canonicalize) | ✅ 강제 |
| **onto repo root** (authority/, design-principles/, processes/) | T3, product (self-hosted 특수 배치) | ⏸ patch only | ✅ 강제 |
| `~/.onto/mediums/{X}/principles/` | T3, medium (cell 2) | ⏸ patch only | ✅ 강제 (정의만) |
| `~/.onto/domains/{X}/principles/` | T3, domain (cell 3) | ⏸ patch only | ✅ 강제 (정의만) |
| `~/.onto/principles/` | T3, methodology (cell 4) | ⏸ patch only | ✅ 강제 (정의만) |

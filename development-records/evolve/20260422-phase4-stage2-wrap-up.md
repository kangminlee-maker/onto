...
| **queue** | 사전 | 본질 sink: `.onto/govern/queue.ndjson` (event-sourced ledger, Step 4 §1.3). transient seat: `.onto/govern/staging/<proposal-id>/` (Principal decide 후 cleanup — 본질 sink 아님) | 독립 (reader) | `onto govern list-drift-candidates → show → decide-staged` |
...
`queue.ndjson` 은 single seat 이지만 5 entry kind (pending decision / decided verdict / apply_failed / manual_edit / v0 freeform) — Step 4 §1.3 정본 참조. govern.md §14.6 의 govern queue 행도 동일 5-kind 명세. consumer 는 entry kind 로 필터링.

`.onto/govern/staging/<proposal-id>/` 은 **transient seat** (Principal decide 직후 삭제) 이라 invariant 의 "본질 sink = permanent canonical recording location" 정의에 해당하지 않음 — 본 매트릭스의 queue route 는 본질 sink (queue.ndjson) + transient seat (staging) 를 함께 사용하는 패턴.
...
#### 6.1.1 Lifecycle backlog (PR #198 review 2차 consensus 명시)

본 PR 은 설계 합의 only 이라 다음 lifecycle 정의는 구현 세션 책임. 본 backlog 는 review 가 발견한 gap 을 구현 세션에서 누락 없이 처리하기 위한 명시:

- **CF-apply-failed-recovery-gap**: `GovernApplyFailedEvent` 가 staging 을 보존하고 reject 와 구별하지만 operator-facing discovery / retry / cleanup path 미정의. 구현 시 (a) `onto govern list-failed` 같은 discovery CLI, (b) 재시도/폐기 결정 surface, (c) stale staging 의 자동 cleanup 정책 정의 필요. 미정의 시 staging directory 가 orphan 누적
- **CF-pause-scope-and-resume-gap**: pause 의 scope 가 promote-executor 의 mutation step 진입 직전 flag 체크 (Step 4 §3.3) 인데 surrounding text 가 broader gate 처럼 표현 + resume 후 skipped 자동 work 의 재진입 정책 미정의. 구현 시 (a) "Emergency" 단어의 적절성 재검토 (실제로는 "auto-pause" 가 더 정확), (b) skipped work 가 다음 promote session 에서 자동 retry 되는지 또는 별도 명령으로 trigger 되는지 결정 필요
- **CF-principal-direct-observability-gap**: `manual_edit` marker 가 queue.ndjson 에 쓰이지만 query/read surface 부재. 구현 시 `onto govern list --kind manual_edit` 또는 `onto govern history --target <path>` 같은 read surface 신설 필요
...
- UF-step4-over-specification + UF-step4-decision-multi-owner-drift (2차 review 동족): Step 4 의 같은 결정이 §6 + §9 + wrap-up §2.5 multi-owner — 양식 (option table + decision record + summary) 의 자연 결과. 광범위 trim 은 별도 PR scope (양식 자체 재검토 필요)
- UF-purpose-alignment-evidence-gap (axiology, 2차 review): review wrapper (`scripts/review-pr.sh`) 가 PR diff 만 격리 전달하므로 axiology lens 가 rank 1-3 authority binding 을 내부에서 못 봄. 본 PR finding 아닌 review process 자체 backlog — wrapper 가 상위 authority anchor 를 selective 하게 함께 전달하는 옵션 검토 가치

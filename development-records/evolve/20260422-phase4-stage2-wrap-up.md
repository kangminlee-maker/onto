...
| Route | 개입 시점 | sink 위치 | dogfood 의존 | Principal action UI |
|---|---|---|---|---|
| **self_apply** | 사후 | `.onto/govern/self-apply-log/<session>/events.jsonl` | 독립 (reader) | promote session stdout summary |
| **queue** | 사전 | `.onto/govern/staging/<proposal-id>/` + `.onto/govern/queue.ndjson` | 독립 (reader) | `onto govern list-drift-candidates → show → decide-staged` |
| **principal_direct** | 대신 | `.onto/govern/queue.ndjson` (route=principal_direct marker) | 독립 (reader) | 본인 편집 + `onto govern record-manual-edit` |

모든 본질 sink 가 govern namespace 내부 → §0.1 invariant 충족 → dogfood off → 모든 route 정상 작동.
...
- promote-executor 의 staging apply 실패 시 자동 retry 정책 — Step 4 §2.3

...
**semantic grouping 이 필요하면**: Step 3 Hook γ Rationale Reviewer (LLM-owned seat) 에 이연. Reviewer 가 wip.yml 을 전수 읽을 때 `pack_missing_areas` 를 refine. 본 Step 2 의 `pack_missing_areas` population 은 **provisional** — Step 3 에서 재작성 가능.

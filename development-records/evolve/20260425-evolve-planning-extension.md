---
as_of: 2026-04-25
last_diagnosed: 2026-04-28  # post-PR #244 cross-check — §3.1.0 현재 구조 진단 추가
status: design-draft
...
purpose: |
  ...
  Post-write 진단 (2026-04-28): main HEAD 검증 결과 adapter 분리
  (`adapters/code-product/` ↔ `adapters/methodology/scope-types/process.ts`)
  는 이미 도입되어 있고, entry_mode 3-enum 도 CLI 에 노출된 상태. 본 설계의
  실제 작업은 *기존 분리 위에 entry_mode 분기 + 후반부 wiring 을 마감*
  하는 것이며 *신규 adapter 분리 도입* 이 아님. §3.1.0 참조.
input_artifacts:
  friction_log: development-records/evolve/20260425-dogfood-friction-log-task1.md (PR #215, merged f544bfd)
  ...
  current_split_adapters:
    code_product: src/core-runtime/evolve/adapters/code-product/compile/
    methodology: src/core-runtime/evolve/adapters/methodology/scope-types/process.ts
    cli_entry_mode: src/core-runtime/evolve/cli.ts:110
scope_decision:
  in_this_design:
...
#### 3.1.0 현재 구조 진단 (post-PR #244 main 기준, 2026-04-28 cross-check)

본 설계 작성 시점에는 evolve 후반부가 *단일 pipeline* 처럼 보이지만, main 검증 결과 **adapter 디렉토리 분리는 이미 도입되어 있음** (sprint-kit 흡수 시 — `development-records/absorptions/sprint-kit-20260410.md`):

- `src/core-runtime/evolve/adapters/code-product/compile/` — code-product 의 `compile.ts` + `compile-defense.ts` 가 분리된 adapter 디렉토리에 존재
- `src/core-runtime/evolve/adapters/methodology/scope-types/process.ts` — **process scope type 자체는 이미 정의됨** (authority sources / target document / perspectives 필드)
- `src/core-runtime/evolve/cli.ts:110` — `entry_mode` 가 `"experience" | "interface" | "process"` 3-enum 으로 노출됨 (CLI 인자 수용)

**그러나 후반부 wiring 이 미완성**:

- `scope-types/process.ts:34` 의 `entry_mode: "experience"` — process scope type 의 default entry_mode 가 `"experience"` 로 fallback. 즉 *type* 으로는 분리됐으나 *entry_mode* 차원에선 아직 experience 로 매핑.
- `commands/draft.ts:337-344` 의 `handleCompile` — entry_mode 분기 없이 `compile(action.compileInput)` 직접 호출. compile 은 code-product adapter 의 출력 (build-spec / brownfield-detail / delta-set / validation-plan) 을 강제 — process scope type 으로 진입한 흐름도 동일 path 를 탐.
- 결과: `process` entry_mode 가 *수용* 은 되지만 *후반부 분기는 미구현* — F-15 (code-product 편향) 의 직접 원인.

따라서 **본 설계의 §3.1 핵심은 "기존 분리 구조 (`adapters/code-product` ↔ `adapters/methodology/scope-types/process.ts`) 위에 *entry_mode 분기 + 후반부 generate_surface/apply path* 를 완성"** 하는 것. *신규 분리 도입* 이 아니라 *기존 분리의 미완성 영역 마감* 이라는 framing 이 더 정확.

#### 3.1.1 분기 후 surface / compile / apply 매트릭스
...
| **`process`** | **design-doc draft (markdown)** | **n/a (compile 생략)** | **design-doc commit + PR** | **design-doc.md** |

**구현 위치 (진단 반영)**:
- surface 분기: `evolve/adapters/methodology/scope-types/process.ts` 의 `entry_mode` default 를 `"process"` 로 수정 + `processScopeDefaults.surface_type: "markdown"` 보존
- compile skip: `commands/draft.ts:handleCompile` 에 `entry_mode === "process"` 분기 추가 → "process mode 에선 compile 생략, apply 로 진행" 안내 후 early-return
- apply 분기: `commands/apply.ts` 에 process branch 추가 (§3.4 참조)
...
**진단 반영**: §3.1.0 의 분리 구조 (`adapters/code-product/` ↔ `adapters/methodology/scope-types/process.ts`) 가 이미 존재 — 본 Phase 의 작업은 *그 위에 wiring 완성*.

1. `scope-types/process.ts` 의 `entry_mode` default 를 `"experience"` → `"process"` 로 수정 + `surface_type: "markdown"` 보존 (§3.1.1 진단)
2. `commands/draft.ts:handleCompile` 에 `entry_mode === "process"` 분기 추가 — compile skip + early-return + 안내 메시지
3. `draft generate_surface` 의 process 분기: §3.3 frontmatter 골격 markdown 을 `surface/design-doc-draft.md` 로 출력
4. `commands/apply.ts` 에 process 분기: design doc 이동 + git commit (§3.4)
5. `--push-pr` 옵션 + `--commit-message` / `--pr-title` / `--pr-body`
6. `e2e-evolve-full-cycle.test.ts` 에 process mode 시나리오 추가 (`start → align → draft (process) → apply` 가 design doc commit 으로 종결)
...
- **`scope-types/process.ts:34` 의 `entry_mode: "experience"` 가 의도적 fallback 인지, 미완성 wiring 의 흔적인지 확인 필요**: 진단 (2026-04-28) 결과 process scope type 이 `entry_mode` 차원에선 experience 로 매핑됨. Phase B Step 1 가 이 값을 `"process"` 로 수정하는데, 기존 코드의 다른 호출 경로 (만약 있다면) 가 이 default 에 의존하고 있는지 grep 으로 회귀 surface 파악 후 변경.

...
import { printOntoReleaseChannelNotice, readOntoVersion, } from "./core-runtime/release-channel/release-channel.js";
import { readSingleOptionValueFromArgv, } from "./core-runtime/review/review-artifact-utils.js";
/**
 ...
 */
async function checkOntoDirectoryInit(projectRoot, ontoHome, argv) {
    ...
    if (path.resolve(projectRoot) === path.resolve(ontoHome))
        return true;
    // Non-interactive: require --allow-onto-init
    if (argv.includes("--allow-onto-init"))
        return true;
...
async function handleReview(ontoHome, argv) {
    ...
    if (!argv.includes("--project-root")) {
        const firstNonFlag = argv.find((arg) => !arg.startsWith("--") && !arg.startsWith("@"));
    ...
    if (projectRoot) {
        const allowed = await checkOntoDirectoryInit(path.resolve(projectRoot), ontoHome, argv);
        if (!allowed) {
            console.error("[onto] Review canceled: .onto/ directory creation not approved.");
            return 1;
        }
    }
    const { runReviewInvokeCli } = await import("./core-runtime/cli/review-invoke.js");
    return runReviewInvokeCli(enrichedArgv);
}
async function handleCompleteSession(ontoHome, argv) {
    const enrichedArgv = [...argv];
    if (!argv.includes("--onto-home")) {
        enrichedArgv.push("--onto-home", ontoHome);
    }
    const { runCompleteReviewSessionCli } = await import("./core-runtime/cli/complete-review-session.js");
    return runCompleteReviewSessionCli(enrichedArgv);
}
async function handleCoordinator(ontoHome, argv) {
    ...
    switch (subcommand) {
        case "start": {
            ...
            if (!enrichedArgv.includes("--project-root")) {
                const firstNonFlag = enrichedArgv.find((arg) => !arg.startsWith("--") && !arg.startsWith("@"));
            ...
            const projectRoot = prIdx >= 0 && prIdx + 1 < enrichedArgv.length
                ...
                : undefined;
            if (projectRoot) {
                const allowed = await checkOntoDirectoryInit(path.resolve(projectRoot), ontoHome, enrichedArgv);
                if (!allowed) {
                    console.error("[onto] Coordinator canceled: .onto/ directory creation not approved.");
                    return 1;
                }
            }
            const { coordinatorStart } = await import("./core-runtime/cli/coordinator-state-machine.js");
            try {
                ...
                console.error(`[onto] coordinator start failed: ${error instanceof Error ? error.message : String(error)}`);
                return 1;
            }
        }
        case "next": {
            ...
            const { coordinatorNext } = await import("./core-runtime/cli/coordinator-state-machine.js");
            ...
            try {
                ...
                console.error(`[onto] coordinator next failed: ${error instanceof Error ? error.message : String(error)}`);
                return 1;
            }
        }
        case "status": {
            const { coordinatorStatus } = await import("./core-runtime/cli/coordinator-state-machine.js");
            ...
            try {
                ...
                console.error(`[onto] coordinator status failed: ${error instanceof Error ? error.message : String(error)}`);
                return 1;
            }
        }
        case "--help":
 ...
 */
function detectInstallationMode(ontoHome, projectRoot) {
...
async function handleMigrateSessionRoots(argv) {
    const projectRoot = resolveProjectRoot();
    ...
    const { migrateSessionRoots } = await import("./core-runtime/cli/migrate-session-roots.js");
...
async function handlePromote(ontoHome, argv) {
    const projectRoot = resolveProjectRoot();
    // DD-20: wire the builtin spec registrar before any REGISTRY use.
    await import("./core-runtime/learning/shared/artifact-registry-init.js");
    // Migration gate (DD-8)
    try {
        const { ensureSessionRootsMigrated } = await import("./core-runtime/cli/session-root-guard.js");
...
async function handlePromoteAnalyze(ontoHome, projectRoot, argv) {
    const sessionId = readSingleOptionValueFromArgv(argv, "session-id") ?? generateSessionId();
    const skipPanel = argv.includes("--skip-panel");
    const skipAudit = argv.includes("--skip-audit");
    const { runPromoter } = await import("./core-runtime/learning/promote/promoter.js");
...
async function handlePromoteApply(ontoHome, projectRoot, argv) {
    ...
    const { runPromoteExecutor } = await import("./core-runtime/learning/promote/promote-executor.js");
...
async function handlePromoteResolveConflict(projectRoot, argv) {
    ...
    if (!sessionId || sessionId.startsWith("--") || !selectedAttemptId) {
        console.error("[onto] usage: onto promote --resolve-conflict <session-id> " +
            "--select <attempt-id> [--note '<reason>']");
        return 1;
    }
    const { gatherRecoveryContext, resolveRecoveryTruth, saveRecoveryResolution } = await import("./core-runtime/learning/shared/recovery-context.js");
    ...
    if (resolved.kind !== "manual_escalation_required") {
        console.error(`[onto] no manual escalation required for session ${sessionId} ` +
    ...
    if (!valid.has(selectedAttemptId)) {
        console.error(`[onto] selected attempt_id ${selectedAttemptId} not in conflicting attempts. ` +
            `Valid: ${[...valid].join(", ")}`);
        return 1;
    }
...
async function handlePromoteStatus(projectRoot, argv) {
    ...
    const { loadApplyState } = await import("./core-runtime/learning/promote/apply-state.js");
...
async function handleReclassifyInsights(ontoHome, argv) {
    const projectRoot = resolveProjectRoot();
    // DD-20: wire the builtin spec registrar before any REGISTRY use.
    await import("./core-runtime/learning/shared/artifact-registry-init.js");
    try {
    ...
    if (applyReportPath !== undefined) {
        const { applyInsightReclassifications } = await import("./core-runtime/learning/promote/insight-reclassifier.js");
    ...
    const { runInsightReclassifier } = await import("./core-runtime/learning/promote/insight-reclassifier.js");
...
async function handleInfo(ontoHome) {
    const projectRoot = resolveProjectRoot();
    ...
    switch (subcommand) {
        case "review":
            ...
            if (subcommandArgv.includes("--complete-session")) {
                const filteredArgv = subcommandArgv.filter((arg) => arg !== "--complete-session");
        ...
        case "evolve": {
            const { handleEvolveCli } = await import("./core-runtime/evolve/cli.js");
            return handleEvolveCli(ontoHome, subcommandArgv);
        }
        case "reconstruct": {
            const { handleReconstructCli } = await import("./core-runtime/evolve/commands/reconstruct.js");
        ...
        case "govern": {
            const { handleGovernCli } = await import("./core-runtime/govern/cli.js");
            return handleGovernCli(ontoHome, subcommandArgv);
        }
        case "config": {
            const { handleConfigCli } = await import("./core-runtime/config/onto-config-cli.js");
            return handleConfigCli(ontoHome, subcommandArgv);
        }
        case "install": {
            const { handleInstallCli } = await import("./core-runtime/install/cli.js");
            return handleInstallCli(ontoHome, subcommandArgv);
        }
        case "learn":
        case "build":
        case "ask":
            console.error(`[onto] Subcommand '${subcommand}' is not yet implemented in global CLI.`);
            return 1;
        case "--version":
        case "-v": {
            const version = await readOntoVersion();

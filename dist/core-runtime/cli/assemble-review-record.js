 ...
 */
async function readOrchestratorReportedRealization(sessionRoot) {
    const coordinatorStatePath = path.join(sessionRoot, "coordinator-state.yaml");
    if (!(await fileExists(coordinatorStatePath))) {
        return {};
    }
    try {
        const stateFile = await readYamlDocument(coordinatorStatePath);
...
async function detectDeliberationStatus(executionResult, synthesisPath, deliberationPath, binding) {
    ...
    if (executionResult?.deliberation_status === "not_needed" ||
...
async function summarizeErrorLog(errorLogPath) {
    ...
    return {
        ...
        hasExecutionFailure: /\|\s+(?:lens|synthesize) failure:\s+/m.test(errorLogText),
...
export async function runAssembleReviewRecordCli(argv) {
    ...
    if (executionResult) {
        for (const unitResult of executionResult.lens_execution_results) {
            ...
            lensResultRefs[unitResult.unit_id] = toRelativePath(unitResult.output_path, projectRoot);
        }
    }
    ...
    const reviewRecord = {
        ...
        degradation_notes_ref: ((executionResult?.execution_status !== "completed") ||
        ...
        synthesis_result_ref: (await fileExists(synthesisPath))
            ...
            : null,
        deliberation_status: await detectDeliberationStatus(executionResult, synthesisPath, deliberationPath, invocationBindingArtifact),

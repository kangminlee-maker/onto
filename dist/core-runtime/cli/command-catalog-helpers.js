 ...
 */
import { CURRENT_CATALOG_VERSION, META_NAME_REGISTRY, } from "./command-catalog.js";
const BARE_ONTO_SENTINEL = "<<bare>>";
function addOrThrow(set, key, target) {
    if (set.has(key)) {
        const existing = set.get(key);
        throw new Error(`Normalized invocation collision: "${key}" already mapped. ` +
            `existing=${JSON.stringify(existing)}, new=${JSON.stringify(target)}`);
    }
    set.set(key, target);
}
export function getNormalizedInvocationSet(catalog) {
    const set = new Map();
            ...
            for (const r of entry.realizations) {
                if (r.kind === "slash" || r.kind === "cli") {
                    ...
                    for (const param of r.parameter_set) {
                        const expanded = r.invocation_pattern.replace(`{${r.parameter_name}}`, param);
...
// ---------------------------------------------------------------------------
function isReservedInvocation(invocation) {
    return invocation.startsWith("<<") || invocation.startsWith(">>");
}
export function assertReservedNamespaceUnused(catalog) {
    for (const entry of catalog.entries) {
        if (entry.kind === "public") {
            for (const r of entry.realizations) {
                const inv = r.kind === "patterned_slash" ? r.invocation_pattern : r.invocation;
                if (isReservedInvocation(inv)) {
                    throw new Error(`Reserved namespace violation: "${inv}" in PublicEntry "${entry.identity}". ` +
            ...
            for (const r of entry.realizations) {
                if (isReservedInvocation(r.invocation)) {
                    throw new Error(`Reserved namespace violation: "${r.invocation}" in MetaEntry "${entry.name}".`);
                }
            }
        }
    }
...
// ---------------------------------------------------------------------------
export function assertNoRuntimeScriptCollision(catalog) {
    const seen = new Set();
    for (const entry of catalog.entries) {
        if (entry.kind !== "runtime_script")
            continue;
        if (seen.has(entry.name)) {
            throw new Error(`RuntimeScriptEntry collision: "${entry.name}" appears more than once.`);
        }
        seen.add(entry.name);
    }
}
...
// ---------------------------------------------------------------------------
export function assertMetaNameRegistered(catalog) {
    const registered = new Set(META_NAME_REGISTRY);
    for (const entry of catalog.entries) {
        if (entry.kind !== "meta")
            continue;
        if (!registered.has(entry.name)) {
            throw new Error(`MetaEntry.name "${entry.name}" not in META_NAME_REGISTRY ` +
                `[${[...registered].join(", ")}].`);
        }
    }
}
...
function collectAllNames(catalog) {
    const names = new Set();
    for (const entry of catalog.entries) {
...
export function assertSuccessorReferenceExists(catalog) {
    const allNames = collectAllNames(catalog);
    for (const entry of catalog.entries) {
        if (entry.successor && !allNames.has(entry.successor)) {
            throw new Error(`successor reference not found: entry "${entryName(entry)}" ` +
                `successor="${entry.successor}".`);
        }
    }
}
export function assertRuntimeScriptsReferenceExists(catalog) {
    const runtimeScriptNames = new Set();
    for (const entry of catalog.entries) {
        if (entry.kind === "runtime_script")
            runtimeScriptNames.add(entry.name);
    }
    for (const entry of catalog.entries) {
        if (entry.kind !== "public" || !entry.runtime_scripts)
            continue;
        for (const ref of entry.runtime_scripts) {
            if (!runtimeScriptNames.has(ref)) {
                throw new Error(`runtime_scripts reference not found: PublicEntry "${entry.identity}" ` +
...
function compareSemver(a, b) {
    const parts = (s) => s.split(".").map((p) => Number.parseInt(p, 10));
    const av = parts(a);
    const bv = parts(b);
...
export function assertDeprecationLifecycle(catalog) {
    for (const entry of catalog.entries) {
        if (entry.removed_in && !entry.deprecated_since) {
            throw new Error(`Deprecation lifecycle violation: entry "${entryName(entry)}" has ` +
        ...
        if (entry.removed_in &&
            ...
            throw new Error(`Deprecation lifecycle violation: entry "${entryName(entry)}" ` +
...
// ---------------------------------------------------------------------------
export function assertRepairPathPreboot(catalog) {
    for (const entry of catalog.entries) {
        if (entry.kind !== "public")
            continue;
        if (entry.repair_path === true && entry.phase !== "preboot") {
            throw new Error(`repair_path constraint violation: PublicEntry "${entry.identity}" ` +
...
// ---------------------------------------------------------------------------
export function assertEntryRealizationsNonEmpty(catalog) {
    for (const entry of catalog.entries) {
        if (entry.kind === "public" || entry.kind === "meta") {
            if (entry.realizations.length === 0) {
                throw new Error(`Empty realizations: ${entry.kind} entry "${entryName(entry)}" ` +
...
// ---------------------------------------------------------------------------
export function assertNoAliasCollision(catalog) {
    const seen = new Map();
    for (const entry of catalog.entries) {
        if (!entry.aliases)
            continue;
        for (const alias of entry.aliases) {
            if (seen.has(alias)) {
                throw new Error(`Alias collision: "${alias}" claimed by both ` +
...
// ---------------------------------------------------------------------------
export function assertCatalogVersionSupported(catalog) {
    if (catalog.version !== CURRENT_CATALOG_VERSION) {
        throw new Error(`Unsupported catalog.version: declared=${catalog.version}, ` +

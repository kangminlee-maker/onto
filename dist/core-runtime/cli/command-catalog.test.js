...
describe("COMMAND_CATALOG (real catalog)", () => {
    ...
    it("includes deprecated legacy entries with valid lifecycle", () => {
        const deprecated = COMMAND_CATALOG.entries.filter((e) => e.deprecated_since !== undefined);
        const names = deprecated.map((e) => e.kind === "public" ? e.identity : "name" in e ? e.name : "");
    ...
    it("repair_path commands (install, config) are preboot", () => {
        const repairPathEntries = COMMAND_CATALOG.entries.filter((e) => e.kind === "public" && e.repair_path === true);
        expect(repairPathEntries.length).toBeGreaterThanOrEqual(2);
        for (const entry of repairPathEntries) {
            if (entry.kind === "public")
                expect(entry.phase).toBe("preboot");
        }
    });

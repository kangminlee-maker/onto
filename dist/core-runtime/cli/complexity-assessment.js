...
import { callLlm, resolveLearningProviderConfig, } from "../learning/shared/llm-caller.js";
...
// ---------------------------------------------------------------------------
function resolveMainLlmConfig(ontoConfig) {
    ...
    if (mainLlm?.provider) {
        ...
        if (mainLlm.model)
            partial.model_id = mainLlm.model;
        if (mainLlm.base_url)
            partial.base_url = mainLlm.base_url;
        if (mainLlm.max_tokens)
            partial.max_tokens = mainLlm.max_tokens;
        return partial;
    }
    if (ontoConfig.subagent_llm?.provider) {
        const partial = {
            provider: ontoConfig.subagent_llm.provider,
        };
        if (ontoConfig.subagent_llm.model)
            partial.model_id = ontoConfig.subagent_llm.model;
        if (ontoConfig.subagent_llm.base_url)
            partial.base_url = ontoConfig.subagent_llm.base_url;
        return partial;
    }
...
export async function assessComplexity(targetDescription, reviewIntent, ontoConfig) {
...
export async function selectLenses(targetDescription, reviewIntent, ontoConfig) {
    const llmConfig = resolveMainLlmConfig(ontoConfig);

...
import { callLlmWithTools, } from "../learning/shared/llm-tool-loop.js";
...
export async function runInlineHttpReviewUnitExecutorCli(argv) {
    ...
    const toolModeFromConfig = typeof ontoConfig.subagent_llm?.tool_mode === "string"
        ? ontoConfig.subagent_llm.tool_mode
        : undefined;
    ...
    if (requestedToolMode === "native" && toolLoopProvider === null) {
        throw new Error(`--tool-mode=native requires provider in {anthropic, openai, litellm}; got "${cliOverrides.provider ?? "(auto)"}".`);
    }
    ...
    if (tryNative && toolLoopProvider) {
        ...
        if (!modelForLoop) {
            throw new Error(`tool-native mode requires a model id (set --model, OntoConfig.${toolLoopProvider}.model, or OntoConfig.model).`);
        }
        try {

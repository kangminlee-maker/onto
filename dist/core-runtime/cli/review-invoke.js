...
const KNOWN_PASSTHROUGH_FLAG_NAMES = ["codex"];
        ...
        try {
            if (fsSync.existsSync(srcPath)) {
                ...
                if (srcStat.mtimeMs > distStat.mtimeMs) {
                    process.stderr.write(`[onto] WARNING: dist/${filename}.js is older than src/${filename}.ts. ` +
...
function appendExecutorModelArgs(config, argv, ontoConfig) {
    ...
    const model = readSingleOptionValueFromArgv(argv, "model") ??
        ontoConfig?.model ??
 ...
 */
function appendSubagentLlmArgs(config, ontoConfig) {
    const args = [...config.args];
 ...
 */
export function detectClaudeCodeHost() {
 ...
 */
export function detectCodexAvailable() {
...
export function resolveExecutionProfile(args) {
    ...
    let host_runtime = plan.host_runtime === "mock" ? "standalone" : plan.host_runtime;
    ...
    const execution_realization = plan.execution_realization === "mock" ? "ts_inline_http" : plan.execution_realization;
    return {
        type: "resolved",
 ...
 */
export function tryResolveTopologyForHandoff(ontoConfig, cached) {
    if (ontoConfig === undefined)
        return null;
 ...
 */
export function tryTopologyDerivedExecutor(ontoConfig, ontoHome, cached) {
    if (ontoConfig === undefined)
        return null;
    if (!ontoHome)
        return null;
    ...
    if (!hasStandaloneLensExecutor(topology))
        return null;
    try {
        const base = mapTopologyToExecutorConfig(topology, ontoHome, ontoConfig);
        process.stderr.write(`[plan:executor] topology=${topology.id} bin=${base.bin} ` +
...
function resolveExecutorConfig(argv, optionPrefix, ontoConfig, ontoHome, cachedTopology) {
    ...
    if (typeof explicitBin === "string" && explicitBin.length > 0) {
        return appendExecutorModelArgs({ bin: explicitBin, args: explicitArgs }, argv, ontoConfig);
    }
    ...
    if (explicitRealization === "codex" || explicitRealization === "mock" || explicitRealization === "ts_inline_http") {
        return appendExecutorModelArgs(buildExecutorConfigFromRealization(explicitRealization, ontoHome), argv, ontoConfig);
    }
    ...
    const topologyDerived = tryTopologyDerivedExecutor(ontoConfig, ontoHome, cachedTopology);
    if (topologyDerived) {
    ...
    const hasExternalProvider = (subagentLlm && typeof subagentLlm.provider === "string" && subagentLlm.provider.length > 0) ||
        Boolean(ontoConfig?.external_http_provider);
    if (hasExternalProvider) {
        return appendSubagentLlmArgs(buildExecutorConfigFromRealization("ts_inline_http", ontoHome), ontoConfig);
    }
    return appendExecutorModelArgs(buildExecutorConfigFromRealization("codex", ontoHome), argv, ontoConfig);
}
...
async function resolveTargetInput(projectRoot, requestedTarget, explicitFilesystemAllowedRoots, argv) {
...
async function resolveReviewInvokeInputs(argv, ontoConfig, projectRoot) {
    ...
    if (typeof diffRange === "string" && diffRange.length > 0) {
        ...
        if (typeof primaryRefRaw !== "string" || primaryRefRaw.length === 0) {
            throw new Error("Bundle review target requires --primary-ref or at least one --member-ref.");
        }
        ...
        resolvedTargetRefs = orderedRefs.filter((resolvedRef, index) => orderedRefs.indexOf(resolvedRef) === index);
    }
    else {
        const resolvedTargetInput = await resolveTargetInput(projectRoot, requestedTarget, explicitFilesystemAllowedRoots, argv);
...
async function readOptionalReviewSummary(sessionRoot) {
    ...
    const reviewRecord = (await fileExists(reviewRecordPath))
        ? await readYamlDocument(reviewRecordPath)
        : null;
    return {
        reviewRecord,
        binding,
...
function resolveMaxConcurrentLenses(argv, ontoConfig) {
...
function rejectRemovedFlags(argv) {
    if (hasOptionFlag(argv, "claude")) {
        throw new Error("--claude is no longer accepted. The `onto review` CLI runs Codex only. " +
    ...
    for (const removed of ["host-runtime", "execution-realization", "execution-mode"]) {
        ...
        if (present) {
            throw new Error(`--${removed} is no longer accepted. The \`onto review\` CLI is codex-only; ` +
                "remove the flag.");
        }
    }
}
...
async function resolveReviewInvokeSetup(argv) {
    ...
    const resolvedInvokeInputs = await resolveReviewInvokeInputs(argv, ontoConfig, projectRoot);
    const maxConcurrentLenses = resolveMaxConcurrentLenses(argv, ontoConfig);
    const { optionTokens: argvWithoutPositionals } = splitArgvIntoOptionsAndPositionals(argv, [...KNOWN_INVOKE_ONLY_OPTION_NAMES, ...KNOWN_PASSTHROUGH_OPTION_NAMES], [...KNOWN_INVOKE_ONLY_FLAG_NAMES, ...KNOWN_PASSTHROUGH_FLAG_NAMES]);
    const normalizedStartArgv = appendReviewInvokeDerivedArgs(ensureSessionIdArg(stripOptionsFromArgv(argvWithoutPositionals, [...KNOWN_INVOKE_ONLY_OPTION_NAMES], [...KNOWN_INVOKE_ONLY_FLAG_NAMES])), resolvedInvokeInputs);
    ...
    const executionProfile = profileResolution.type === "resolved" ? profileResolution.profile : null;
    const profileForArtifacts = executionProfile ?? {
        execution_realization: "subagent",
        host_runtime: "codex",
    };
    const startArgvWithProfile = appendCanonicalExecutionProfileArgs(normalizedStartArgv, profileForArtifacts);
...
export async function runReviewInvokeCli(argv) {
    ...
    const cachedTopology = cachedTopologyResolution.type === "resolved"
        ? cachedTopologyResolution.topology
        : null;
    const resolvedRequestText = setup.resolvedInvokeInputs.requestText;
    const defaultExecutorConfig = resolveExecutorConfig(argv, "", setup.ontoConfig, setup.ontoHome, cachedTopology);
    const synthesizeExecutorConfig = resolveExecutorConfig(argv, "synthesize-", setup.ontoConfig, setup.ontoHome, cachedTopology);
    ...
    const topologyForDispatch = cachedTopology ?? undefined;
    ...
    const routeProfile = setup.executionProfile ?? {
        execution_realization: "subagent",
        host_runtime: "codex",
    };
    const routeSummary = {
    ...
    console.log(JSON.stringify({
        ...
        review_result: {
            ...
            deliberation_status: reviewSummary.reviewRecord?.deliberation_status ?? null,
            participating_lens_ids: reviewSummary.reviewRecord?.participating_lens_ids ??
                promptExecutionResult.participating_lens_ids,
            degraded_lens_ids: reviewSummary.reviewRecord?.degraded_lens_ids ??
        ...
        max_concurrent_lenses: setup.maxConcurrentLenses,
        executor_realization: defaultExecutorConfig.bin === packageManagerBin()

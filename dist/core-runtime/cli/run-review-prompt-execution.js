...
function defaultMaxConcurrentLensesForExecutorConfig(executorConfig) {
...
async function invokeExecutor(executorConfig, projectRoot, sessionRoot, dispatch) {
    ...
    const child = spawn(executorConfig.bin, [
        ...
        env: (() => {
            ...
            if (typeof ontoHome !== "string" || ontoHome.length === 0) {
                throw new Error("ONTO_HOME not set when spawning lens executor. " +
...
async function readStructuredDeliberationStatus(synthesizeOutputPath, deliberationOutputPath, executionRealization) {
    ...
    if (await fileExists(synthesizeOutputPath)) {
        ...
        if (frontmatterStatus === "not_needed" ||
            frontmatterStatus === "performed" ||
            frontmatterStatus === "required_but_unperformed") {
            return frontmatterStatus;
        }
    }
    if (executionRealization === "agent-teams" &&
        (await fileExists(deliberationOutputPath))) {
        return "performed";
    }
    return null;
}
async function writeExecutionResultArtifact(executionPlan, artifact) {
...
export async function executeReviewPromptExecution(params) {
    ...
    const maxConcurrentLenses = Math.max(1, Math.min(params.maxConcurrentLenses ?? 1, executionPlan.lens_prompt_packet_seats.length || 1));
    ...
    if (params.topology?.id === "codex-nested-subprocess") {
        console.log(`[review runner] topology=codex-nested-subprocess → outer codex teamlead dispatch (bypasses per-lens worker pool)`);
        await appendExecutionProgress(executionPlan.error_log_path, "runner topology dispatch: codex-nested-subprocess", [
    ...
    for (let attempt = 0; attempt <= synthesizeMaxRetries; attempt++) {
        try {
            await invokeExecutor(synthesizeExecutorConfig, projectRoot, sessionRoot, synthesizeDispatch);
    ...
    const degradedLensIds = executionFailures
        ...
        .map((failure) => failure.unit_id);
    const executionCompletedAtMs = Date.now();
    const deliberationStatus = await readStructuredDeliberationStatus(executionPlan.synthesis_output_path, executionPlan.deliberation_output_path, executionPlan.execution_realization);
    await writeExecutionResultArtifact(executionPlan, {
        session_id: executionPlan.session_id,
        session_root: sessionRoot,
        execution_realization: executionPlan.execution_realization,
...
export async function runReviewPromptExecution(argv) {
    ...
    const maxConcurrentLenses = typeof values["max-concurrent-lenses"] === "string" &&

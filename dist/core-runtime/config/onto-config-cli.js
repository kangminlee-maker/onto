...
export async function handleConfigCli(ontoHome, argv) {
    ...
    switch (sub) {
        ...
        case "-h":
            ...
            return 0;
        default:
            console.error(`error: unknown subcommand "${sub}". Run \`onto config --help\` for usage.`);
            return 1;
    }
}
function printHelp() {
...
async function handleShow(ontoHome, projectRoot) {
    try {
        ...
        console.log("");
        console.log(renderPreview(previewTopologyDerivation(validation.config, {
...
function renderShow(review, detection, projectRoot) {
    const lines = [];
    ...
    lines.push(`  max_concurrent_lenses:   ${review.max_concurrent_lenses ?? "(default)"}`);
    lines.push(`  lens_deliberation:       ${review.lens_deliberation ?? "synthesizer-only (default)"}`);
    lines.push("");
    ...
    lines.push(`  litellm_endpoint:        ${detection.litellm_endpoint ?? "(not configured)"}`);
    return lines.join("\n");
}
function renderTeamlead(teamlead) {
    if (!teamlead)
        return "main (default)";
...
// set
// ---------------------------------------------------------------------------
async function handleSet(projectRoot, argv) {
    ...
    if (argv.length !== 2) {
        console.error("error: `onto config set` requires exactly <key> and <value>. " +
...
// edit (interactive)
// ---------------------------------------------------------------------------
async function handleEdit(projectRoot) {
    if (!process.stdin.isTTY) {
        console.error("error: `onto config edit` requires an interactive TTY. " +
...
// re-detect
// ---------------------------------------------------------------------------
async function handleRedetect(ontoHome, projectRoot) {
    const merged = await resolveOrthogonalConfigChain(ontoHome, projectRoot);
    const review = merged.review ?? {};
    const detection = detectReviewAxes().detected;
    ...
    console.log(`  litellm_endpoint:        ${detection.litellm_endpoint ?? "(not configured)"}`);
    const validation = validateReviewConfig(review);
    if (!validation.ok) {
        console.log("");
    ...
    console.log("");
    console.log("Note: re-detect is read-only. Use `onto config edit` or `onto config set` to apply changes.");
    return 0;
}
// ---------------------------------------------------------------------------
// validate (write-free)
 ...
 */
async function readProjectReviewBlock(configPath) {
    try {
        ...
        process.stderr.write("[onto:config] warning: existing review block in config is invalid — starting from empty state\n");
        for (const err of validation.errors) {
...
function writeAndReport(configPath, next, changes) {
    ...
    if (result.replacedExistingBlock)
        console.log("  replaced_existing_block: true");
    return 0;
}

 ...
 */
export function isOntoRoot(dir) {
    try {
        const pkgPath = path.join(dir, "package.json");
        if (!fs.existsSync(pkgPath))
            return false;
        const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
        if (pkg.name !== "onto-core")
            return false;
        if (!fs.existsSync(path.join(dir, ".onto", "roles")))
            return false;
        if (!fs.existsSync(path.join(dir, ".onto", "authority")))
            return false;
        return true;
    }
    catch {
 ...
 */
function isPreMigrationOntoRoot(dir) {
    try {
        const pkgPath = path.join(dir, "package.json");
        if (!fs.existsSync(pkgPath))
            return false;
...
function buildInvalidHomeError(label, resolved) {
    const base = `Invalid ${label}: ${resolved}. ` +
 ...
 */
export function resolveOntoHome(ontoHomeFlag, env = process.env) {
    ...
    if (fromCwd)
        return fromCwd;
    throw new Error("Cannot locate onto installation. Set ONTO_HOME environment variable, pass --onto-home, or run from the onto repository.");
}

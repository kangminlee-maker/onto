 ...
 * 않도록 한다.
 */
function makeHasOntoConfig(homeDir) {
    const home = canonPath(homeDir);
    return (dir) => {
        if (canonPath(dir) === home)
            return false;
 ...
 * 로 취급하지 않는다.
 */
function makeHasGitOrPackageJson(homeDir) {
    const home = canonPath(homeDir);
    return (dir) => {
        if (canonPath(dir) === home)
            return false;
 ...
 */
export function resolveProjectRoot(targetPath, homeDir = os.homedir()) {

...
import { executeReconstructStart, executeReconstructExplore, executeReconstructComplete, executeReconstructConfirm, } from "./reconstruct.js";
let tmpRoot;
beforeEach(() => {
...
function readStateFile(sessionRoot) {
...
describe("executeReconstructStart", () => {
    ...
    it("중복 session id 는 에러", () => {
        ...
        expect(() => executeReconstructStart({
            source: "./src",
            intent: "second",
            sessionsDir: tmpRoot,
            sessionId: "dup",
...
describe("executeReconstructExplore", () => {
    ...
    it("존재하지 않는 session id 는 에러", () => {
        expect(() => executeReconstructExplore({ sessionsDir: tmpRoot, sessionId: "nope" })).toThrow(/session not found/);
    });
    it("converted 이후 explore 는 에러", () => {
        ...
        expect(() => executeReconstructExplore({ sessionsDir: tmpRoot, sessionId: "e3" })).toThrow(/already converted/);
    });
});
describe("executeReconstructComplete", () => {
    function startAndExplore(id) {
        executeReconstructStart({
            source: "./src",
            intent: "t",
            sessionsDir: tmpRoot,
    ...
    it("explore 없이 complete 는 에러 (gathering_context 에서 직접 complete 금지)", () => {
        ...
        expect(() => executeReconstructComplete({ sessionsDir: tmpRoot, sessionId: "c4" })).toThrow(/requires at least one explore/);
    });
    it("이미 converted 된 session 재 complete 는 에러", () => {
        startAndExplore("c5");
        executeReconstructComplete({ sessionsDir: tmpRoot, sessionId: "c5" });
        expect(() => executeReconstructComplete({ sessionsDir: tmpRoot, sessionId: "c5" })).toThrow(/already converted/);
    });
    it("존재하지 않는 session id 는 에러", () => {
        expect(() => executeReconstructComplete({ sessionsDir: tmpRoot, sessionId: "nope" })).toThrow(/session not found/);
    });
});
describe("confirm (W-B-07: principal confirmation seat)", () => {
    function startExploreComplete(id) {
        executeReconstructStart({
            source: "./src",
            intent: "t",
            sessionsDir: tmpRoot,
    ...
    it("exploring 상태에서 confirm 은 에러", () => {
        ...
        expect(() => executeReconstructConfirm({
            sessionsDir: tmpRoot,
            sessionId: "cf3",
            verdict: "passed",

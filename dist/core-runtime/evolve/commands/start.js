...
import { createScope, generateScopeId, normalizeProjectName, resolveScopePaths, } from "../../scope-runtime/scope-manager.js";
...
import { parseStartInput, loadProjectConfig, resolveSources, writeSourcesYaml, } from "../config/project-config.js";
...
import { sourceKey, toGroundingSource, isScanError, isScanSkipped } from "../../readers/types.js";
 ...
 */
export function findExistingScope(scopesDir, projectName) {
...
// ─── Main ───
export async function executeStart(input) {
    ...
    if (input.scopeId && input.title) {
        return executePathBDirect(input, progress);
    }
...
// ─── Path A: New scope ───
async function handleNewScope(input, projectName, scopeId, _paths, progress) {
    ...
    const defaultSources = config.default_sources.map((s) => toBriefSourceEntry(s));
...
async function handleBriefExecution(input, scopeId, paths, progress) {
...
async function executePathBDirect(input, progress) {
    // Step 1: Parse input
    const parsed = parseStartInput(input.rawInput);
...
// ─── Path C: Resume ───
async function handleResume(paths, scopeId, existingEvents, progress) {
    ...
    if (TERMINAL_STATES.has(currentState)) {
        ...
        const hasGroundingCompleted = existingEvents.some((e) => e.type === "grounding.completed");
        ...
        const undecidedCount = state.constraint_pool.constraints.filter(c => c.status === "undecided" || c.status === "clarify_pending").length;
...
async function executeGrounding(paths, scopeId, sources, progress, projectRoot, briefValidation) {
    // Write sources.yaml
    ...
    try {
        const ontologySource = scanResults.find((r) => r.source.type === "add-dir" && r.source.path?.includes("ontology")) ?? scanResults.find((r) => r.source.type === "github-tarball" && r.source.url?.includes("ontology"));
        ...
        if (ontologySource?.source.type === "add-dir") {
            ...
            const url = ontologySource.source.url;
            const repoMatch = url.match(/github\.com\/([^/]+\/[^/]+)/);
            if (repoMatch) {
                const repo = repoMatch[1].replace(/\.git$/, "");
                const token = resolveGitHubToken();
                const headers = { Accept: "application/vnd.github.raw", "User-Agent": "sprint-kit" };
                if (token)
...
function readEtagCache(projectRoot) {
...
function writeEtagCache(projectRoot, cache) {
    try {
...
// ─── Source dispatch ───
async function scanSource(source, etagCache) {
    switch (source.type) {
        case "add-dir":
        ...
        default: {
            ...
            return { source: source, error_type: "parse", message: `Unknown source type` };
        }
    }
}
// ─── Process mode: methodology adapter path ───
import { createHash } from "node:crypto";
async function handleProcessStart(input, progress) {
    const { checkAuthorityConsistency } = await import("../adapters/methodology/perspectives/authority-consistency.js");
    const { validateProcessScopeConfig } = await import("../adapters/methodology/scope-types/process.js");
    ...
    const sourcesYamlContent = authoritySources.map(s => `- path: ${join(input.projectRoot, s.path)}\n  rank: ${s.rank}\n  description: "${s.description}"`).join("\n");
 ...
 */
function discoverAuthoritySources(projectRoot) {
    const sources = [];
 ...
 */
function parseAuthorityTable(content, projectRoot) {
    const sources = [];
        ...
        if (inTable && line.startsWith("|")) {
            ...
            if (cells.length >= 3) {
                ...
                if (!isNaN(rank) && rank >= 1) {
                    ...
                    if (pathStr.endsWith("/") || pathStr.includes("*") || pathStr.includes(","))
                        continue;
 ...
 */
function loadAuthoritySections(sources, projectRoot) {
    const sections = [];
    const loadErrors = [];
 ...
 */
function resolveTargetDocument(rawInput, projectRoot) {
 ...
 */
function parseDocumentSections(docPath, content) {
    const sections = [];
 ...
 */
function extractRulesFromContent(relativePath, projectRoot) {
    try {

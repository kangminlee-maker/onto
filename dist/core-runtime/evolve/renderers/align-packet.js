 ...
 */
export function renderAlignPacket(state, content) {
...
function validate(state, content) {
    ...
    if (missingIds.length > 0) {
        throw new Error(`renderAlignPacket: constraint_id not found in pool: ${missingIds.join(", ")}. constraint.discovered 이벤트로 해당 ID를 pool에 먼저 등록하세요.`);
    }
    // Check interface_extras for interface scope
    if (state.entry_mode === "interface" && !content.interface_extras) {
        throw new Error("renderAlignPacket: entry_mode is 'interface' but interface_extras is missing. AlignPacketContent에 interface_extras(api_scope, breaking_change, version_policy)를 포함하세요.");
    }
}
// ─── Section 1: To-be ───
function renderToBe(lines, state, content) {
...
// ─── Section 2: As-is ───
function renderAsIs(lines, content) {
...
// ─── Section 3: Tension ───
function renderTension(lines, state, content) {
    ...
    lines.push(`요청한 것(to-be)과 현재 현실(as-is) 사이에 ${content.tensions.length}건의 충돌이 발견되었습니다.`);
    lines.push("");
    // Summary table
...
// ─── Section 4: Decision ───
function renderDecision(lines, content) {
...
// ─── Section 2.5: Unverified Assumptions ───
function renderUnverifiedAssumptions(lines, state) {
    const active = state.constraint_pool.constraints.filter((c) => c.status !== "invalidated");
    const unverified = active.filter((c) => isEvidenceUnverified(c.evidence_status));
    // §2.6 — Exploration Phase 5에서 발견된 가정 (§2.5와 독립적으로 판단)
    const explorationAssumptions = state.exploration_progress?.assumptions?.filter((a) => a.status === "unverified") ?? [];
    // §2.5와 §2.6 모두 비어있으면 섹션 전체 생략
    if (unverified.length === 0 && explorationAssumptions.length === 0)
        return;
    ...
    if (unverified.length > 0) {
        ...
        if (ratio >= 0.8) {
            lines.push(`> 현재 발견된 ${active.length}건의 제약 조건 중 **${unverified.length}건이 정책 문서에서 확인되지 않았습니다.** Approve 전에 정책 문서와 대조를 권장합니다.`);
            lines.push("");
        }
        else {
            lines.push("아래 항목은 정책 문서에서 확인되지 않은 가정입니다. Approve 전에 확인을 권장합니다.");
            lines.push("");
            ...
            for (const c of unverified) {
                ...
                lines.push(`| ${c.constraint_id} | ${statusLabel} | ${c.summary} | ${note} |`);
            }
            lines.push("");
        }
    }
    ...
    if (explorationAssumptions.length > 0) {
        ...
        lines.push("");
        lines.push("아래 항목은 Exploration 대화에서 발견된 PO 결정의 전제입니다. 아직 검증되지 않았습니다.");
        lines.push("");
        lines.push("| # | 가정 내용 | 발견 Phase | 상태 |");
        lines.push("|---|----------|-----------|------|");
        explorationAssumptions.forEach((a, i) => {
            lines.push(`| ${i + 1} | ${a.content} | Phase ${a.source_phase ?? "?"} | 미검증 |`);
        });
        lines.push("");
    }
    lines.push("---");
...
function generateFallbackNote(c) {
    if (c.source_refs.length === 0)
        return "정책 문서에서 이 규칙의 근거를 확인해 주세요";

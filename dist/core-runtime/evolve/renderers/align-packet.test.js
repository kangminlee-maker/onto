    ...
    it("renders Align Packet matching golden structure", () => {
        ...
        const content = {
            ...
            tensions: [
                ...
                { constraint_id: "CST-002", what: "이용약관 충돌", why_conflict: "차단이 공정 매칭과 양립 불가", scale: "약관 검토 필요", options: [{ choice: "예외 조항 추가", pros: "빠른 적용", risk: "약관 일관성" }, { choice: "사유 수집으로 제한", pros: "데이터 수집", risk: "UX 복잡도" }, { choice: "현재 약관 유지", pros: "변경 없음", risk: "법적 분쟁" }, { choice: "법무 확인", pros: "법적 안정성", risk: "일정 지연" }] },
...
describe("align-packet — Section 1 To-be", () => {
    ...
    it("renders interface extras when entry_mode is interface", () => {
        const md = renderAlignPacket(makeState({ entry_mode: "interface" }), makeContent({
...
describe("align-packet — empty collections edge cases", () => {
    ...
    it("interface_extras is ignored when entry_mode is experience", () => {
        const md = renderAlignPacket(makeState({ entry_mode: "experience" }), makeContent({
            interface_extras: { api_scope: "public", breaking_change: "forbidden", version_policy: "extend" },
        }));

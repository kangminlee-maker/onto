...
import { findConstraint } from "../../scope-runtime/constraint-pool.js";
 ...
 */
export function renderDraftPacket(state, content) {
...
// ─── Validation ───
function validate(state, content) {
    ...
    if (missingIds.length > 0) {
        throw new Error(`renderDraftPacket: constraint_id not found in pool: ${missingIds.join(", ")}. constraint.discovered 이벤트로 해당 ID를 pool에 먼저 등록하세요.`);
    }
    for (const d of content.constraint_details) {
        ...
        if (entry.decision_owner !== d.decision_owner) {
            ...
            throw new Error(`renderDraftPacket: decision_owner mismatch for ${d.constraint_id} — pool: "${entry.decision_owner}", content: "${d.decision_owner}". ${fix}`);
        }
        // W-B-09: constraint source 일치 가드 — pool에 source_refs가 없는 constraint reject
        if (!entry.source_refs || entry.source_refs.length === 0) {
            throw new Error(`renderDraftPacket: constraint ${d.constraint_id} has no source_refs in pool. constraint.discovered 이벤트에 source_refs를 포함하세요.`);
        }
        // invalidated constraint 는 Section 4(자동 생성)에서만 노출되어야 함. Section 3 detail로 들어오면 중복/모순 발생.
        if (entry.status === "invalidated") {
            throw new Error(`renderDraftPacket: constraint ${d.constraint_id} is invalidated — must not appear in constraint_details. Invalidated constraints are auto-rendered in Section 4.`);
        }
    }
}
// ─── Section 1: 확정된 Surface ───
function renderSurface(lines, state, content) {
    ...
    if (content.mockup_revisions !== undefined) {
        lines.push(`- **mockup 반복**: ${content.mockup_revisions}회 수정 후 주체자 확정`);
    }
    lines.push("");
...
function renderStatus(lines, state, content) {
    lines.push("### 2. 현재까지의 결정 현황");
    lines.push("");
    // Align decisions
    const alignItems = [];
    if (state.direction)
        alignItems.push("방향 승인");
    if (state.scope_boundaries)
        alignItems.push("범위 확정");
    ...
    for (const d of content.constraint_details) {
        ...
        if (entry.status === "decided")
            decided++;
        else if (entry.status === "clarify_pending")
            clarifyPending++;
        else
            undecided++;
    }
    lines.push(`- Draft에서 결정 필요: 아래 ${active}건 (${decided}건 결정 완료, ${undecided + clarifyPending}건 미결정)`);
    ...
    if (poolClarify > 0) {
        lines.push(`- 잠금 전 필수 해소: \`clarify\` 상태 항목이 ${poolClarify}건 있어 잠글 수 없음`);
    }
    else {
...
function renderConstraints(lines, state, content) {
    ...
    for (const d of sorted) {
        ...
        lines.push(`| ${d.constraint_id} | ${formatPerspective(entry.perspective)} | ${entry.summary} | ${severityLabel} |`);
    }
    lines.push("");
    lines.push("---");
    lines.push("");
...
function renderConstraintDetail(lines, detail, entry) {
    const severityTag = entry.severity === "required" ? "필수" : "권장";
    const policyTag = isPolicyChangeRequired(entry) ? " [정책 변경 검토 필요]" : "";
    const ownerTag = detail.decision_owner === "builder" ? " (Builder 결정 항목)" : "";
    lines.push(`#### ${detail.constraint_id} | ${formatPerspective(entry.perspective)} | ${entry.summary} — ${severityTag}${policyTag}${ownerTag}`);
    lines.push("");
    ...
    if (detail.decision_owner === "product_owner") {
        ...
        for (const opt of detail.options_table) {
            lines.push(`| ${opt.choice} | ${opt.pros} | ${opt.description} | ${opt.risk} | ${opt.reversal_cost} |`);
        }
        lines.push("");
        ...
        lines.push("");
        lines.push(`**이 작업 관점에서의 판단:** ${detail.builder_judgment}`);
        lines.push("");
        if (detail.guardrail) {
...
function renderInvalidated(lines, state) {
    ...
    const requiredInvalidated = invalidated.filter((c) => c.severity === "required");
    if (requiredInvalidated.length > 0) {
        for (const c of requiredInvalidated) {
            lines.push(`> **${c.constraint_id}**: 이 항목은 기능 불능 또는 규정 위반으로 분류되었습니다. 제외에 동의하시나요?`);
        }
        lines.push("");
    }
    lines.push("동의하지 않으면 해당 항목을 결정 목록으로 복원할 수 있습니다.");
    lines.push("");
    lines.push("---");
    lines.push("");
}
// ─── Section 5: 제약 조건 ───
function renderGuardrails(lines, content) {
...
// ─── Section 6: 지금 결정할 것 ───
function renderDecision(lines, state, content) {
    ...
    if (content.constraint_details.length === 0) {
        ...
        if (state.constraint_pool.summary.clarify_pending > 0) {
            lines.push(`> \`clarify\`를 선택한 항목은 해소될 때까지 잠글 수 없습니다 (현재 ${state.constraint_pool.summary.clarify_pending}건)`);
            lines.push("");
        }
        ...
        lines.push("- **Review**: 이 Packet 전체에 대해 6-Agent Panel Review를 요청합니다.");
        lines.push("");
        lines.push("선택: **Approve** + 각 CST 결정 / **Revise** / **Reject** / **Redirect to Align** / **Review**");
    }
    lines.push("");
}
// ─── Helpers ───
function sortConstraintDetails(details, state) {
...
function formatSeverity(entry, detail) {

...
function isGovernanceCoreTarget(filePath) {
    if (GOVERNANCE_CORE_FILES.includes(filePath))
        return true;
    return GOVERNANCE_CORE_DIR_PREFIXES.some((prefix) => startsWithDirPrefix(filePath, prefix));
}
/**
...
export function classifyProposal(proposal) {
    ...
    if (hasGovernanceCore) {
        return {
            ...
            reason: "target 에 governance core (.onto/authority/, .onto/principles/, .onto/processes/govern.md) 가 포함됨. §1.3 Principal 직접 분기.",
        };
    }
    if (proposal.change_kind === "docs_only" && proposal.target_files.length === 1) {
        return {
            ...
            reason: "change_kind=docs_only + target 단일 파일. §1.3 drift 양의 피드백 없는 local contained 변경.",
        };
    }
    return {
        route: "queue",
        matched_rule: "drift_default",
        reason: "docs-only 단일 파일도 아니고 governance core 도 아님. §1.3 drift 증폭 가능성 default — 큐 적재.",
    };
}
/**
 * 분류 결과에 따라 route 실행.
 ...
 */
export function routeProposal(proposal, projectRoot, now = new Date()) {

...
// Aggregation
// ---------------------------------------------------------------------------
export function buildHealthReport(items, target) {
    ...
    for (const item of items) {
        ...
        if (hasMethodology && hasDomain)
            ...
            axis.methodology_only += 1;
        else if (hasDomain)
            axis.domain_only += 1;
        if (item.role === null)
            purpose.unassigned += 1;
        else
            purpose[item.role] += 1;
        type[item.type] += 1;
        event_markers += item.event_markers.length;
        if (item.retention_confirmed_at !== null)
            retention_confirmed += 1;
        if (item.raw_line.includes("tag-incomplete"))
            tag_incomplete += 1;
        if (item.raw_line.includes("consolidated into"))
            consolidated += 1;
        const prior = lineMax.get(item.source_path) ?? 0;
        if (item.line_number > prior)
            lineMax.set(item.source_path, item.line_number);
...
function extractAgentFromPath(sourcePath) {
...
export function renderHealthReport(report) {
    ...
    if (report.largest_file) {
        lines.push(`| Largest file | ${report.largest_file.agent} (${report.largest_file.lines} lines) |`);
    }
    else {
    ...
    lines.push("|-----|-------|---|");
    lines.push(`| methodology-only | ${report.axis_distribution.methodology_only} | ${pct(report.axis_distribution.methodology_only, total)}% |`);
    lines.push(`| domain-only | ${report.axis_distribution.domain_only} | ${pct(report.axis_distribution.domain_only, total)}% |`);
    lines.push(`| dual-tag | ${report.axis_distribution.dual} | ${pct(report.axis_distribution.dual, total)}% |`);
    lines.push("");
    ...
    lines.push("| Tag | Count | % |");
    lines.push("|-----|-------|---|");
    lines.push(`| fact | ${report.type_distribution.fact} | ${pct(report.type_distribution.fact, total)}% |`);
    lines.push(`| judgment | ${report.type_distribution.judgment} | ${pct(report.type_distribution.judgment, total)}% |`);
    lines.push("");
    ...
    lines.push(`| File size | ${sizeStatus} | 최대 ${largestLines}행 (100 NOTICE, 200 WARNING). 대응: /onto:promote 로 consolidation·retirement 검토 |`);
    lines.push(`| Event markers | ${report.event_markers} | applied-then-found-invalid 누적. /onto:promote 실행 시 퇴역 후보로 표면화 |`);
    lines.push(`| Tag-incomplete | ${report.tag_incomplete} | Creation gate 실패 수. 해당 학습 항목의 태그 수동 보완 필요 |`);
    lines.push(`| Consolidated | ${report.consolidated} | Cross-agent dedup 수행 수. 조치 불필요 (정보만) |`);
    lines.push(`| Retention-confirmed | ${report.retention_confirmed} | 명시적 유지 판정 수 |`);
    lines.push("");

 ...
 */
export function deriveSlotId(approvedPromotionId, targetDoc, domain) {
    return crypto
        .createHash("sha256")
 ...
 */
export function identifyDomainDocCandidates(verdicts, config = {}) {
    const generator = config.instanceIdGenerator ?? generateUlid;
    const candidates = [];
    for (const verdict of verdicts) {
        if (verdict.consensus !== "promote_3_3" &&

...
import { summarizeDegradedStates, severityOf, } from "./degraded-state.js";
import { generateUlid, initApplyState, markApplied, markFailed, transitionStatus, } from "./apply-state.js";
import { composePanel, candidateIdOf, validatePanelMemberReview, aggregateConsensus, } from "./panel-reviewer.js";
import { determineAuditEligibility, DEFAULT_AUDIT_POLICY, } from "./judgment-auditor.js";
...
import { saveRecoveryResolution, loadRecoveryResolution, resolveRecoveryTruth, gatherRecoveryContext, buildEscalationMessage, DEFAULT_RECOVERY_POLICY, } from "../shared/recovery-context.js";
import { REGISTRY, RegistryInitError, UnregisteredArtifactKindError, IncompatibleVersionError, InvalidArtifactError, } from "../shared/artifact-registry.js";
import { inspectMigrationStatus, ensureSessionRootsMigrated, MigrationRequiredError, } from "../../cli/session-root-guard.js";
...
function writeLearningFile(dir, agentId, contents) {
...
function syntheticReview(verdict) {
    return {
...
describe("Phase 3 Promote E2E (focused)", () => {
    ...
    it("E-P1 collector promote mode filters out [insight] from project", () => {
        ...
        writeLearningFile(projDir, "structure", [
            "<!-- format_version: 1 -->",
    ...
    it("E-P2 collector reclassify-insights mode targets only [insight]", () => {
        ...
        writeLearningFile(globalDir, "structure", [
            "<!-- format_version: 1 -->",
            "- [fact] [methodology] [guardrail] g (source: p, d, 2026-01-01) [impact:normal]",
            "- [fact] [methodology] [insight] i (source: p, d, 2026-01-01) [impact:normal]",
        ].join("\n"));
    ...
    it("E-P3 verifyBaselineHash detects unchanged files", () => {
        ...
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "<!-- format_version: 1 -->\n- [fact] [methodology] [foundation] x (source: p, d, 2026-01-01) [impact:normal]\n");
        const result = collect({ mode: "promote", projectRoot });
    ...
    it("E-P4 verifyBaselineHash detects content_sha256 drift", () => {
        ...
        const filePath = writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "<!-- format_version: 1 -->\n- [fact] [methodology] [foundation] x (source: p, d, 2026-01-01) [impact:normal]\n");
        const result = collect({ mode: "promote", projectRoot });
    ...
    it("E-P14 panel validator rejects all-yes + non-promote verdict", () => {
        ...
        assert(result.failures.some((f) => f.includes("verdict is not promote")), "right reason (mentions verdict is not promote)");
    });
    ...
    it("E-P17 audit eligibility: count_threshold ≥10", () => {
        ...
        const auditState = { schema_version: "1", obligations: [] };
    ...
    it("E-P20 health snapshot axis percentages sum within tolerance", () => {
        ...
        const total = snap.axis_distribution.methodology_only_pct +
    ...
    it("E-P25 runPromoter assembles report without LLM calls", async () => {
        ...
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", [
            "<!-- format_version: 1 -->",
            "- [fact] [methodology] [foundation] x (source: p, d, 2026-01-01) [impact:normal]",
        ].join("\n"));
    ...
    it("E-P26 runPromoteExecutor returns stale_baseline when source drifts", async () => {
        ...
        const learnFile = writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", [
            "<!-- format_version: 1 -->",
            "- [fact] [methodology] [foundation] x (source: p, d, 2026-01-01) [impact:normal]",
        ].join("\n"));
        ...
        fs.writeFileSync(path.join(promoter.sessionRoot, "promote-decisions.json"), JSON.stringify(decisions, null, 2));
    ...
    it("E-P28 runPromoteExecutor applies a single approved promotion", async () => {
        ...
        const projectLine = "- [fact] [methodology] [foundation] e2e body (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", ["<!-- format_version: 1 -->", projectLine].join("\n"));
    ...
    it("E-P30 runPromoter assembles report with panel + audit via mock LLM", async () => {
        ...
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", [
            "<!-- format_version: 1 -->",
            ...Array.from({ length: 11 }, (_, i) => `- [judgment] [methodology] [foundation] philo-${i} (source: p, d, 2026-01-0${(i % 9) + 1}) [impact:normal]`),
        ].join("\n"));
        // Global: structure file (so panel composition has known agents)
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "structure", ["<!-- format_version: 1 -->"].join("\n"));
        const previousHome = process.env.HOME;
        ...
        try {
            ...
            assert(consensus === "promote_3_3" || consensus === "promote_2_3", `expected unanimous promote consensus, got ${consensus}`);
            assert(result.report.audit_summary.execution.audited_agents.includes("axiology"), "axiology audited");
            assertEqual(result.report.audit_summary.outcomes.retain, 11, "all 11 retained by mock");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
    ...
    it("E-P31 audit blocks gracefully on provider error", async () => {
        ...
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "structure", [
            "<!-- format_version: 1 -->",
            ...Array.from({ length: 11 }, (_, i) => `- [judgment] [methodology] [foundation] judge-${i} (source: p, d, 2026-01-01) [impact:normal]`),
        ].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            assert(result.report.audit_summary.eligibility.length > 0, "audit was eligible");
            assertEqual(result.report.audit_summary.execution.audited_items_count, 0, "no items audited (provider failed)");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
    ...
    it("E-P32 insight reclassifier classifies via mock LLM", async () => {
        ...
        try {
            ...
            assertEqual(result.reclassified[0].proposed_role, "foundation", "mock defaults to foundation");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
    ...
    it("E-P33 promote-executor applies domain doc update via mock LLM", async () => {
        ...
        const projectLine = "- [fact] [methodology] [domain/finance] [foundation] mock semantics term (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "semantics", ["<!-- format_version: 1 -->", projectLine].join("\n"));
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "semantics", ["<!-- format_version: 1 -->"].join("\n"));
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", ["<!-- format_version: 1 -->"].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            const promoter = await runPromoter({
            ...
            assertEqual(promoter.report.panel_verdicts.length, 1, "1 panel verdict");
            assertEqual(promoter.report.domain_doc_candidates.length, 1, "1 domain doc candidate");
            ...
            if (outcome.kind !== "completed")
                return;
            assertEqual(outcome.summary.domain_doc_updates_applied, 1, "1 domain doc update applied");
            // Verify the domain doc was actually written
            const docPath = path.join(fakeOnto, "domains", "finance", "concepts.md");
            assert(fs.existsSync(docPath), "concepts.md created");
            const docContent = fs.readFileSync(docPath, "utf8");
            assert(docContent.includes(`<!-- slot_id: ${candidate.slot_id} -->`), "slot_id marker present");
    ...
    it("E-P34 domain doc applicator skips already-written slot", async () => {
        ...
        const projectLine = "- [fact] [methodology] [domain/finance] [foundation] e2e34 (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "semantics", ["<!-- format_version: 1 -->", projectLine].join("\n"));
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", ["<!-- format_version: 1 -->"].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            fs.rmSync(path.join(promoter.sessionRoot, "promote-execution-result.json"), { force: true });
    ...
    it("E-P35 cross_agent_dedup applicator consolidates and marks members", async () => {
        ...
        const lineA = "- [fact] [methodology] [foundation] same principle a (source: p, d, 2026-01-01) [impact:normal]";
        const lineB = "- [fact] [methodology] [foundation] same principle b (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", lineA].join("\n"));
        writeLearningFile(path.join(fakeOnto, "learnings"), "logic", ["<!-- format_version: 1 -->", lineB].join("\n"));
        ...
        fs.symlinkSync(path.join(fakeOnto, "learnings"), path.join(fakeHome, ".onto", "learnings"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeHome;
        try {
            const promoter = await runPromoter({
            ...
            const consolidatedLine = "- [fact] [methodology] [foundation] consolidated principle (source: cluster-1, d, 2026-04-08) [impact:normal]";
            ...
            if (outcome.kind !== "completed")
                return;
            assertEqual(outcome.summary.cross_agent_dedup_applied, 1, "1 cluster applied");
            ...
            assert(structureContent.includes("consolidated principle"), "structure has consolidated line");
            assert(logicContent.includes("consolidated") && logicContent.includes("cluster-test-35"), "logic line marked as consolidated");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
    ...
    it("E-P36 saveRecoveryResolution appends to resolution_history", () => {
        ...
        fs.mkdirSync(path.join(projectRoot, ".onto", "sessions", "promote", sessionId), { recursive: true });
        // First decision
        const entry1 = {
        ...
        assertEqual(loaded.selected_attempt_id, "01ATTEMPT_B", "top-level reflects latest");
        assertEqual(loaded.resolution_history[0].selected_attempt_id, "01ATTEMPT_A", "first entry preserved");
        assertEqual(loaded.resolution_history[1].selected_attempt_id, "01ATTEMPT_B", "second entry latest");
    });
    ...
    it("E-P38 collector parse errors surface as warnings without crash", async () => {
        ...
        assert(result.report.collection.parse_errors.length > 0, "parse errors collected");
        assert(result.report.warnings.some((w) => w.includes("parse")), "parse warning surfaced");
        assertEqual(result.report.collection.candidate_items.length, 1, "valid line still collected");
    });
    ...
    it("E-P39a judgment-auditor chunks ≥13 items into multiple batches", async () => {
        ...
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", [
            "<!-- format_version: 1 -->",
            ...Array.from({ length: 25 }, (_, i) => `- [judgment] [methodology] [foundation] big-batch-${i} (source: p, d, 2026-01-0${(i % 9) + 1}) [impact:normal]`),
        ].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            assertEqual(result.report.audit_summary.execution.audited_items_count, 25, "all 25 items audited (chunked)");
            // 25 / 12 = 3 batches → 3 LLM calls
            assertEqual(result.report.audit_summary.execution.llm_calls, 3, "3 LLM calls (chunked by AUDIT_BATCH_SIZE=12)");
            assertEqual(result.report.audit_summary.failed_agents.length, 0, "no failures");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
    ...
    it("E-P39b audit failures appear in summary.failed_agents", async () => {
        ...
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", [
            "<!-- format_version: 1 -->",
            ...Array.from({ length: 11 }, (_, i) => `- [judgment] [methodology] [foundation] fail-${i} (source: p, d, 2026-01-01) [impact:normal]`),
        ].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            assertEqual(result.report.audit_summary.failed_agents.length, 1, "axiology failure recorded in failed_agents");
            assertEqual(result.report.audit_summary.failed_agents[0].agent_id, "axiology", "failed agent id");
            assertEqual(result.report.audit_summary.failed_agents[0].judgment_count_total, 11, "judgment_count_total surfaced for operator visibility");
            assert(result.report.audit_summary.failed_agents[0].failed_chunks_count >= 1, "failed_chunks_count >= 1 for blocked agent");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
            ...
            if (previousMock === undefined)
                ...
                process.env.ONTO_LLM_MOCK = previousMock;
        }
    });
    ...
    it("E-P39 cross_agent_dedup_approvals enumerated as pending", async () => {
        ...
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", ["<!-- format_version: 1 -->"].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-39",
            projectRoot,
        ...
        assertEqual(outcome.kind, "failed_resumable", "outcome is failed_resumable for missing cluster");
        if (outcome.kind !== "failed_resumable")
            return;
    ...
    it("E-P40 resume skips already-applied decisions (no double-write)", async () => {
        ...
        const projectLine = "- [fact] [methodology] [foundation] resume-target (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", ["<!-- format_version: 1 -->", projectLine].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-40",
        ...
        assertEqual(sizeAfterFirst, sizeAfterSecond, "global file unchanged on resume (no double-write)");
    });
    ...
    it("E-P41 cross_agent_dedup preflight fails on missing member file", async () => {
        ...
        const lineA = "- [fact] [methodology] [foundation] preflight-a (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", lineA].join("\n"));
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated (source: cluster, d, 2026-04-08) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        assertEqual(sizeBefore, sizeAfter, "primary file untouched on preflight failure");
    });
    ...
    it("E-P42 cross_agent_dedup skips re-application via cluster_id marker", async () => {
        ...
        const lineA = "- [fact] [methodology] [foundation] idem-a (source: p, d, 2026-01-01) [impact:normal]";
        const lineB = "- [fact] [methodology] [foundation] idem-b (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", lineA].join("\n"));
        writeLearningFile(path.join(fakeOnto, "learnings"), "logic", ["<!-- format_version: 1 -->", lineB].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
        ...
        const consolidatedLine = "- [fact] [methodology] [foundation] idem-consolidated (source: cluster, d, 2026-04-08) [impact:normal]";
        ...
        assertEqual(sizeAfterFirst, sizeAfterSecond, "primary file unchanged on idempotent re-apply");
    });
    ...
    it("E-P45 saveRecoveryResolution rejects empty resolution_history", () => {
        ...
        fs.mkdirSync(path.join(projectRoot, ".onto", "sessions", "promote", "test-45"), { recursive: true });
        let threw = false;
        try {
            saveRecoveryResolution(projectRoot, {
                schema_version: "1",
    ...
    it("E-P46 failed_agents.failed_chunks_count exposes failed batch count", async () => {
        ...
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", [
            "<!-- format_version: 1 -->",
            ...Array.from({ length: 25 }, (_, i) => `- [judgment] [methodology] [foundation] m3-${i} (source: p, d, 2026-01-01) [impact:normal]`),
        ].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            const failed = result.report.audit_summary.failed_agents.find((f) => f.agent_id === "axiology");
    ...
    it("E-P47 domain doc applicator rejects invalid reflection_form", async () => {
        ...
        const projectLine = "- [fact] [methodology] [domain/finance] [foundation] e-p47 scope item (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "coverage", ["<!-- format_version: 1 -->", projectLine].join("\n"));
        writeLearningFile(path.join(fakeOnto, ".onto", "learnings"), "axiology", ["<!-- format_version: 1 -->"].join("\n"));
        const previousHome = process.env.HOME;
        process.env.HOME = fakeOnto;
        try {
            ...
            assertEqual(promoter.report.domain_doc_candidates.length, 1, "1 domain doc candidate (coverage → domain_scope.md)");
            const candidate = promoter.report.domain_doc_candidates[0];
            ...
            assertEqual(outcome.kind, "failed_resumable", "outcome failed due to invalid reflection_form");
            if (outcome.kind !== "failed_resumable")
                return;
            assertEqual(outcome.summary.domain_doc_updates_applied, 0, "no domain doc applied");
            assertEqual(outcome.summary.failed_decisions, 1, "1 failed decision");
        }
        finally {
            if (previousHome === undefined)
    ...
    it("E-P48 mock LLM rejects unknown system prompt", async () => {
        ...
        let threw = false;
        try {
            await callLlm("You are an unknown agent doing something the mock doesn't recognize", "test", { max_tokens: 10 });
        }
        catch (error) {
            threw =
    ...
    it("E-P49 llm-caller guides to install codex when OAuth present but binary missing", async () => {
        ...
        try {
            ...
            assert(caught.message.includes("codex 바이너리") ||
                caught.message.includes("codex binary"), `error should mention codex binary missing; got: ${caught.message}`);
            assert(caught.message.includes("https://github.com/openai/codex"), "error should include codex install link");
            assert(caught.message.includes("chatgpt OAuth") ||
                caught.message.includes("OAuth"), "error should acknowledge OAuth credential is present");
        }
        finally {
            if (previousHome === undefined)
                delete process.env.HOME;
                ...
                process.env.HOME = previousHome;
            if (previousAnth !== undefined)
                process.env.ANTHROPIC_API_KEY = previousAnth;
            if (previousOpen !== undefined)
                process.env.OPENAI_API_KEY = previousOpen;
            if (previousMock !== undefined)
                process.env.ONTO_LLM_MOCK = previousMock;
    ...
    it("E-P50 llm-caller accepts auth.json OPENAI_API_KEY field (api-key mode)", async () => {
        ...
        try {
            ...
            const result = await callLlm("You are reviewing promotion candidates for a learning management system as an expert in the role of structure (originator).", "candidate_id=test type=fact tags=[methodology] role=foundation\n   content: x", { max_tokens: 100 });
    ...
    it("E-P52 REGISTRY lazy init on first use populates builtin specs", () => {
        ...
        try {
            ...
            assert(false, `lazy init failed unexpectedly: ${e.message}`);
        }
        assert(initializedBefore, "lazy init produced registered specs on first call");
        assert(REGISTRY.listRegistered().includes("audit_state"), "builtin audit_state spec is registered after lazy init");
    });
    ...
    it("E-P53 Registry init failure is cached: same error on retries (NQ-19)", () => {
        ...
        try {
            ...
            assert(first.message.includes("simulated boot failure"), "RegistryInitError wraps original cause message");
        }
        finally {
    ...
    it("E-P54 saveToFile InvalidArtifactError when validate() fails (SYN-CC1)", () => {
        ...
        assert(caught.message.includes("obligations"), "error names the missing field");
    ...
    it("E-P55 saveToFile rejects undefined schema_version (SYN-CONS-03)", () => {
        ...
        assert(caught.message.toLowerCase().includes("schema_version"), "error mentions schema_version");
    ...
    it("E-P56 saveToFile rejects schema_version mismatch", () => {
        ...
        const bad = { schema_version: "999", obligations: [] };
        let caught = null;
        try {
            REGISTRY.saveToFile("audit_state", target, bad);
        }
        ...
        assert(caught instanceof InvalidArtifactError, "expected InvalidArtifactError");
        assert(caught.message.includes("999") || caught.message.includes("schema_version"), "error references the bad version");
        assert(!fs.existsSync(target), "file not written");
    });
    ...
    it("E-P57 saveToFile rejects unsupported extension", () => {
        ...
        assert(msg.includes(".txt") || msg.toLowerCase().includes("extension") || msg.toLowerCase().includes("format"), `error references the bad extension (got: ${msg})`);
        assert(!fs.existsSync(target), "file not written");
    });
    ...
    it("E-P58 loadFromFile rejects pre-v7 artifact with clear guidance", () => {
        ...
        assert(msg.toLowerCase().includes("discard") || msg.toLowerCase().includes("regenerate"), "message gives actionable guidance");
    });
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    function synthApplyState(sessionId, attemptId, generation, recordedAt) {
        return {
            schema_version: "1",
            session_id: sessionId,
    ...
    function synthApplyStateSource(sessionId, attemptId, generation, recordedAt) {
        return {
            kind: "apply_state",
    ...
    function synthContext(sessionId, apply) {
        return {
            session_id: sessionId,
    ...
    it("E-P59 gatherRecoveryContext reads a valid apply_state file", async () => {
        ...
        const sessionRoot = path.join(projectRoot, ".onto", "sessions", "promote", sessionId);
        fs.mkdirSync(sessionRoot, { recursive: true });
        const state = synthApplyState(sessionId, "01ARAFG00000000000000000AA", 7, "2026-04-08T10:00:00Z");
        REGISTRY.saveToFile("apply_execution_state", path.join(sessionRoot, "promote-execution-result.json"), state);
        const context = await gatherRecoveryContext(sessionId, projectRoot);
        assert(context.apply_state !== null, "apply_state populated");
        assertEqual(context.apply_state.freshness.attempt_id, "01ARAFG00000000000000000AA", "attempt_id carried through");
        assertEqual(context.apply_state.freshness.generation, 7, "generation carried");
        assertEqual(context.apply_state.freshness.source_kind, "apply_state", "source_kind labelled");
    ...
    it("E-P61 gatherRecoveryContext treats corrupt apply_state as absent", async () => {
        ...
        fs.writeFileSync(path.join(sessionRoot, "promote-execution-result.json"), JSON.stringify({ session_id: sessionId, obligations: [] }), "utf8");
        const context = await gatherRecoveryContext(sessionId, projectRoot);
    ...
    it("E-P64 resolveRecoveryTruth 2 attempt_ids default policy → manual escalation", () => {
        const apply = synthApplyStateSource("sess-64", "01AAAAA_attempt_a_00000000", 5, "2026-04-08T12:00:00Z");
        ...
        assert(truth.escalation_reason.includes("resolve-conflict"), "reason mentions cli resolution path");
    });
    ...
    it("E-P65 resolveRecoveryTruth auto_resolve picks lexicographically latest attempt", () => {
        const apply = synthApplyStateSource("sess-65", "01AAAAA_attempt_a_00000000", 5, "2026-04-08T12:00:00Z");
        const ckpt = {
            kind: "checkpoint_manifest",
            manifest: {
                schema_version: "1",
        ...
        assertEqual(truth.latest_source.freshness.attempt_id, "01BBBBB_attempt_b_00000000", "canonical = lex-max attempt_id");
    });
    ...
    it("E-P66 resolveRecoveryTruth honors operator resolution (selected attempt exists)", () => {
        ...
        assertEqual(truth.latest_source.freshness.attempt_id, attemptA, "latest_source matches operator choice");
        assert(truth.resolution_artifact_path !== undefined, "resolution_artifact_path set on operator_resolution");
    });
    ...
    it("E-P67 resolveRecoveryTruth re-escalates when operator attempt is stale", () => {
        ...
        const apply = synthApplyStateSource(sessionId, "01AAAAA_attempt_a_00000000", 5, "2026-04-08T12:00:00Z");
        const ckpt = {
            kind: "checkpoint_manifest",
            manifest: {
                schema_version: "1",
        ...
        if (truth.kind !== "manual_escalation_required")
            return;
        assert(truth.escalation_reason.toLowerCase().includes("no source matches") ||
            truth.escalation_reason.toLowerCase().includes("re-record"), "reason explains why the prior resolution is stale");
    });
    ...
    it("E-P69 saveRecoveryResolution merge updates top-level to latest decision", () => {
        ...
        assertEqual(loaded.selected_attempt_id, "01SECOND", "top-level reflects latest decision");
        assertEqual(loaded.resolved_at, "2026-04-09T01:00:00Z", "top-level resolved_at reflects latest");
        assertEqual(loaded.resolution_history.length, 2, "both decisions preserved in history");
        assertEqual(loaded.resolution_history[0].selected_attempt_id, "01FIRST", "first decision preserved at head");
        assertEqual(loaded.resolution_history[1].selected_attempt_id, "01SECOND", "second decision at tail");
    });
    ...
    it("E-P70 initApplyState fresh attempt_id is unique + markApplied preserves it", () => {
        ...
        assertEqual(aAfter.attempt_id, a.attempt_id, "markApplied preserves attempt_id (DD-22)");
    ...
    it("E-P71 AuditObligation constructor rejects empty status_history", async () => {
        ...
        assert(caught.message.includes("status_history"), "error names status_history");
    });
    ...
    it("E-P72 AuditObligation constructor rejects status ↔ last history mismatch", async () => {
        ...
        assert(caught instanceof InvariantViolatedError, "InvariantViolatedError thrown");
        const msg = caught.message;
        assert(msg.includes("in_progress") && msg.includes("pending"), "error names both declared and actual status");
    });
    ...
    it("E-P73 AuditObligation.fromJSON rejects missing required field", async () => {
        ...
        assert(caught instanceof InvariantViolatedError, "InvariantViolatedError thrown");
        assert(caught.message.includes("status"), "error names the missing field");
        assert(caught.message.includes("ob-73"), "error surfaces obligation_id for triage");
    });
    ...
    function writeFakeBackup(root, bak) {
        ...
        REGISTRY.saveToFile("backup_metadata", path.join(dir, "backup-metadata.yaml"), metadata);
    ...
    it("E-P77 pruneBackups respects active_session protection", async () => {
        ...
        const result = await pruneBackups({ backup_storage_max_bytes: 1_000_000, keep_last_n: 0, keep_for_days: 0 }, fakeRoot);
        ...
        assert(fs.existsSync(path.join(fakeRoot, "active-one", "backup-metadata.yaml")), "active_session backup dir preserved");
        assert(!fs.existsSync(path.join(fakeRoot, "unprotected-one")), "unprotected backup dir removed");
    });
    ...
    it("E-P78 pruneBackups respects failed_unrecoverable protection", async () => {
        ...
        const result = await pruneBackups({ backup_storage_max_bytes: 0, keep_last_n: 0, keep_for_days: 0 }, fakeRoot);
        ...
        assert(fs.existsSync(path.join(fakeRoot, "failed-recovery-needed")), "failed_unrecoverable dir still exists");
    });
    ...
    it("E-P79 pruneBackups enforces keep_last_n and prunes oldest first", async () => {
        ...
        const result = await pruneBackups({ backup_storage_max_bytes: 1_000_000, keep_last_n: 2, keep_for_days: 30 }, fakeRoot);
    ...
    it("E-P80 pruneBackups enforces storage_max_bytes as secondary pressure", async () => {
        ...
        const result = await pruneBackups({ backup_storage_max_bytes: 2500, keep_last_n: 5, keep_for_days: 30 }, fakeRoot);
    ...
    it("E-P81 pruneBackups appends .prune-log.jsonl entry per pruned session", async () => {
        ...
        const result = await pruneBackups({ backup_storage_max_bytes: 1_000_000, keep_last_n: 10, keep_for_days: 30 }, fakeRoot);
    ...
    it("E-P82 setBackupProtection toggle flows through to pruneBackups", async () => {
        const { setBackupProtection, pruneBackups } = await import("../shared/recoverability.js");
        ...
        let result = await pruneBackups({ backup_storage_max_bytes: 0, keep_last_n: 0, keep_for_days: 0 }, fakeRoot);
        assertEqual(result.pruned, 0, "protected not pruned");
        // Lift protection
        setBackupProtection("toggle-target", null, fakeRoot);
        // Second run: now eligible
        result = await pruneBackups({ backup_storage_max_bytes: 0, keep_last_n: 0, keep_for_days: 0 }, fakeRoot);
        assertEqual(result.pruned, 1, "after lift, pruned");
        assert(!fs.existsSync(path.join(fakeRoot, "toggle-target")), "backup dir removed after protection lifted");
    });
    // -------------------------------------------------------------------------
    ...
    it("E-P83 processCarryForward increments count without expiring under max", async () => {
        const { AuditObligation } = await import("./audit-obligation.js");
        const { processCarryForward, countCarriedForward } = await import("../shared/audit-state.js");
        const ob = new AuditObligation({
            obligation_id: "ob-83",
            trigger_kind: "count_threshold",
            detected_at: "2026-04-09T00:00:00Z",
        ...
        const state = { schema_version: "1", obligations: [ob] };
    ...
    it("E-P84 processCarryForward expires on carry-forward exhaustion", async () => {
        const { AuditObligation } = await import("./audit-obligation.js");
        const { processCarryForward, getExpiredUnattended, getActiveObligations } = await import("../shared/audit-state.js");
        const ob = new AuditObligation({
            obligation_id: "ob-84",
            trigger_kind: "count_threshold",
            detected_at: "2026-04-09T00:00:00Z",
        ...
        assertEqual(getActiveObligations(state).length, 0, "expired is not active anymore");
    ...
    it("E-P86 processCarryForward skips terminal obligations", async () => {
        const { AuditObligation } = await import("./audit-obligation.js");
        const { processCarryForward, getActiveObligations } = await import("../shared/audit-state.js");
        function buildTerminal(obligationId, terminalState) {
            const ob = new AuditObligation({
    ...
    it("E-P87 AuditState save/load round-trip preserves DD-21 class semantics", async () => {
        const { AuditObligation } = await import("./audit-obligation.js");
        const { saveAuditState, loadAuditState } = await import("../shared/audit-state.js");
        const ob = new AuditObligation({
            obligation_id: "ob-87",
            trigger_kind: "count_threshold",
            detected_at: "2026-04-09T00:00:00Z",
        ...
        ob.transition("in_progress", "picked up");
        const pre = { schema_version: "1", obligations: [ob] };
    ...
    it("E-P90 audit-state filter helpers partition the ledger correctly", async () => {
        const { AuditObligation } = await import("./audit-obligation.js");
        const { getActiveObligations, getExpiredUnattended, countCarriedForward, findObligation, } = await import("../shared/audit-state.js");
        function fresh(id) {
        ...
        assertEqual(activeSet.map((o) => o.obligation_id).sort().join(","), "ob-90a,ob-90b", "active subset");
        ...
        assertEqual(findObligation(state, "ob-90b")?.obligation_id, "ob-90b", "findObligation locates by id");
        assertEqual(findObligation(state, "nonexistent"), undefined, "findObligation returns undefined for unknown id");
    });
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    function buildVerdict(candidateId, agentId, applicabilityTags, consensus) {
        const item = syntheticItem({
            agent_id: agentId,
    ...
    it("E-P92 DD-19 regeneration: stable slot_id + distinct instance_id", () => {
        ...
        assertEqual(gen1[0].slot_id, gen2[0].slot_id, "slot_id stable first candidate");
        assertEqual(gen1[1].slot_id, gen2[1].slot_id, "slot_id stable second candidate");
        // instance_ids are fresh per call
        assert(gen1[0].instance_id !== gen2[0].instance_id, "instance_id changes across generations");
        assert(gen1[1].instance_id !== gen2[1].instance_id, "instance_id changes across generations");
        // And instance_ids within a single generation are distinct (DD-19 ULID)
        assert(gen1[0].instance_id !== gen1[1].instance_id, "two candidates in same call have distinct instance_ids");
    });
    ...
    it("E-P97 DEFAULT_AUDIT_POLICY values match Phase 3 design contracts", () => {
        assertEqual(DEFAULT_AUDIT_POLICY.judgment_threshold, 10, "judgment_threshold = 10 per DD-13 P-14 sequencing");
        assertEqual(DEFAULT_AUDIT_POLICY.obligation_max_carry_forward, 3, "obligation_max_carry_forward = 3 per DD-17 lifecycle");
    });
    // -------------------------------------------------------------------------
     ...
     */
    function overrideHome(fakeHome) {
        const previousHome = process.env.HOME;
        process.env.HOME = fakeHome;
        return () => {
            if (previousHome === undefined)
    ...
    it("E-P98 collector promote mode: candidate_items disjoint from global_items", () => {
        ...
        writeLearningFile(path.join(fakeHome, ".onto", "learnings"), "structure", "- [fact] [methodology] [foundation] global-only-entry (source: p, d, 2026-01-01) [impact:normal]");
        // Project scope: a DIFFERENT learning
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "- [fact] [methodology] [foundation] project-only-entry (source: p, d, 2026-01-02) [impact:normal]");
        ...
        try {
            ...
            assertEqual(result.candidate_items[0].scope, "project", "candidate scope=project");
            assertEqual(result.candidate_items[0].content, "project-only-entry", "candidate content");
            ...
            for (const content of candidateContents) {
                assert(!globalContents.includes(content) ||
                    result.candidate_items.find((c) => c.content === content)?.scope !==
                        "global", `candidate "${content}" must not share (scope=global, content) with global_items`);
            }
        }
        finally {
            restore();
    ...
    it("E-P99 collector captures all event markers on a learning", () => {
        ...
        try {
            ...
            assert(item.event_markers[0].includes("applied-then-found-invalid"), "first marker kind preserved");
            assert(item.event_markers[0].includes("2026-02-15"), "first marker date preserved");
            // Second marker
            assert(item.event_markers[1].includes("observed-obsolete"), "second marker kind preserved");
            assert(item.event_markers[1].includes("2026-03-20"), "second marker date preserved");
    ...
    it("E-P100 collector captures retention_confirmed_at without mixing into event_markers", () => {
        ...
        try {
            ...
            const item = result.project_items[0];
            assertEqual(item.retention_confirmed_at, "2026-02-01", "retention_confirmed_at isolated");
            assertEqual(item.event_markers.length, 1, "only event marker captured");
            assert(item.event_markers[0].includes("applied-then-found-invalid"), "event marker preserved");
        }
        finally {
            restore();
        }
    ...
    it("E-P102 collector records parse errors without skipping surrounding valid lines", () => {
        ...
        try {
            ...
            assertEqual(result.project_items[0].content, "valid-before", "first valid");
            assertEqual(result.project_items[1].content, "valid-after", "second valid — surrounds errors");
    ...
    it("E-P103 collector baseline hash captures per-file metadata", () => {
        ...
        const fileContents = "- [fact] [methodology] [foundation] item (source: p, d, 2026-01-01) [impact:normal]\n";
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", fileContents);
        const restore = overrideHome(fakeHome);
        try {
            const result = collect({ mode: "promote", projectRoot });
            ...
            const projectBaselines = result.baseline_hash.files.filter((f) => f.scope === "project");
    ...
    // -------------------------------------------------------------------------
    function syntheticEmergencyEntry(sessionId, attemptId, generation) {
        return {
            schema_version: "1",
    ...
    it("E-P104 EmergencyLogSpec.validate rejects entries missing required fields", async () => {
        const { EmergencyLogSpec } = await import("../shared/specs/emergency-log-spec.js");
        ...
        assert(result.errors.some((e) => e.includes("entry_id")), "error mentions entry_id");
        ...
        assert(result2.errors.some((e) => e.includes("fatal_error_kind")), "error mentions fatal_error_kind");
    });
    ...
    it("E-P105 EmergencyLogSpec.parse rejects pre-v7 entries", async () => {
        ...
        assert(caught.message.toLowerCase().includes("pre-v7"), "message identifies pre-v7");
    });
    ...
    it("E-P106 EmergencyLogSpec.serialize emits one-line JSON without trailing newline", async () => {
        const { EmergencyLogSpec } = await import("../shared/specs/emergency-log-spec.js");
        const entry = syntheticEmergencyEntry("sess-106", "01AAAA", 0);
        ...
        assertEqual(reparsed.entry_id, "entry-0", "round-trip preserves entry_id");
    });
    ...
    it("E-P107 REGISTRY.appendToFile frames entries as JSONL", async () => {
        ...
        const e1 = syntheticEmergencyEntry("sess-107", "01AAAA", 0);
        const e2 = syntheticEmergencyEntry("sess-107", "01AAAA", 1);
        ...
        assertEqual(content.split("\n").length, 3, "3 split parts (two lines + trailing empty)");
    ...
    it("E-P108 REGISTRY.appendToFile rejects wrong schema_version", async () => {
        ...
        assert(caught.message.toLowerCase().includes("schema_version"), "error names schema_version");
    ...
    it("E-P109 appendToFile preserves append order for multi-entry JSONL", async () => {
        ...
        const e_hi = syntheticEmergencyEntry("sess-109", "01CCCC", 5);
        const e_lo = syntheticEmergencyEntry("sess-109", "01CCCC", 2);
        const e_mid = syntheticEmergencyEntry("sess-109", "01CCCC", 3);
    ...
    it("E-P111 retirement drops undated markers but surfaces them in reason", () => {
        ...
        assert(c.reason.includes("undated"), "reason surfaces undated marker count");
        assert(c.reason.includes("1 undated"), "reason shows exact undated count");
    });
    ...
    it("E-P112 retirement threshold=1 includes single-marker items", () => {
        ...
        assertEqual(candidates[0].item.content, "single-marker-item", "correct item");
    ...
    it("E-P114 session-root-guard Case 3: marker + legacy → warn, not throw", () => {
        ...
        assert(combined.includes("legacy") && combined.includes("migrate-session-roots"), "warning mentions legacy + remediation command");
    });
    ...
    it("E-P115 session-root-guard rejects incompatible layout marker", async () => {
        const { IncompatibleLayoutError } = await import("../../cli/session-root-guard.js");
        ...
        const rawYaml = 'schema_version: "1"\nlayout_version: v99-from-the-future\nwritten_at: "2026-04-09T00:00:00Z"\n';
        fs.writeFileSync(path.join(projectRoot, ".onto", ".layout-version.yaml"), rawYaml, "utf8");
        let caught = null;
        try {
            ensureSessionRootsMigrated(projectRoot);
        }
        ...
        assert(caught instanceof IncompatibleLayoutError, "IncompatibleLayoutError thrown");
        const err = caught;
        assert(err.message.includes("v3") || err.message.toLowerCase().includes("supported"), `message names the supported version or phrase (got: ${err.message})`);
    });
    ...
    it("E-P118 cross-agent dedup pre-filter groups overlapping cross-agent items", async () => {
        ...
        const items = [
            syntheticItem({
                agent_id: "structure",
                content: "verify preconditions constraint validation before applying changes durable",
            }),
            syntheticItem({
                agent_id: "axiology",
                // Shares: verify preconditions constraint validation applying changes
                content: "verify preconditions constraint validation while applying changes idempotent",
            }),
            syntheticItem({
        ...
        assert(shortlist.some((i) => i.agent_id === "structure"), "structure item present");
        assert(shortlist.some((i) => i.agent_id === "axiology"), "axiology item present");
    });
    ...
    it("E-P119 cross-agent dedup buildShortlists handles degenerate inputs", async () => {
        ...
        assertEqual(buildShortlists([
            syntheticItem({ agent_id: "structure", content: "alpha beta gamma delta" }),
        ]).shortlists.length, 0, "single item → empty");
        assertEqual(buildShortlists([
            syntheticItem({ agent_id: "structure", content: "alpha beta gamma delta" }),
            syntheticItem({ agent_id: "structure", content: "alpha beta gamma delta" }),
        ]).shortlists.length, 0, "single-agent pool → empty (only cross-agent counts)");
    });
    ...
    it("E-P120 discoverCrossAgentDedupClusters returns a cluster via mock LLM", async () => {
        const { discoverCrossAgentDedupClusters } = await import("./panel-reviewer.js");
        const previousMock = process.env.ONTO_LLM_MOCK;
        process.env.ONTO_LLM_MOCK = "1";
        try {
            const candidates = [
            ...
            assertEqual(c.member_items.length, 2, "both members preserved");
    ...
    it("E-P121 discoverCrossAgentDedupClusters cluster_id is deterministic", async () => {
        ...
        try {
            ...
            assertEqual(run1.clusters[0].cluster_id, run2.clusters[0].cluster_id, "cluster_id stable across runs");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P122 rewriteInsightRoleTag covers concrete + drop_role cases", async () => {
        ...
        const base = "- [fact] [methodology] [insight] body (source: p, d, 2026-01-01) [impact:normal]";
        // Concrete role → substitution
        const toGuardrail = rewriteInsightRoleTag(base, "guardrail");
        assertEqual(toGuardrail, "- [fact] [methodology] [guardrail] body (source: p, d, 2026-01-01) [impact:normal]", "guardrail replacement");
        const toFoundation = rewriteInsightRoleTag(base, "foundation");
        assertEqual(toFoundation, "- [fact] [methodology] [foundation] body (source: p, d, 2026-01-01) [impact:normal]", "foundation replacement");
        // drop_role with a trailing space → consume the trailing space
        const dropped = rewriteInsightRoleTag(base, "drop_role");
        assertEqual(dropped, "- [fact] [methodology] body (source: p, d, 2026-01-01) [impact:normal]", "drop_role removes bracket and trailing space");
        // Idempotent failure: raw line without [insight] → null
        const noInsight = rewriteInsightRoleTag("- [fact] [methodology] [foundation] already-applied (source: p, d, 2026-01-01) [impact:normal]", "guardrail");
    ...
    it("E-P123 applyInsightReclassifications rewrites role tags end-to-end", async () => {
        const { applyInsightReclassifications } = await import("./insight-reclassifier.js");
        const dir = makeTmpDir("e-p123");
        const agentFile = path.join(dir, "structure.md");
        const line1 = "- [fact] [methodology] [insight] alpha (source: p, d, 2026-01-01) [impact:normal]";
        const line2 = "- [fact] [methodology] [insight] beta (source: p, d, 2026-01-02) [impact:normal]";
        const otherLine = "- [fact] [methodology] [foundation] gamma (source: p, d, 2026-01-03) [impact:normal]";
        fs.writeFileSync(agentFile, [line1, line2, otherLine, ""].join("\n"), "utf8");
        ...
        assert(content.includes("[guardrail] alpha"), "alpha became guardrail");
        assert(!content.includes("[insight] alpha"), "[insight] gone for alpha");
        assert(content.includes("[methodology] beta"), "beta dropped its role bracket");
        assert(!content.includes("[insight] beta"), "[insight] gone for beta");
        assert(content.includes("[foundation] gamma"), "untouched foundation line preserved");
    });
    ...
    it("E-P124 applyInsightReclassifications is idempotent on repeat runs", async () => {
        ...
        const agentFile = path.join(dir, "structure.md");
        const line = "- [fact] [methodology] [insight] item (source: p, d, 2026-01-01) [impact:normal]";
        ...
        assertEqual(second.skipped_already_applied, 1, "second run detects already-applied");
    ...
    it("E-P126 applyInsightReclassifications dry-run does not write", async () => {
        ...
        assertEqual(result.entries[0].outcome, "would_apply", "entry outcome is would_apply, not applied");
        // File content unchanged
        const content = fs.readFileSync(agentFile, "utf8");
        assert(content.includes("[insight] item"), "dry-run preserved the original tag");
        assert(!content.includes("[guardrail]"), "dry-run did not write the new tag");
    });
    ...
    it("E-P127 applyInsightReclassifications surfaces missing-file failure", async () => {
        ...
        const existingLine = "- [fact] [methodology] [insight] ok (source: p, d, 2026-01-01) [impact:normal]";
        ...
        assert(failedEntry.error_message !== null &&
            failedEntry.error_message.includes("source file not found"), "error message surfaces the cause");
    ...
    it("E-P128 cross-agent dedup buildShortlists caps shortlist count", async () => {
        ...
        const built = buildShortlists(items);
        assertEqual(built.shortlists.length, MAX_SHORTLISTS_PER_RUN, "exactly MAX_SHORTLISTS_PER_RUN returned when more groups exist");
        // C4: bounded-loss metrics surface the dropped groups
        assert(built.shortlists_cap_dropped_count >= 5, `at least 5 groups cap-dropped (got ${built.shortlists_cap_dropped_count})`);
        assertEqual(built.total_valid_groups, 25, "all 25 groups were valid before cap");
    });
    ...
    it("E-P129 cross-agent dedup caps items per shortlist", async () => {
        ...
        const items = [];
        const sharedContent = "verify preconditions constraint validation before applying changes durable";
        for (let i = 0; i < 15; i++) {
            items.push(syntheticItem({
        ...
        assertEqual(built.shortlists[0].length, MAX_ITEMS_PER_SHORTLIST, "capped at MAX_ITEMS_PER_SHORTLIST");
        // C4: the truncation is surfaced in metrics
        assertEqual(built.shortlists_truncated_count, 1, "one shortlist was truncated");
        assertEqual(built.members_truncated_total, 15 - MAX_ITEMS_PER_SHORTLIST, "total members removed = original − cap");
    });
    ...
    it("E-P130 applyInsightReclassifications rewrites multiple files in one run", async () => {
        ...
        const lineA = "- [fact] [methodology] [insight] alpha (source: p, d, 2026-01-01) [impact:normal]";
        const lineB = "- [fact] [methodology] [insight] beta (source: p, d, 2026-01-02) [impact:normal]";
    ...
    it("E-P131 applyInsightReclassifications entries carry full per-outcome metadata", async () => {
        ...
        const fileA = path.join(dir, "structure.md");
        const lineA = "- [fact] [methodology] [insight] apply-me (source: p, d, 2026-01-01) [impact:normal]";
        fs.writeFileSync(fileA, lineA + "\n", "utf8");
        const reportPath = path.join(dir, "report.json");
        fs.writeFileSync(reportPath, JSON.stringify({
            session_id: "test-131",
        ...
        // scan can locate it.
        assertEqual(result.skipped_source_drift, 1, "one source drift (raw_line missing + line_number does not match)");
        assertEqual(result.skipped_already_applied, 0, "no evidence-based idempotent skips in this fixture");
        ...
        assertEqual(applied.anchor_resolution, "line_number_and_raw_line", "applied entry resolved via the strong anchor");
        const drifted = result.entries.find((e) => e.outcome === "skipped_source_drift");
        ...
        const noProp = result.entries.find((e) => e.outcome === "skipped_no_proposal");
    ...
    it("E-P132 runPromoter surfaces cross_agent_dedup_clusters via mock LLM", async () => {
        ...
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "- [fact] [methodology] [foundation] verify preconditions constraint validation applying changes durable (source: p, d, 2026-01-01) [impact:normal]");
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "axiology", "- [fact] [methodology] [foundation] verify preconditions constraint validation applying changes idempotent (source: p, d, 2026-01-02) [impact:normal]");
        try {
            const result = await runPromoter({
                mode: "promote",
                sessionId: "test-132",
            ...
            assert(cluster.member_items.length >= 2, "cluster contains multiple members");
            const agents = new Set(cluster.member_items.map((i) => i.agent_id));
            assert(agents.size >= 2, "members span ≥2 distinct agents");
            assertEqual(cluster.user_approval_required, true, "user approval required flag");
            assert(cluster.cluster_id.length === 12, "12-char deterministic cluster_id");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P133 significantTokens + jaccard handle empty/stopword-only content", async () => {
        ...
        const stopwordsOnly = significantTokens("these those have been being should would could will must does");
    ...
    it("E-P134 cross_agent_dedup applicator honors project source_path (C1)", async () => {
        ...
        const projectLine = "- [fact] [methodology] [foundation] project-scope candidate content (source: p, d, 2026-01-01) [impact:normal]";
        const projectLearningsDir = path.join(projectRoot, ".onto", "learnings");
        fs.mkdirSync(projectLearningsDir, { recursive: true });
        fs.writeFileSync(path.join(projectLearningsDir, "axiology.md"), ["<!-- format_version: 1 -->", projectLine].join("\n"), "utf8");
        // Global-scope sibling line under fakeOnto/learnings/
        const globalLine = "- [fact] [methodology] [foundation] global-scope sibling content (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", globalLine].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-134",
            projectRoot,
        ...
        const projectAxiologyPath = path.join(projectLearningsDir, "axiology.md");
        const globalStructurePath = path.join(fakeOnto, "learnings", "structure.md");
        const consolidatedLine = "- [fact] [methodology] [foundation] consolidated cross-scope principle (source: cluster, d, 2026-04-09) [impact:normal]";
        const cluster = {
        ...
        assert(projectContent.includes("consolidated") &&
            projectContent.includes("mixed-scope-test"), "project-scope member marked consolidated in the PROJECT file");
        assert(!fs.existsSync(path.join(fakeOnto, "learnings", "axiology.md")), "global axiology.md was NOT incorrectly created");
        ...
        assert(globalContent.includes("consolidated cross-scope principle"), "consolidated line landed in primary owner's global file");
    });
    ...
    it("E-P135 dedup rejects cluster when LLM primary_owner is off-shortlist (C2)", async () => {
        ...
        try {
            ...
            assert(shortlistAgents.has(discovery.clusters[0].primary_owner_agent), "primary_owner_agent must be in shortlist");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P136 insight apply evidence-based idempotency via line_number anchor (C3)", async () => {
        ...
        const agentFile = path.join(dir, "structure.md");
        const rawLine = "- [fact] [methodology] [insight] anchor-item (source: p, d, 2026-01-01) [impact:normal]";
        // File already has the REWRITTEN form at line 1 (simulating an earlier
        // apply run that succeeded)
        const rewritten = "- [fact] [methodology] [foundation] anchor-item (source: p, d, 2026-01-01) [impact:normal]";
        ...
        const result = applyInsightReclassifications({ reportPath });
        assertEqual(result.skipped_already_applied, 1, "line_number anchor catches already-rewritten form");
        assertEqual(result.applied, 0, "nothing re-applied");
        assertEqual(result.skipped_source_drift, 0, "NOT drift — evidence present");
        assertEqual(result.entries[0].anchor_resolution, "line_number_only", "anchor resolution records line_number_only");
        assertEqual(result.report_generated_at, "2026-04-09T10:00:00Z", "report generated_at surfaces in result");
    });
    ...
    it("E-P137 insight apply surfaces ambiguous raw_line as source drift (CC1)", async () => {
        ...
        const agentFile = path.join(dir, "structure.md");
        const ambiguousLine = "- [fact] [methodology] [insight] dup-body (source: p, d, 2026-01-01) [impact:normal]";
        ...
        assertEqual(result.skipped_source_drift, 1, "ambiguity routed to source_drift, not silent success");
        ...
        assert(entry.error_message !== null &&
            entry.error_message.includes("multiple verbatim"), "error message names the ambiguity cause");
    });
    ...
    it("E-P138 significantTokens preserves Korean characters (U4)", async () => {
        ...
        const built = buildShortlists(items);
        assertEqual(built.shortlists.length, 1, "Korean-heavy cross-agent pair forms a shortlist");
    });
    ...
    it("E-P139 createRecoverabilityCheckpoint honors ontoHome override (U3)", async () => {
        const { createRecoverabilityCheckpoint } = await import("../shared/recoverability.js");
        ...
        fs.writeFileSync(path.join(overriddenLearnings, "structure.md"), "- [fact] [methodology] [foundation] override-test (source: p, d, 2026-01-01) [impact:normal]\n", "utf8");
        const prep = await createRecoverabilityCheckpoint("test-139", projectRoot, "01TEST139", 0, { ontoHome: fakeOnto });
        assertEqual(prep.kind, "created", "checkpoint created");
        if (prep.kind !== "created" || !prep.checkpoint)
            return;
        const globalBackup = prep.checkpoint.backups.find((b) => b.source_kind === "global_learnings");
        assert(globalBackup !== undefined, "global_learnings backup entry present");
        // The source_path must point at the OVERRIDDEN ontoHome, not ~/.onto
        assertEqual(globalBackup.source_path, overriddenLearnings, "global_learnings source_path matches override");
    });
    ...
    it("E-P140 onto reclassify-insights --apply routes argv through handler (U7)", async () => {
        ...
        const agentFile = path.join(dir, "structure.md");
        const line = "- [fact] [methodology] [insight] cli-test (source: p, d, 2026-01-01) [impact:normal]";
        fs.writeFileSync(agentFile, line + "\n", "utf8");
        const reportPath = path.join(dir, "report.json");
        fs.writeFileSync(reportPath, JSON.stringify({
            session_id: "test-140",
    ...
    it("E-P141 dedup drops cluster + bumps metric when LLM returns off-shortlist owner (CG3)", async () => {
        ...
        try {
            const items = [
                syntheticItem({
                    agent_id: "structure",
                    content: "invariant check constraint validation rollback guarantee durable",
                }),
                syntheticItem({
                    agent_id: "axiology",
                    content: "invariant check constraint validation rollback guarantee idempotent",
                }),
            ];
            const discovery = await discoverCrossAgentDedupClusters(items, []);
            assertEqual(discovery.clusters.length, 0, "cluster dropped because owner is off-shortlist");
            assertEqual(discovery.metrics.llm_failures.primary_owner_not_in_shortlist, 1, "metric increment: primary_owner_not_in_shortlist");
            ...
            if (previousBogus === undefined)
                ...
                process.env.ONTO_LLM_MOCK_DEDUP_BOGUS_OWNER = previousBogus;
        }
    });
    ...
    it("E-P142 same_principle=false counted outside llm_failures (UF2)", async () => {
        ...
        try {
            ...
            assertEqual(discovery.metrics.same_principle_rejected, 1, "same_principle_rejected counter bumped");
            // NOT counted under llm_failures
            assertEqual(discovery.metrics.llm_failures.provider_error, 0, "not provider_error");
            assertEqual(discovery.metrics.llm_failures.malformed_json, 0, "not malformed");
            assertEqual(discovery.metrics.llm_failures.missing_field, 0, "not missing_field");
            assertEqual(discovery.metrics.llm_failures.primary_owner_not_in_shortlist, 0, "not owner mismatch");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P143 runPromoter surfaces cross_agent_dedup warnings end-to-end (UF3)", async () => {
        ...
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "- [fact] [methodology] [foundation] verify constraint validation applying change durable (source: p, d, 2026-01-01) [impact:normal]");
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "axiology", "- [fact] [methodology] [foundation] verify constraint validation applying change idempotent (source: p, d, 2026-01-02) [impact:normal]");
        try {
            const result = await runPromoter({
                mode: "promote",
                sessionId: "test-143",
            ...
            assertEqual(result.report.cross_agent_dedup_clusters.length, 0, "no clusters passed runtime guard");
            // But a warning DID surface
            const warnings = result.report.warnings ?? [];
            const dedupWarnings = warnings.filter((w) => w.includes("cross_agent_dedup:"));
            assert(dedupWarnings.length > 0, `expected cross_agent_dedup warning in report.warnings (got ${JSON.stringify(warnings)})`);
            assert(dedupWarnings.some((w) => w.includes("primary_owner_not_in_shortlist")), "warning names the specific failure channel");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P144 cross_agent_dedup marks same-agent siblings (CG1 transitive)", async () => {
        ...
        const structureLineA = "- [fact] [methodology] [foundation] same principle early (source: p, d, 2026-01-01) [impact:normal]";
        const structureLineB = "- [fact] [methodology] [foundation] same principle later (source: p, d, 2026-02-01) [impact:normal]";
        const axiologyLine = "- [fact] [methodology] [foundation] same principle philosophical (source: p, d, 2026-01-10) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLineA, structureLineB].join("\n"));
        writeLearningFile(path.join(fakeOnto, "learnings"), "axiology", ["<!-- format_version: 1 -->", axiologyLine].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-144",
            projectRoot,
        ...
        const consolidatedLine = "- [fact] [methodology] [foundation] consolidated transitive (source: cluster, d, 2026-04-09) [impact:normal]";
        const cluster = {
        ...
        assert(structureLines.includes(structureLineA), "primary member (structureLineA) kept as exact line in structure file");
        // structureLineB (sibling) must be marked consolidated — the exact line
        // should no longer be present in the file as-is.
        assert(!structureLines.includes(structureLineB), "sibling member (structureLineB) no longer present as raw line");
        // Positive check: a consolidated marker referencing this sibling exists.
        assert(structureLines.some((l) => l.includes("consolidated") &&
            l.includes("transitive-test") &&
            l.includes("same principle later")), "structure file carries consolidated marker for sibling structureLineB");
        // axiologyLine must also be marked — exact line replaced.
        assert(!axiologyLines.includes(axiologyLine), "axiology member no longer present as raw line");
        assert(axiologyLines.some((l) => l.includes("consolidated") &&
            l.includes("transitive-test") &&
            l.includes("same principle philosophical")), "axiology file carries consolidated marker");
        // Consolidated line was appended to the primary file
        assert(structureContent.includes("consolidated transitive"), "consolidated line appended to primary (structure) file");
        assert(structureContent.includes("cluster_id: transitive-test"), "cluster marker present in primary");
    });
    ...
    it("E-P145 cross_agent_dedup preflight rejects ambiguous duplicate raw_line (CG2)", async () => {
        ...
        const structureLine = "- [fact] [methodology] [foundation] primary alpha (source: p, d, 2026-01-01) [impact:normal]";
        ...
        const axiologyLine = "- [fact] [methodology] [foundation] duplicate beta (source: p, d, 2026-01-01) [impact:normal]";
        const axiologyFile = path.join(fakeOnto, "learnings", "axiology.md");
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLine].join("\n"));
        writeLearningFile(path.join(fakeOnto, "learnings"), "axiology", [
            "<!-- format_version: 1 -->",
            axiologyLine,
            "- [fact] [methodology] [foundation] decoy (source: p, d, 2026-01-01) [impact:normal]",
            axiologyLine,
        ...
        const structurePath = path.join(fakeOnto, "learnings", "structure.md");
        const consolidatedLine = "- [fact] [methodology] [foundation] consolidated ambig (source: cluster, d, 2026-04-09) [impact:normal]";
        const cluster = {
        ...
        assert(outcome.kind === "failed_resumable", `expected failed_resumable (got ${outcome.kind})`);
        ...
        assertEqual(matchCount, 2, "both duplicate raw_lines preserved (no silent mutation)");
        assert(!axiologyContent.includes("consolidated (") ||
            !axiologyContent.includes("ambig-test"), "no consolidated marker written for ambiguous member");
    });
    // -------------------------------------------------------------------------
    ...
    it("E-P146 cross_agent_dedup index-based identity handles identical raw_line members (SYN-C1)", async () => {
        ...
        const sharedLine = "- [fact] [methodology] [foundation] literally identical content (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", sharedLine].join("\n"));
        const projectLearningsDir = path.join(projectRoot, ".onto", "learnings");
        fs.mkdirSync(projectLearningsDir, { recursive: true });
        const projectStructurePath = path.join(projectLearningsDir, "structure.md");
        fs.writeFileSync(projectStructurePath, ["<!-- format_version: 1 -->", sharedLine].join("\n"), "utf8");
        const globalStructurePath = path.join(fakeOnto, "learnings", "structure.md");
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-146",
        ...
        const consolidatedLine = "- [fact] [methodology] [foundation] consolidated identical case (source: cluster, d, 2026-04-09) [impact:normal]";
        const cluster = {
        ...
        assert(globalLines.includes(sharedLine), "global structure.md primary slot preserved");
        assert(globalLines.some((l) => l.includes("consolidated identical case")), "consolidated line appended to global primary");
        ...
        const projectLines = fs
            ...
            .split("\n");
        assert(!projectLines.includes(sharedLine), "project structure.md sharedLine was replaced (non-primary slot)");
        assert(projectLines.some((l) => l.includes("consolidated") && l.includes("identical-content-test")), "project structure carries consolidated marker");
    });
    ...
    it("E-P147 replaceLineAtIndex honors resolved anchor, not first verbatim match (SYN-C2)", async () => {
        ...
        const structureLine = "- [fact] [methodology] [foundation] primary alpha (source: p, d, 2026-01-01) [impact:normal]";
        const duplicateLine = "- [fact] [methodology] [foundation] dup content beta (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLine].join("\n"));
        ...
        const structurePath = path.join(fakeOnto, "learnings", "structure.md");
        const consolidatedLine = "- [fact] [methodology] [foundation] consolidated anchor auth (source: cluster, d, 2026-04-09) [impact:normal]";
        const cluster = {
        ...
        assert(axiologyLines[3].includes("consolidated") &&
            axiologyLines[3].includes("anchor-auth-test"), `line 4 (index 3) replaced with consolidated marker (got "${axiologyLines[3]}")`);
        // The FIRST copy at index 1 must remain untouched.
        assertEqual(axiologyLines[1], duplicateLine, "line 2 (index 1) is unchanged — SYN-C2 fix prevents first-verbatim replacement");
    });
    ...
    it("E-P148 cross_agent_dedup fails closed on partial-apply state (SYN-CC1)", async () => {
        ...
        const primaryLine = "- [fact] [methodology] [foundation] primary untouched (source: p, d, 2026-01-01) [impact:normal]";
        const memberLine = "- [fact] [methodology] [foundation] member pre-marked (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", primaryLine].join("\n"));
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated partial (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        const structureContent = fs.readFileSync(structurePath, "utf8");
        assert(!structureContent.includes("consolidated partial"), "primary file NOT mutated");
        assert(!structureContent.includes("cluster_id: partial-apply-test"), "primary file has NO cluster marker (fail-closed)");
        // Verify the failure surfaced in apply-state with the SYN-CC1 message
        const applyStatePath = path.join(promoter.sessionRoot, "promote-execution-result.json");
        ...
        const dedupFail = failed.find((f) => f.decision_kind === "cross_agent_dedup");
        assert(dedupFail !== undefined, "dedup failure present");
        assert(dedupFail.error_message.includes("Manual recovery required") ||
            dedupFail.error_message.includes("SYN-CC1"), `error message names manual recovery path (got: ${dedupFail.error_message})`);
    });
    ...
    it("E-P149 pickPrimaryMemberIndex edge cases (SYN-D1, SYN-U2)", async () => {
        ...
        assertEqual(pickPrimaryMemberIndex(single, "structure"), 0, "single owner candidate → its index");
        ...
        assertEqual(pickPrimaryMemberIndex(multi, "structure"), 2, "earliest dated structure member wins (index 2)");
        ...
        assertEqual(pickPrimaryMemberIndex(equalDates, "structure"), 0, "equal dates → first-encountered owner wins (index 0)");
        ...
        assertEqual(pickPrimaryMemberIndex(nullAfter, "structure"), 2, "dated member outranks null-dated sibling (index 2)");
        ...
        assertEqual(pickPrimaryMemberIndex(datedAfter, "structure"), 0, "dated member at index 0 wins over null-dated sibling");
        ...
        assertEqual(pickPrimaryMemberIndex(allNull, "structure"), 1, "all-null owners → first encountered (index 1)");
        ...
        assertEqual(pickPrimaryMemberIndex(dupContent, "structure"), 0, "earliest of two identical-content owners by slot (index 0)");
    });
    ...
    it("E-P150 same_principle_rejected surfaces in report.warnings (SYN-U3)", async () => {
        ...
        const fakeAuditStatePath = path.join(fakeOnto, "audit-state.yaml");
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "structure", "- [fact] [methodology] [foundation] overlapping constraint validation applying change durable (source: p, d, 2026-01-01) [impact:normal]");
        writeLearningFile(path.join(projectRoot, ".onto", "learnings"), "axiology", "- [fact] [methodology] [foundation] overlapping constraint validation applying change idempotent (source: p, d, 2026-01-02) [impact:normal]");
        try {
            const result = await runPromoter({
                mode: "promote",
                sessionId: "test-150",
            ...
            assertEqual(result.report.cross_agent_dedup_clusters.length, 0, "no cluster (LLM rejected)");
            const warnings = result.report.warnings ?? [];
            const rejected = warnings.filter((w) => w.includes("cross_agent_dedup:") &&
                w.includes("same_principle=false"));
            assert(rejected.length > 0, `expected same_principle=false warning (got ${JSON.stringify(warnings)})`);
            // Ensure it's not conflated with failure warnings
            assert(!rejected[0].includes("LLM failures"), "warning is NOT labeled as llm_failures");
        }
        finally {
            if (previousMock === undefined)
                delete process.env.ONTO_LLM_MOCK;
    ...
    it("E-P151 trailing-newline guard protects applyPromotion path (SYN-U4)", async () => {
        ...
        const lastExistingLine = "- [fact] [methodology] [foundation] existing tail line (source: p, d, 2026-01-01) [impact:normal]";
        const globalStructurePath = path.join(fakeOnto, "learnings", "structure.md");
        fs.mkdirSync(path.dirname(globalStructurePath), { recursive: true });
        // Write WITHOUT trailing newline
        fs.writeFileSync(globalStructurePath, ["<!-- format_version: 1 -->", lastExistingLine].join("\n"), "utf8");
        // Project file with candidate line
        const candidateLine = "- [fact] [methodology] [foundation] newly promoted content (source: p, d, 2026-01-15) [impact:normal]";
        const projectStructurePath = path.join(projectRoot, ".onto", "learnings", "structure.md");
        fs.mkdirSync(path.dirname(projectStructurePath), { recursive: true });
        fs.writeFileSync(projectStructurePath, ["<!-- format_version: 1 -->", candidateLine].join("\n"), "utf8");
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-151",
            projectRoot,
        ...
        assert(globalLines.includes(lastExistingLine), "existing tail line preserved as a standalone line (no collision)");
        assert(globalLines.includes(candidateLine), "promoted candidate line present as a standalone line");
        // No line contains BOTH — that would indicate collision
        const collision = globalLines.find((l) => l.includes("existing tail line") && l.includes("newly promoted content"));
        assert(collision === undefined, "no line collides existing + promoted content");
    });
    // -------------------------------------------------------------------------
    ...
    it("E-P152 PromoteReportSpec rejects legacy schema v1 with regenerate guidance (4-C1)", async () => {
        const { PromoteReportSpec } = await import("../shared/specs/promote-report-spec.js");
        const legacyJson = JSON.stringify({
            schema_version: "1",
        ...
        assert(caught instanceof IncompatibleVersionError, "expected IncompatibleVersionError");
        const msg = caught.message;
        assert(msg.includes('legacy schema_version="1"'), "message names the legacy version explicitly");
        assert(msg.includes("primary_member_index"), "message names the field that's missing in v1");
        assert(msg.includes("onto promote"), "message names the regenerate command");
        assert(msg.toLowerCase().includes("discard") ||
            msg.toLowerCase().includes("regenerate"), "message gives an action verb");
    });
    ...
    it("E-P153 replaceLineAtIndex fail-closes on post-preflight drift", async () => {
        ...
        const axiologyLine = "- [fact] [methodology] [foundation] drift test line (source: p, d, 2026-01-01) [impact:normal]";
        const axiologyPath = path.join(fakeOnto, "learnings", "axiology.md");
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLine].join("\n"));
        writeLearningFile(path.join(fakeOnto, "learnings"), "axiology", ["<!-- format_version: 1 -->", axiologyLine].join("\n"));
        const promoter = await runPromoter({
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated drift (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        assert(outcome.kind === "failed_resumable", `expected failed_resumable on drift (got ${outcome.kind})`);
        // Primary file must NOT have been mutated (no consolidated line, no cluster marker)
        const structureContent = fs.readFileSync(structurePath, "utf8");
        assert(!structureContent.includes("consolidated drift"), "primary file untouched on drift");
        assert(!structureContent.includes("cluster_id: drift-test"), "cluster marker NOT written on drift");
    });
    ...
    it("E-P154 SYN-CC1 error message includes concrete recovery guidance (4-D2(a))", async () => {
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated cc1 guidance (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        const msg = dedupFail.error_message;
        // Manual recovery heading
        assert(msg.includes("Manual recovery required") && msg.includes("SYN-CC1"), "message names manual recovery + SYN-CC1 contract");
        // Option A: checkpoint manifest path (session-specific)
        assert(msg.includes("restore-manifest.yaml"), "option A references restore-manifest.yaml");
        assert(msg.includes("test-154"), "checkpoint path is session-specific (names session id)");
        // Option B: stray marker removal + rerun command
        assert(msg.includes("remove the stray") &&
            msg.includes("consolidated"), "option B names the marker removal action");
        assert(msg.includes("onto promote --apply test-154"), "option B includes the exact rerun command with session id");
    });
    ...
    it("E-P155 PromoteReportSpec rejects malformed primary_member_index values (4-Rec1)", async () => {
        const { PromoteReportSpec } = await import("../shared/specs/promote-report-spec.js");
        function buildReport(primaryMemberIndex, memberItemCount = 2) {
            return {
                schema_version: "2",
                session_id: "test-155",
        ...
        assert(neg.errors.some((e) => e.includes("primary_member_index")), "error names the field");
        ...
        assert(frac.errors.some((e) => e.includes("primary_member_index")), "error names the field");
        ...
        assert(oor.errors.some((e) => e.includes("primary_member_index")), "error names the field");
        assert(oor.errors.some((e) => e.includes("out of member_items bounds")), "error identifies bounds violation");
        ...
        assert(missing.errors.some((e) => e.includes("primary_member_index")), "error names the field");
        ...
        assert(!ok.errors.some((e) => e.includes("primary_member_index")), "valid integer 0 does NOT produce primary_member_index errors");
    });
    ...
    it("E-P156 withFileLock fails fast when another holder occupies the lock", async () => {
        ...
        const structureLine = "- [fact] [methodology] [foundation] primary (source: p, d, 2026-01-01) [impact:normal]";
        const axiologyLine = "- [fact] [methodology] [foundation] member (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLine].join("\n"));
        const axiologyPath = path.join(fakeOnto, "learnings", "axiology.md");
        writeLearningFile(path.join(fakeOnto, "learnings"), "axiology", ["<!-- format_version: 1 -->", axiologyLine].join("\n"));
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated lock test (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        assert(elapsed < 7000, `lock wait should terminate around waitMs=5000 (took ${elapsed}ms)`);
        assert(outcome.kind === "failed_resumable", `expected failed_resumable, got ${outcome.kind}`);
        // Verify the failure message surfaces the lock path
        const applyState = JSON.parse(fs.readFileSync(path.join(promoter.sessionRoot, "promote-execution-result.json"), "utf8"));
        const failed = applyState.failed_decisions ?? [];
        const dedupFail = failed.find((f) => f.decision_kind === "cross_agent_dedup");
        assert(dedupFail !== undefined, "dedup failure recorded");
        assert(dedupFail.error_message.includes("could not acquire lock") ||
            dedupFail.error_message.includes("lock"), `error message names the lock problem (got: ${dedupFail.error_message})`);
        // Neither file should have been mutated
        const structureContent = fs.readFileSync(structurePath, "utf8");
        const axiologyContent = fs.readFileSync(axiologyPath, "utf8");
        assert(!structureContent.includes("consolidated lock test"), "primary untouched");
        assert(axiologyContent.includes(axiologyLine), "member unchanged (original raw_line preserved)");
    ...
    it("E-P157 pickPrimaryMemberIndex uses explicit slot tiebreaker (4-Rec3)", async () => {
        ...
        assertEqual(pickPrimaryMemberIndex(allEqual, "structure"), 0, "lowest slot index wins for all-equal candidates");
        ...
        assertEqual(pickPrimaryMemberIndex(tiedDates, "structure"), 2, "earliest date wins; among ties, lowest slot index (2 < 3)");
    });
    // -------------------------------------------------------------------------
    ...
    it("E-P158 withFileLock reclaims stale lock when holder PID is dead", async () => {
        ...
        const structureLine = "- [fact] [methodology] [foundation] primary reclaim (source: p, d, 2026-01-01) [impact:normal]";
        const axiologyLine = "- [fact] [methodology] [foundation] member reclaim (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureLine].join("\n"));
        const axiologyPath = path.join(fakeOnto, "learnings", "axiology.md");
        writeLearningFile(path.join(fakeOnto, "learnings"), "axiology", ["<!-- format_version: 1 -->", axiologyLine].join("\n"));
        ...
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated reclaim (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        assertEqual(outcome.kind, "completed", "executor completed — stale lock was reclaimed");
        // The stale lockfile must be gone after reclaim + release
        assert(!fs.existsSync(lockPath), "stale lockfile was removed (reclaimed, then released by successful apply)");
        // Axiology file was successfully marked
        const philoContent = fs.readFileSync(axiologyPath, "utf8");
        assert(philoContent.includes("consolidated") && philoContent.includes("reclaim-test"), "axiology marked after reclaim");
    });
    ...
    it("E-P159 isPending filter prevents promotion replay on resume (narrow proof)", async () => {
        ...
        const candidateLine = "- [fact] [methodology] [foundation] rerun fidelity candidate (source: p, d, 2026-01-01) [impact:normal]";
        const projectLearnings = path.join(projectRoot, ".onto", "learnings");
        fs.mkdirSync(projectLearnings, { recursive: true });
        fs.writeFileSync(path.join(projectLearnings, "structure.md"), ["<!-- format_version: 1 -->", candidateLine].join("\n"), "utf8");
        ...
        const structureGlobal = "- [fact] [methodology] [foundation] global structure (source: p, d, 2026-01-01) [impact:normal]";
        writeLearningFile(path.join(fakeOnto, "learnings"), "structure", ["<!-- format_version: 1 -->", structureGlobal].join("\n"));
        const promoter = await runPromoter({
            mode: "promote",
            sessionId: "test-159",
            projectRoot,
        ...
        const structurePath = path.join(fakeOnto, "learnings", "structure.md");
        const missingMemberFile = path.join(fakeOnto, "learnings", "axiology-missing.md");
        const cluster = {
            ...
            consolidated_line: "- [fact] [methodology] [foundation] consolidated rerun (source: cluster, d, 2026-04-09) [impact:normal]",
            user_approval_required: true,
        };
        promoter.report.cross_agent_dedup_clusters.push(cluster);
        fs.writeFileSync(promoter.reportPath, JSON.stringify(promoter.report, null, 2));
        ...
        assert(firstOutcome.kind === "failed_resumable", `first apply fails resumable (got ${firstOutcome.kind})`);
        ...
        assert(globalContent.includes("rerun fidelity candidate"), "first apply: promotion landed in global structure.md");
        ...
        const promoCountBefore = (beforeRerun.match(/rerun fidelity candidate/g) ?? []).length;
        assertEqual(promoCountBefore, 1, "exactly one copy of the promoted line before rerun");
        ...
        // file.
        assert(secondOutcome.kind === "completed" ||
            secondOutcome.kind === "failed_resumable" ||
            secondOutcome.kind === "no_decisions", `second apply returns a terminal outcome (got ${secondOutcome.kind})`);
        ...
        const promoCountAfter = (globalContent.match(/rerun fidelity candidate/g) ?? []).length;
        assertEqual(promoCountAfter, 1, "rerun did NOT replay the already-applied promotion (isPending filter on resume path)");
        ...
        const applyStatePath = path.join(promoter.sessionRoot, "promote-execution-result.json");
        const applyState = JSON.parse(fs.readFileSync(applyStatePath, "utf8"));
        assert(Array.isArray(applyState.applied_decisions) &&
            applyState.applied_decisions.some((a) => a.decision_kind === "promotion"), "apply-state records the promotion as applied");
    });
    ...
    it("E-P160 withFileLock releases lock even when wrapped fn throws", async () => {
        const { __testExports: execExports } = await import("./promote-executor.js");
        ...
        assert(!fs.existsSync(lockPath), "lockfile removed after successful fn (happy)");
        ...
        assertEqual(caught.message, "boom", "original error preserved");
        assert(!fs.existsSync(lockPath), "lockfile removed after fn threw (critical — prevents deadlock on retry)");
        ...
        assert(elapsed < 1000, `second acquire was fast (no stale-wait), elapsed=${elapsed}ms`);
    });
    ...
    it("E-P161 withFileLock wait is non-spinning (Atomics.wait, low CPU)", async () => {
        ...
        try {
            ...
            threw = true;
            assert(e instanceof Error && e.message.includes("could not acquire lock"), `expected lock-wait timeout error (got ${e.message})`);
        }
        ...
        assert(wallElapsed >= waitMs - 50, `wall time respected waitMs (elapsed=${wallElapsed}ms, waitMs=${waitMs}ms)`);
        ...
        assert(cpuMsTotal < wallElapsed * 0.5, `non-spinning wait: cpu time should be << wall time ` +
            `(cpu=${cpuMsTotal.toFixed(1)}ms, wall=${wallElapsed}ms, ratio=${((cpuMsTotal / wallElapsed) *
                100).toFixed(1)}%)`);
        // Cleanup
        try {
            fs.unlinkSync(lockPath);
    ...
    it("E-P162 withFileLock refuses reclaim across malformed-PID parser shapes (6-SYN-U1)", async () => {
        ...
        assert(caught1 instanceof Error &&
            caught1.message.includes("could not acquire lock"), "malformed PID lockfile → acquire times out (no reclaim)");
        assert(elapsed1 >= 200, `waited at least waitMs (elapsed=${elapsed1}ms)`);
        // The lockfile is still there — reclaim did NOT happen
        assert(fs.existsSync(lockPath), "malformed lockfile preserved — reclaim refused");
        ...
        assert(caught2 instanceof Error &&
            caught2.message.includes("could not acquire lock"), "empty lockfile → acquire times out (no reclaim)");
        assert(fs.existsSync(lockPath), "empty lockfile preserved — reclaim refused");
        ...
        assert(caught3 instanceof Error &&
            caught3.message.includes("could not acquire lock"), "negative PID lockfile → acquire times out (no reclaim)");
        assert(fs.existsSync(lockPath), "negative-PID lockfile preserved — reclaim refused");
        // Cleanup
        try {
            fs.unlinkSync(lockPath);
        }

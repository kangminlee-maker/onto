...
class ArtifactRegistry {
    ...
    ensureInitialized() {
        ...
        if (!this.builtinRegistrar) {
            ...
            this.cachedInitError = new RegistryInitError("no builtin registrar wired. Import 'artifact-registry-init.js' " +
        ...
        try {
            ...
            this.cachedInitError = new RegistryInitError(error instanceof Error ? error.message : String(error), error instanceof Error ? error : undefined);
            throw this.cachedInitError;
        }
    }
    /**
    ...
    register(spec) {
        if (spec.supported_schema_versions.length === 0) {
            throw new Error(`${spec.kind}: supported_schema_versions cannot be empty`);
        }
        if (!spec.supported_schema_versions.includes(spec.current_schema_version)) {
            throw new Error(`${spec.kind}: current_schema_version "${spec.current_schema_version}" ` +
        ...
        if (spec.supported_schema_versions.length > 1 && !spec.migrate) {
            throw new Error(`${spec.kind}: supports ${spec.supported_schema_versions.length} schema versions ` +
    ...
    get(kind) {
        ...
        if (!spec) {
            throw new UnregisteredArtifactKindError(`Artifact kind '${kind}' not registered. ` +
                `Available: ${[...this.specs.keys()].sort().join(", ")}`);
        }
        return spec;
    }
    loadFromFile(kind, filePath) {
        this.ensureInitialized();
        ...
        if (!validation.valid) {
            throw new InvalidArtifactError(`${kind}: ${validation.errors.join(", ")}`);
        }
        return parsed;
    }
    saveToFile(kind, filePath, data) {
        this.ensureInitialized();
        const spec = this.get(kind);
        // SYN-CC1 part 1: structural validation before write
        const validation = spec.validate(data);
        if (!validation.valid) {
            throw new InvalidArtifactError(`${kind} validation failed: ${validation.errors.join(", ")}`);
        }
        ...
        if (dataObj.schema_version === undefined) {
            throw new InvalidArtifactError(`${kind}: data missing required schema_version field. ` +
                `Expected "${spec.current_schema_version}".`);
        }
        if (dataObj.schema_version !== spec.current_schema_version) {
            throw new InvalidArtifactError(`${kind}: data has schema_version "${dataObj.schema_version}" ` +
     ...
     */
    appendToFile(kind, filePath, entry) {
        this.ensureInitialized();
        const spec = this.get(kind);
        ...
        const dataObj = entry;
        if (dataObj.schema_version === undefined) {
            throw new InvalidArtifactError(`${kind}: entry missing required schema_version field.`);
        }
        if (dataObj.schema_version !== spec.current_schema_version) {
            throw new InvalidArtifactError(`${kind}: entry has schema_version "${dataObj.schema_version}" ` +
                `but spec current is "${spec.current_schema_version}".`);
        }
        const format = formatFromPath(filePath, kind, spec);
...
function formatFromPath(filePath, kind, spec) {
    ...
    if (ext === "json" || ext === "jsonl") {
        ...
        throw new Error(`${kind}: unsupported file extension ".${ext}" (path=${filePath})`);
    }
    if (!spec.supported_formats.includes(format)) {
        throw new Error(`${kind}: format ${format} (from ${filePath}) not in supported_formats ` +

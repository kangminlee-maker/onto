...
function formatIssueTimestamp(now) {
    const pad = (n) => String(n).padStart(2, "0");
    return (`${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())} ` +
        `${pad(now.getHours())}:${pad(now.getMinutes())}`);
}
function renderIssueEntry(title, symptom, ctx, now) {
    const lines = [
        "",
...
export function logLiteLLMIssue(title, symptom, ctx) {
    ...
    try {
        ...
        process.stderr.write(`[onto] Failed to write LiteLLM issue log to ${logPath}: ${writeErr.message ?? String(writeErr)}\n`);
    }
}
/**
...
export function resolveLearningProviderConfig(args) {
    ...
    const base_url = cli.llm_base_url ?? process.env.LITELLM_BASE_URL ?? config.llm_base_url;
    ...
    if (config.anthropic?.model)
        models_per_provider.anthropic = config.anthropic.model;
    if (config.openai?.model)
        models_per_provider.openai = config.openai.model;
    if (config.litellm?.model)
        models_per_provider.litellm = config.litellm.model;
    if (config.codex?.model)
        models_per_provider.codex = config.codex.model;
    const out = {};
    if (provider)
...
function pickProviderModel(config, provider) {
    switch (provider) {
        case "codex":
 ...
 */
function resolveProvider(preferred, configBaseUrl) {
...
function tryNonCodexProviders(configBaseUrl) {
...
function maybeEmitCodexInstallNotice(opts) {
    if (codexInstallNoticeShown || opts.suppress)
        return;
...
function maybeEmitCostOrderTransitionNotice(resolved) {
    if (costOrderTransitionNoticeShown)
        return;
    if (process.env.ONTO_SUPPRESS_COST_ORDER_NOTICE === "1")
        return;
    if (resolved.provider !== "codex" && resolved.provider !== "litellm")
        return;
    ...
    const msg = [
        `[onto] provider resolution changed by cost-order: would have used ${wouldHaveBeen} (per-token), now using ${resolved.provider} (${resolved.provider === "codex" ? "subscription" : "variable; typically per-token audit"}).`,
...
export function __resetNoticeFlagsForTests() {
...
async function callOpenAI(systemPrompt, userPrompt, apiKey, modelId, maxTokens, baseUrl, providerLabel = "openai") {
    ...
    try {
        ...
        if (isLiteLLM) {
            logLiteLLMIssue(`LiteLLM call failed (${e.status ?? e.name ?? "unknown"})`, e.message ?? String(err), {
                model_id: modelId,
                base_url: baseUrl,
                status: e.status,
                error_name: e.name,
    ...
    if (isLiteLLM && text === "") {
        logLiteLLMIssue("LiteLLM returned empty content", "chat.completions succeeded but message.content was empty", {
            model_id: modelId,
            base_url: baseUrl,
            prompt_hash: promptHash,
        });
 ...
 */
async function callCodexCli(systemPrompt, userPrompt, modelId, reasoningEffort) {
    ...
    emitModelCallLog(`codex call: model="${modelId ?? "(codex default)"}" effort="${reasoningEffort ?? "(unset)"}" timeout_ms=${DEFAULT_TIMEOUT_MS}`);
...
async function dispatchByPlan(systemPrompt, userPrompt, config) {
    ...
    if (plan.provider_identity === "litellm") {
        ...
        if (!baseUrl) {
            throw new Error("ExecutionPlan.provider_identity=litellm requires base_url (plan.base_url, config.base_url, or LITELLM_BASE_URL env)");
        }
        const modelId = config.model_id ?? plan.model_id ?? config.models_per_provider?.litellm;
        if (!modelId)
            throw missingModelError("litellm");
        const apiKey = process.env.LITELLM_API_KEY ?? "sk-litellm-placeholder";
...
export async function callLlm(systemPrompt, userPrompt, config) {
    ...
    if (config?.provider === "codex") {
        return callCodexCli(systemPrompt, userPrompt, config.model_id ?? config.models_per_provider?.codex, config.reasoning_effort);
    }
    ...
    if (config?.provider === "litellm") {
        ...
        if (!baseUrl) {
            throw new Error("external_http_provider=litellm requires base_url (set via LlmCallConfig.base_url, env LITELLM_BASE_URL, or .onto/config.yml llm_base_url)");
        }
    ...
    switch (resolved.provider) {
        ...
        case "litellm": {
            ...
            if (!modelId)
                throw missingModelError("litellm");
            return callOpenAI(systemPrompt, userPrompt, resolved.apiKey, modelId, maxTokens, resolved.baseUrl, "litellm");
        }
        case "anthropic": {
            const modelId = config?.model_id ?? perProviderModel;
            if (!modelId)

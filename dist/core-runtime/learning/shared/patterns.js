...
export const TAG_PATTERN = /^[-*+]\s+\[(fact|judgment)\](?:\s+\[(?:methodology|domain\/[^\]]+)\])+(?:\s+\[(?:guardrail|foundation|convention)\])?\s+(.+?)\s+\(source:\s*[^,]+,\s*[^,]+,\s*\d{4}-\d{2}-\d{2}\)\s+\[impact:(high|normal)\]\s*$/u;
/** Source metadata extraction — A-8, P-2 */
export const SOURCE_PATTERN = /\(source:\s*([^,]+),\s*([^,]+),\s*(\d{4}-\d{2}-\d{2})\)/;
/** Guardrail 3-element check — A-9 (role=guardrail only) */
export const GUARDRAIL_PATTERN = /\*\*Situation\*\*:.*\*\*Result\*\*:.*\*\*Corrective action\*\*:/s;
/** Content extraction — A-10/P-3 exact match. Boundary aligned with TAG_PATTERN (CCNS-1). */
export const CONTENT_CAPTURE = /\[(?:fact|judgment)\](?:\s+\[[^\]]+\])+\s+(.+?)\s+\(source:\s*[^,]+,\s*[^,]+,\s*\d{4}-\d{2}-\d{2}\)/;
// ---------------------------------------------------------------------------
...
export const RAW_LLM_LINE_RE = /^[-*+]\s+\[(fact|judgment)\](?:\s+\[(?:methodology|domain\/[^\]]+)\])+(?:\s+\[(?:guardrail|foundation|convention)\])?\s+(.+?)\s+\[impact:(high|normal)\]\s*$/u;
/** Newly Learned section header */
export const NEWLY_LEARNED_HEADER_RE = /^###\s+Newly\s+Learned\s*$/m;
/** Event Markers section header */
export const EVENT_MARKERS_HEADER_RE = /^###\s+Applied\s+Learnings\s+—\s+Event\s+Markers\s*$/m;

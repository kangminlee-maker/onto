...
const SUPPORTED = ["2"];
export const PromoteReportSpec = {
    kind: KIND,
    ...
    parse(content, format) {
        ...
        if (typeof raw === "object" &&
            ...
            throw new IncompatibleVersionError(`${KIND}: legacy schema_version="1" detected. ` +
        ...
        if (Array.isArray(obj.cross_agent_dedup_clusters)) {
            obj.cross_agent_dedup_clusters.forEach((raw, i) => {
                ...
                if (typeof c.primary_owner_agent !== "string") {
                    errors.push(`cross_agent_dedup_clusters[${i}].primary_owner_agent must be a string`);
                }
                if (typeof c.primary_member_index !== "number" ||
                    !Number.isInteger(c.primary_member_index) ||
                    c.primary_member_index < 0) {
                    errors.push(`cross_agent_dedup_clusters[${i}].primary_member_index must be a non-negative integer ` +
                        `(schema v2 required; regenerate Phase A via 'onto promote' if the report predates the bump)`);
                }
                if (!Array.isArray(c.member_items)) {
                    errors.push(`cross_agent_dedup_clusters[${i}].member_items must be an array`);
                    return;
                }
                if (typeof c.primary_member_index === "number" &&
                    c.primary_member_index >= c.member_items.length) {
                    errors.push(`cross_agent_dedup_clusters[${i}].primary_member_index (${c.primary_member_index}) ` +

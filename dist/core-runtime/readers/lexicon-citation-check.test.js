...
import { checkFileCitations, checkMultipleFiles, hasRejectViolations, } from "./lexicon-citation-check.js";
const TERMS = [
...
describe("lexicon-citation-check", () => {
    ...
    it("seed/candidate/provisional term 인용 시 warn 위반을 보고한다", () => {
        ...
        const violations = checkFileCitations(".onto/principles/example.md", content, TERMS, { severity: "warn" });
    ...
    it("reject severity 설정 시 reject 위반을 보고한다", () => {
        ...
        const violations = checkFileCitations(".onto/processes/reconstruct.md", content, TERMS, { severity: "reject" });
    ...
    it("authority file (core-lexicon.yaml) 내 참조는 위반으로 보고하지 않는다", () => {
        ...
        const violations = checkFileCitations(".onto/authority/core-lexicon.yaml", content, TERMS);
        expect(violations).toEqual([]);
    });
    // ─── promoted term 은 위반이 아님 ───
    it("promoted term 인용은 위반으로 보고하지 않는다", () => {
        const content = "review_record 를 참조한다.";
        const violations = checkFileCitations(".onto/processes/review.md", content, TERMS);
        expect(violations).toEqual([]);
    });
    // ─── 단어 경계 (부분 매칭 방지) ───
    it("term_id 의 부분 매칭은 위반으로 보고하지 않는다", () => {
        ...
        const violations = checkFileCitations("docs/example.md", content, TERMS);
        expect(violations).toEqual([]);
    });
    // ─── 복수 파일 검사 ───
    ...
    it("warn 만 있으면 hasRejectViolations 는 false", () => {
        const violations = checkFileCitations("docs/test.md", "governance_pipeline 사용", TERMS, { severity: "warn" });

...
describe("scanVault", () => {
    ...
    it.skip("extracts doc_structure from markdown files", () => {
        writeFileSync(join(TMP, "structured.md"), "# Title\n\n## Section A\n\n## Section B\n\nParagraph text.");
        const result = scanVault(makeSource(TMP));
        expect(result.doc_structure.length).toBeGreaterThan(0);
        const doc = result.doc_structure.find((d) => d.file === "structured.md");

 ...
 */
export function parseParticipatingLensPaths(packetBody) {
    ...
    for (const m of section.matchAll(bulletRe)) {
        ...
        if (lensId.length === 0 || rawPath.length === 0)
            continue;
...
function extractParticipatingLensSection(packetBody) {
    // Match `## Participating Lens Outputs` or `## Runtime Participating Lens Outputs`
    // (case-insensitive) up to the next `## ` or EOF.
    const headingRe = /^\s*#{1,6}\s*(?:Runtime\s+)?Participating\s+Lens\s+Outputs\s*$/im;

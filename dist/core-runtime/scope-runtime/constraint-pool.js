...
const CONSTRAINT_EVENT_TYPES = new Set([
 ...
 */
export function buildConstraintPool(events) {
    const map = new Map();
    for (const evt of events) {
        if (!CONSTRAINT_EVENT_TYPES.has(evt.type))
            continue;
        switch (evt.type) {
            case "constraint.discovered": {
                const p = evt.payload;
                // Skip duplicate discovered for the same id (id uniqueness rule)
                if (map.has(p.constraint_id))
                    break;
            ...
            case "constraint.decision_recorded": {
                ...
                if (!entry)
                    break; // skip: referential integrity — gate-guard's responsibility
 ...
 */
export function isConstraintsResolved(pool) {
 ...
 */
export function findConstraint(pool, constraintId) {
    return pool.constraints.find((c) => c.constraint_id === constraintId);
}
// ─── Internal ───
function computeSummary(constraints) {
    let required = 0;
    let recommended = 0;
    let decided = 0;

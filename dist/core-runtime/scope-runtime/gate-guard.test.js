...
function makeStateWithConstraints(constraints, stateOverrides = {}) {
    const entries = constraints.map((c) => ({
        ...
        status: c.status,
        discovered_at: 1,
        ...(c.status === "decided" ? { decision: "inject", selected_option: "opt", decided_at: 2 } : {}),
    }));
...
function makeEvent(type, payload = {}, revision = 10, actor = "user") {
    return {
...
describe("gate-guard — Rule 1: state transition", () => {
    it("allows valid transition: draft → grounding.completed → grounded", () => {
        ...
        if (result.allowed)
            expect(result.next_state).toBe("grounded");
    });
    ...
    it("allows global event (scope.deferred) from non-terminal", () => {
        ...
        expect(result.allowed).toBe(true);
        if (result.allowed)
            expect(result.next_state).toBe("deferred");
    });
    it("allows observational event (feedback.classified)", () => {
        ...
        expect(result.allowed).toBe(true);
        if (result.allowed)
            expect(result.next_state).toBe("surface_iterating");
    });
});
...
describe("gate-guard — Rule 2: referential integrity", () => {
    ...
    it("denies decision_recorded for non-existent constraint", () => {
        ...
        expect(result.allowed).toBe(false);
        if (!result.allowed)
            expect(result.reason).toContain("CST-999");
    });
...
describe("gate-guard — Rule 3b: required constraint invalidation [GC-017]", () => {
    it("denies system invalidation of required constraint", () => {
        ...
        expect(result.allowed).toBe(false);
        if (!result.allowed)
            expect(result.reason).toContain("cannot be invalidated by system alone");
    });
...
describe("gate-guard — Rule 4: convergence blocking", () => {
    ...
    it("denies surface.revision_applied when blocked", () => {
        ...
        expect(result.allowed).toBe(false);
        if (!result.allowed)
            expect(result.reason).toContain("Convergence blocked");
    });
...
describe("gate-guard — conditional targets: constraints_resolved", () => {
    ...
    it("transitions to constraints_resolved when last constraint decided", () => {
        ...
        expect(result.allowed).toBe(true);
        if (result.allowed)
            expect(result.next_state).toBe("constraints_resolved");
    });
...
describe("gate-guard — golden data integration", () => {
    it("every golden event passes validation against its preceding state", async () => {
        ...
        const allEvents = readFileSync(goldenPath, "utf-8")
            .trimEnd()
            .split("\n")
        ...
        for (let i = 0; i < allEvents.length; i++) {
            ...
            expect(result.allowed, `Event ${currentEvent.event_id} (${currentEvent.type}) at revision ${currentEvent.revision} should be allowed. ${!result.allowed ? `Reason: ${result.reason}` : ""}`).toBe(true);
            if (result.allowed) {
                expect(result.next_state, `Event ${currentEvent.event_id}: expected next_state ${currentEvent.state_after}, got ${result.next_state}`).toBe(currentEvent.state_after);
            }
        }
    });
});
...
describe("gate-guard — scope.created edge cases", () => {
    it("denies scope.created when events already exist", () => {
        ...
        expect(result.allowed).toBe(false);
        if (!result.allowed)
            expect(result.reason).toContain("scope.created");
    });
});
...
describe("gate-guard — Rule 6: target.locked constraint resolution", () => {
    it("denies target.locked with undecided constraints", () => {
        ...
        expect(result.allowed).toBe(false);
        if (!result.allowed)
            expect(result.reason).toContain("미결정");
    });
    ...
    it("allows target.locked with all decided/invalidated constraints", () => {
        ...
        expect(result.allowed).toBe(true);
        if (result.allowed)
            expect(result.next_state).toBe("target_locked");
    });
});

...
function readGoldenPool() {
    return JSON.parse(readFileSync(resolve(GOLDEN_DIR, "state/constraint-pool.json"), "utf-8"));
}
function readGoldenVerdictLog() {
    return JSON.parse(readFileSync(resolve(GOLDEN_DIR, "state/verdict-log.json"), "utf-8"));
}
// ─── Event factory helpers ───
let _rev = 0;
function resetRev() { _rev = 0; }
function nextRev() { return ++_rev; }
function evt(type, stateBefore, stateAfter, payload = {}) {
    const r = nextRev();
    return {
        event_id: `evt_${r}`,
        scope_id: "SC-TEST",
...
describe("reducer — golden data", () => {
    ...
    it("direction matches align.locked", () => {
        expect(state.direction).toBe("학생이 튜터를 차단하면 향후 매칭에서 자동 제외");
    });
...
describe("reducer — validation_result", () => {
    ...
    it("contains correct result after validation.completed", () => {
        const items = [
            { val_id: "VAL-001", related_cst: "CST-001", result: "pass", detail: "통과" },
            { val_id: "VAL-002", related_cst: "CST-002", result: "fail", detail: "실패: 엣지 케이스 미처리" },
            { val_id: "VAL-003", related_cst: "CST-003", result: "pass", detail: "통과" },
        ];
        const events = [
            evt("scope.created", null, "draft", { title: "t", description: "d", entry_mode: "experience" }),
            evt("grounding.started", "draft", "draft", { sources: [] }),

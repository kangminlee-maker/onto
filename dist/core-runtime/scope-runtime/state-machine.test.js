...
import { canTransition, canApplyGlobal, canApplyObservational, resolveTransition, allowedTransitionEvents, } from "./state-machine.js";
import { STATES, TERMINAL_STATES, TRANSITION_EVENT_TYPES, GLOBAL_EVENT_TYPES, OBSERVATIONAL_EVENT_TYPES, } from "./types.js";
// ─── Golden data verification ───
const GOLDEN_PATH = resolve(import.meta.dirname, "__fixtures__/example-tutor-block/events.ndjson");
...
describe("state-machine — transition events exhaustive", () => {
    ...
    it("denies all unlisted (state, transitionEvent) combinations", () => {
        const allowedSet = new Set(EXPECTED.map((e) => `${e.state}|${e.event}`));
    ...
    for (const globalEvent of GLOBAL_EVENT_TYPES) {
        for (const state of nonTerminalStates) {
            it(`${state} + ${globalEvent} → allowed`, () => {
                ...
                expect(result.allowed).toBe(true);
                if (result.allowed) {
                    const expected = globalEvent === "scope.deferred" ? "deferred" : "rejected";
                    expect(result.next_state).toBe(expected);
                }
            });
        }
        for (const terminal of ["closed", "deferred", "rejected"]) {
    ...
    it("every observational event is allowed from every non-terminal state", () => {
        for (const state of nonTerminalStates) {
            for (const obsEvent of OBSERVATIONAL_EVENT_TYPES) {
                ...
                expect(result.allowed, `${state} + ${obsEvent} should be allowed`).toBe(true);
                if (result.allowed) {
                    expect(result.next_state).toBe(state);
...
// ═══════════════════════════════════════════════════════════════════
import { REVIEW_STATES, REVIEW_TERMINAL_STATES, REVIEW_TRANSITIONS, canReviewTransition, RECONSTRUCT_STATES, RECONSTRUCT_TERMINAL_STATES, RECONSTRUCT_TRANSITIONS, canReconstructTransition, } from "./state-machine.js";
describe("Review state machine (9-state, W-B-02 dedup)", () => {
    ...
    it("모든 non-terminal state 에서 최소 1개 outgoing edge 존재", () => {
        for (const s of REVIEW_STATES) {
            if (REVIEW_TERMINAL_STATES.has(s))
                continue;
...
describe("Build state machine (W-B-02 dedup)", () => {
    ...
    it("full happy path: (init)→negotiating→gathering_context→exploring→adjudicating→awaiting_user_review→processing_responses→converting→converted", () => {
        ...
        for (const [from, to] of path) {
            expect(canReconstructTransition(from, to)).toBe(true);
        }
    });
    ...
    it("모든 non-terminal state 에서 reconstruct_failed 전이 가능", () => {
        for (const s of RECONSTRUCT_STATES) {
            if (RECONSTRUCT_TERMINAL_STATES.has(s))
                continue;
...
describe("3중 SSOT 일관성 (W-B-02)", () => {
    ...
    it("3 도메인의 state 이름이 서로 겹치지 않는다", () => {
        const designSet = new Set(STATES);
        const reviewSet = new Set(REVIEW_STATES);
        const buildSet = new Set(RECONSTRUCT_STATES);
        // Design ∩ Review
        for (const s of reviewSet) {
            expect(designSet.has(s)).toBe(false);
        }
        // Design ∩ Build
        ...
        // Review ∩ Build
        for (const s of buildSet) {
            expect(reviewSet.has(s)).toBe(false);
        }
    });
});

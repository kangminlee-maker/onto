...
| RI03 | 임시 ID(PDF_NOTE_xxx)는 정규 ID 전환 전까지 참조 무결성 검증 대상에서 "경고" 처리 | PDF 원천 주석의 ID 불안정성 반영 | 경고 |

> **학습 반영 (onto_pragmatics):** RI03은 PDF에서 추출한 주석의 ID 체계(PDF_NOTE_xxx)가 불안정한 현실을 반영한다. HTML 원천에서 동일 주석을 확보하면 정규 ID로 전환하고, 이후 RI02와 동일한 "차단" 수준으로 격상한다.

## 원천 간 의존성 (학습 통합)

### 진실의 원천 전환 규칙 (onto_dependency)

| 우선순위 | 원천 | 조건 | 설명 |
|---|---|---|---|
| 1 | XBRL | 구조 완전·파싱 성공 | 1차 원천 — 기계 판독 가능, 구조화 |
| 2 | HTML | XBRL 불완전 또는 파싱 실패 | 2차 원천 — 반구조화, 파싱 필요 |
| 3 | PDF | HTML 미제공 | 3차 원천 — 비구조화, OCR 필요 |

- XBRL → HTML 전환은 금융업종에서 빈번 (onto_structure: role 코드 불일치)
- HTML이 유일 원천인 경우 SPOF 위험 (onto_dependency: DART HTML SPOF)
- 원천 전환 시 ID 체계 단일화(onto_dependency)가 선행되어야 참조 무결성 유지 가능

### 택소노미/인스턴스 분리 (onto_coverage)

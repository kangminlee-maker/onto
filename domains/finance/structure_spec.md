...
| 원천 구분 | XBRL 자동 / HTML 추출 / 수동 입력 | 노드 생성 경로 |

## 필수 노드 유형 (8개)

| 유형 | 설명 | 예시 | 원천 구분 |
|---|---|---|---|
| Entity | 보고 주체 (법인) | 삼성전자(005930) | XBRL 자동 |
| Statement | 재무제표 유형 | 연결재무상태표, 별도손익계산서 | XBRL 자동 |
| FinancialFact | 개별 재무 수치 | 매출액 300조원 (2023, 연결) | XBRL 자동 |
| Concept | 계정과목 (택소노미 요소) | ifrs:Revenue, dart-ext:SegmentRevenue | XBRL 자동 |
| Period | 보고 기간 | 2023-01-01~2023-12-31, 2023-12-31 | XBRL 자동 |
| Note | 주석 정보 | 우발부채 상세, 회계정책 설명 | HTML 추출 |
| Policy | 회계정책 선택 | 재고자산: 선입선출법 | HTML 추출 |
| Segment | 사업부문 | DS부문, 생활가전부문 | XBRL 자동 / HTML 추출 |

> **학습 반영 (onto_pragmatics):** Segment 노드는 대기업에서 필수적이나, 세그먼트 라벨이 기업별 확장(ENTITY_EXTENSION)이므로 Concept 노드와의 매핑(L2)이 선행되어야 한다. PDF 주석에서 추출한 Note의 경우 ID 체계가 불안정(PDF_NOTE_xxx)하여 참조 무결성 검증 대상이다.
...
> **학습 반영 (onto_structure):** 금융업종의 경우 Statement 구성이 상이하다. 예: 보험업은 "보험계약부채 변동표"가 추가될 수 있다. 업종 분류 축(일반/금융)에 따라 완전성 기준이 분기된다.
...
### 단일화 원칙 (onto_dependency)
- 노드 ID는 원천(XBRL/HTML/PDF)에 관계없이 단일 체계로 통합
- ID 구성: `{EntityCode}_{PeriodKey}_{StatementType}_{ConceptID}`
- PDF 주석에서 생성된 `PDF_NOTE_xxx` 형식 ID는 임시 ID이며, HTML 원천 확보 시 정규 ID로 전환

### XBRL 의미 식별자 (onto_evolution)
- XBRL 택소노미 요소 ID는 단순 기술 식별자가 아니라 의미 식별자(semantic identifier)
- 의미 식별자는 개념의 정의·계층·관계를 내포하므로, 자체 식별자를 새로 만들기보다 XBRL 요소 ID를 최대한 보존·활용

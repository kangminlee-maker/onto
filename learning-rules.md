...
**Learning의 핵심 가치**: 시간과 비용의 절감 (`authority/core-lexicon.yaml` learning term 참조). Verification quality는 이 핵심 가치가 실현되는 형태이지, 그 자체가 목적이 아니다. 핵심 가치보다 더 큰 비용(drift로 인한 재작업)을 만들면 안 되므로, **소비(consumption)는 promoted 학습만 대상**으로 한다.

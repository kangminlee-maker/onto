...
TeamCreate 시 팀 이름은 프로세스 이름을 사용합니다 (예: `onto-review`, `onto-buildfromcode`).
...
`~/.claude/teams/`에 config.json 없이 inbox만 남은 디렉토리, `~/.claude/tasks/`에 어떤 팀에도 속하지 않는 UUID 디렉토리가 축적될 수 있습니다. 팀 생성 전에 기존 고아 파일을 확인하고, 사용자에게 정리 여부를 안내합니다.

...
**적응 축 기반 학습 체계** — 학습은 3개 적응 축(communication, methodology, domain)으로 태깅되며, 하나의 학습이 복수 축에 귀속될 수 있습니다. 프로세스 개선은 실측 데이터와 마찰을 기반으로 지속적으로 수행합니다.

## 프로세스 맵

| 프로세스 | 파일 | 설명 | 관련 프로세스 |
|---|---|---|---|
| 온보딩 | `processes/onboard.md` | 프로젝트에 onto 환경 설정 | → 리뷰, 질문 |
...
### 에이전트 추가/제거 시 touch point 체크리스트

에이전트를 추가하거나 제거할 때, 아래 파일을 모두 갱신해야 합니다:

| # | 파일 | 갱신 항목 |
|---|------|----------|
| 1 | `roles/{agent-id}.md` | 역할 정의 생성/삭제 |
| 2 | `commands/ask-{dimension}.md` | 명령어 파일 생성/삭제 |
| 3 | `process.md` 검증 에이전트 테이블 | 행 추가/삭제 |
| 4 | `process.md` 도메인 문서 매핑 | 행 추가/삭제 |
| 5 | `process.md` teammate 초기 prompt 매핑 | 행 추가/삭제 |
| 6 | `processes/review.md` Philosopher 파일 목록 | 경로 추가/삭제 |
| 7 | `domains/*/` | 해당 에이전트의 도메인 문서 템플릿 추가/삭제 |
| 8 | `dev-docs/BLUEPRINT.md` 에이전트 섹션 | 설명 추가/삭제 |
| 9 | `README.md` 디렉토리 트리 | 파일명 추가/삭제 |
...
2. **CLAUDE.md 하위 호환**: `.onto/config.yml`이 없거나 `domain:`이 없으면, CLAUDE.md의 `domain:` 또는 `agent-domain:` 선언을 사용합니다.
...
5. **도메인 문서 없음**: 선언된 도메인의 문서(`~/.onto/domains/{domain}/`)가 존재하지 않으면, 도메인 규칙 없이 범용 원칙만으로 검증합니다.
...
에이전트의 학습은 **2개 경로**로 저장됩니다:

| 학습 유형 | 저장 경로 | 내용 |
|---|---|---|
| **소통 학습** | `~/.onto/communication/common.md` | 모든 에이전트에 적용되는 소통 규칙 |
| **검증 학습** | `~/.onto/learnings/{agent-id}.md` | 축 태그로 구분된 검증 학습 (methodology + domain 통합) |

**적응 축 모델**: 3분류(communication / methodology / domain)는 배타적 분류 체계가 아니라, 에이전트가 적응해야 할 독립된 3개 축의 선언입니다. 하나의 학습이 복수 축에 기여할 수 있습니다.

프로젝트 수준 학습은 `{project}/.onto/learnings/{agent-id}.md`에 저장됩니다.
...
- Fallback 시 team lead가 각 Agent tool call에 포함해야 하는 내용: 에이전트 정의 + 학습 파일(축 태그 필터링 포함) + 도메인 문서 + 소통 학습 + 작업 지시 + **세션 경로**. (자기 로딩이 불가능하므로 모든 컨텍스트를 직접 포함)
...
**team_name**: `{프로세스 이름}-{세션 ID}` (예: `onto-20260325-a3f7b2c1`)
**세션 디렉토리**: `{project}/.onto/{프로세스}/{세션 ID}/`
...
1. 학습: ~/.onto/learnings/{agent-id}.md
2. 도메인 문서: ~/.onto/domains/{domain}/{해당 도메인 문서}
3. 소통 학습: ~/.onto/communication/common.md
프로젝트 학습이 있으면:
4. 프로젝트 학습: {project}/.onto/learnings/{agent-id}.md
...
**소통 학습**:
- `~/.onto/communication/common.md`: 모든 에이전트에 적용되는 소통 규칙. 팀 리뷰에서 발견된 사항은 여기에 저장합니다.
...
**검증 학습** (`~/.onto/learnings/{agent-id}.md` 또는 `{project}/.onto/learnings/{agent-id}.md`):
- 저장 경로 판별:
  - 프로젝트에 `.onto/learnings/` 디렉토리가 존재하면: `{project}/.onto/learnings/{agent-id}.md` (프로젝트 수준)
  - 존재하지 않으면: `~/.onto/learnings/{agent-id}.md` (글로벌 수준)
- 기존 항목과 중복되면 추가하지 않습니다.
- 기존 항목과 모순되면 새 학습으로 교체합니다.

항목 형식:
```markdown
- [{유형}] [{축 태그}] {학습 내용} (출처: {프로젝트명}, {도메인}, {날짜})
```

`{유형}` 태그:
- `사실`: 정의, 구조, 관계에 대한 객관적 기술. 축적해도 판단 편향을 유발하지 않음.
- `판단`: "이 패턴은 문제이다/아니다" 등의 가치 판단. 맥락 변화에 따라 유효성이 달라질 수 있으므로 재검증 대상.

`{축 태그}` (복수 허용):
- `[methodology]`: 도메인 무관하게 적용 가능한 실용적 검증 기법
- `[domain/{name}]`: 특정 도메인의 맥락에서 유효한 학습
- 하나의 학습에 `[methodology]`와 `[domain/{name}]`을 동시에 부여할 수 있음
- **태그 부재 = open-world**: 특정 축 태그가 없다고 해서 "해당 축에서 무효"를 의미하지 않음. "해당 축에서의 유효성이 아직 확인되지 않음"을 의미

**축 태그 판정 기준** (저장 전 필수, 양방향):
1. "이 학습에서 도메인 고유 용어를 제거해도 원칙이 성립하는가?" → 성립하면 `[methodology]` 태그 부여
2. "이 학습이 특정 도메인의 맥락에서 발생했거나 유효한가?" → 해당하면 `[domain/{name}]` 태그 부여
3. 양쪽 모두 해당하면 양쪽 모두 태그 부여

### 소비 규칙

에이전트가 학습 파일을 로딩한 후, 아래 규칙으로 각 항목을 처리합니다:

1. `[methodology]` 태그 항목: **항상 적용**
2. `[domain/{current-domain}]` 태그 항목: **항상 적용**
3. `[domain/{other-domain}]` 태그만 있는 항목: **검토 후 판단** — 학습 내용에서 도메인 고유 용어를 제거해도 원칙이 성립하면 적용. 성립하지 않으면 무시.
4. 태그 없는 항목 (레거시): `[methodology]`로 간주

### 학습 검증 규칙

**판단 학습 재검증** (권장):
- `[판단]` 유형의 학습이 10건 이상 축적된 에이전트는, `/onto:promote` 실행 시 기존 판단 학습의 유효성을 재검증합니다.
- 재검증 기준: 이 판단이 현재 맥락에서도 여전히 유효한가? 과거 판단에 매몰되어 현재 대상의 목적을 놓치고 있지 않은가?

### 저장 후 안내

새로 추가된 communication 항목이 있으면 사용자에게 알립니다:
"communication 발견사항이 N건 기록되었습니다. `~/.onto/communication/`에서 확인하시고, 전역 설정(`~/.claude/CLAUDE.md`)에 반영할지 결정해 주세요."

...
| 학습 유형 | 저장 경로 | 내용 |
|---|---|---|
| **소통 학습** | `~/.onto-review/communication/common.md` | 모든 에이전트에 적용되는 소통 규칙 |
| **검증 학습** | `~/.onto-review/learnings/{agent-id}.md` | 축 태그로 구분된 검증 학습 (methodology + domain 통합) |

**적응 축 모델**: 3분류(communication / methodology / domain)는 배타적 분류 체계가 아니라, 에이전트가 적응해야 할 독립된 3개 축의 선언입니다. 하나의 학습이 복수 축에 기여할 수 있습니다.

프로젝트 수준 학습은 `{project}/.onto-review/learnings/{agent-id}.md`에 저장됩니다.
...
1. 학습: ~/.onto-review/learnings/{agent-id}.md
2. 도메인 문서: ~/.onto-review/domains/{domain}/{해당 도메인 문서}
3. 소통 학습: ~/.onto-review/communication/common.md
프로젝트 학습이 있으면:
4. 프로젝트 학습: {project}/.onto-review/learnings/{agent-id}.md
...
**소통 학습**:
- `~/.onto-review/communication/common.md`: 모든 에이전트에 적용되는 소통 규칙. 팀 리뷰에서 발견된 사항은 여기에 저장합니다.
...
**검증 학습** (`~/.onto-review/learnings/{agent-id}.md` 또는 `{project}/.onto-review/learnings/{agent-id}.md`):
- 저장 경로 판별:
  - 프로젝트에 `.onto-review/learnings/` 디렉토리가 존재하면: `{project}/.onto-review/learnings/{agent-id}.md` (프로젝트 수준)
  - 존재하지 않으면: `~/.onto-review/learnings/{agent-id}.md` (글로벌 수준)

...
- **경량 모드 (core-axis)**: Step 1.5에서 선택된 2-3명의 검증 lens + `axiology` + `synthesize`를 생성한다.
...
- 경량 모드(core-axis): 선택된 2-3명의 review lens + `axiology`만 Codex task로 생성
...
- 경량 모드(core-axis): 선택된 2-3명의 review lens + `axiology`만 생성
- 전원 모드(full): 8명 전원 생성
- `synthesize`는 이 단계에서 생성하지 않는다 (메인 프로세스 Step 3에서 별도 실행)

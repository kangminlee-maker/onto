...
- **core-axis 모드**: 고정 4 lens (`logic` / `pragmatics` / `evolution` / `axiology`) + `synthesize`를 생성한다. 단, `host_runtime: standalone | litellm | anthropic | openai` 인 경우 Step 1.5 dynamic lens selection (`selectLenses`) 이 활성화되어 LLM 이 2-4 lens 를 선택할 수 있다 (이 경우에도 `axiology` 는 항상 포함).
...
- core-axis 모드: 고정 4 lens (logic / pragmatics / evolution / axiology) Codex task로 생성. (Step 1.5 dynamic selection 활성 시 2-4 lens + axiology)
...
- core-axis 모드: 고정 4 lens (logic / pragmatics / evolution / axiology) 생성. (Step 1.5 dynamic selection 활성 시 2-4 lens + axiology)
- 전원 모드(full): 8명 전원 생성
- `synthesize`는 이 단계에서 생성하지 않는다 (메인 프로세스 Step 3에서 별도 실행)

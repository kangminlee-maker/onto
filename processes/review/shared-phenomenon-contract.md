...
> 상태: Active
> 목적: 여러 lens가 동일 현상에 대해 각자의 claim을 내는 상황(overlap-permitted lens claims)을 다루기 위한 현상 동일성 규칙과 claim 관계 분류를 정의한다.
> 이 문서는 overlap/correlation 규칙의 **유일한 normative seat**이다. 다른 contract 파일은 이 문서를 참조만 한다.
...
- **Shared phenomenon**: 둘 이상의 lens-qualified claim이 같은 target + 같은 evidence locus를 대상으로 하는 상태. claim 내용의 양립 가능성과 무관하게 성립하는 umbrella relation이다.
- **Claim relation**: 같은 shared phenomenon에 속한 두 claim 사이의 관계. identity 성립 후 claim 내용을 비교하여 분류한다.

---

## 3. Phenomenon Identity Rule

두 lens-qualified claim이 같은 shared phenomenon에 속하는지 판정하는 규칙.

**판정 기준**: `target`과 `evidence_anchor`가 동일하면 같은 shared phenomenon이다.

- claim 내용(방향·심각도·결론)은 identity 판정에 **포함되지 않는다**
...
- 이 문서는 4필드의 의미와 identity rule만 정의한다. 직렬화 형식은 `lens-prompt-contract.md`가 소유한다

---

## 6. Authority

- 이 문서(`shared-phenomenon-contract.md`)가 phenomenon identity rule, claim relation 분류, overlap 정책의 **유일한 normative seat**이다
- `lens-prompt-contract.md`는 4필드 직렬화 형식을 소유하되, correlation 규칙은 이 문서를 참조만 한다

...
if [[ ! -f "${ONTO_CLI}" ]]; then
  echo "fixture.sh: onto CLI not built. Run \`npm run build\` in ${ONTO_REPO_ROOT}" >&2
  return 1 2>/dev/null || exit 1
fi
...
# Check that a STDERR log contains an expected pattern.
smoke_assert_stderr_contains() {
  local logfile="$1"
  local pattern="$2"
  local label="$3"
  if ! grep -qE "${pattern}" "${logfile}"; then
    smoke_fail "${label}: STDERR log did not contain pattern '${pattern}' (see ${logfile})"
  fi
}

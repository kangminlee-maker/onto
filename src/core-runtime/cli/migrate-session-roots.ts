...
} from "./session-root-guard.js";

// ---------------------------------------------------------------------------
// Config
// ---------------------------------------------------------------------------

export interface MigrateSessionRootsConfig {
  projectRoot: string;
  /** Plan only — do not move files or write the marker. */
  dryRun?: boolean;
}
...
export interface MigrationResult {
  ...
  /** Sessions that were (or would be) moved. */
  migrated: MigratedSession[];
  /** Per-session failures (only relevant for non-dry runs). */
  failures: { session_id: string; error: string }[];
  /** Whether the layout marker was written this run. */
...
export function migrateSessionRoots(
  config: MigrateSessionRootsConfig,
): MigrationResult {
  ...
  const planned: MigratedSession[] = before.legacy_session_ids.map((id) => ({
    session_id: id,

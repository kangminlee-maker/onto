...
import type { ScopeState, ConstraintPool, ConstraintEntry } from "../../../../scope-runtime/types.js";
...
describe("compile-defense — Layer 1 Checklist", () => {
  ...
  it("fails when a constraint is missing from Section 3", () => {
    ...
    const vp = [{ val_id: "VAL-001", related_cst: "CST-001", decision_type: "inject" }] as ValidationPlanItem[];
    const result = compileDefense(makeState(pool), bs, ds, vp);
...
describe("compile-defense — Layer 2 inject", () => {
  ...
  it("fails when inject has no VAL", () => {
    ...
    const vp: ValidationPlanItem[] = []; // no VAL
    const result = compileDefense(makeState(pool), bs, ds, vp);
    expect(result.passed).toBe(false);
    if (!result.passed) {
...
describe("compile-defense — Layer 2 defer", () => {
  it("passes when defer source_refs not in delta-set", () => {
    ...
    const vp = [{ val_id: "VAL-001", related_cst: "CST-001", decision_type: "defer" }] as ValidationPlanItem[];
    const result = compileDefense(makeState(pool), bs, ds, vp);
    expect(result.passed).toBe(true);
  });
...
describe("compile-defense — Layer 2 override", () => {
  it("passes when override is not reflected", () => {
    ...
    const vp = [{ val_id: "VAL-001", related_cst: "CST-001", decision_type: "override" }] as ValidationPlanItem[];
    const result = compileDefense(makeState(pool), bs, ds, vp);
    expect(result.passed).toBe(true);
  });
...
describe("compile-defense — additional edge cases", () => {
  ...
  it("L1 + L2 violations simultaneously", () => {
    ...
    const vp = [{ val_id: "VAL-001", related_cst: "CST-002", decision_type: "inject" }] as ValidationPlanItem[];
    const result = compileDefense(makeState(pool), bs, ds, vp);
    expect(result.passed).toBe(false);
    if (!result.passed) {

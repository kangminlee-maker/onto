...
export interface EntryModeRouting {
  /** Subpath under paths.surface (e.g. "preview/", "contract-diff/", "design-doc-draft.md") */
  surfaceSubpath: string;
  /** Human-readable surface path label (relative to scope root) */
  surfaceLabel: string;
  /** Guide message after generate_surface — caller appends to "Surface가 생성되었습니다." */
  guideMessageAfterGeneration: string;
  /** scope-md next-action when state is align_locked */
  nextActionAlignLocked: string;
  /** scope-md next-action when state is surface_iterating */
  nextActionSurfaceIterating: string;
  /**
   * Whether runtime auto-writes the surface skeleton (process-mode markdown
   * frontmatter is deterministic; experience/interface caller writes via agent).
   */
  hasRuntimeWrittenSkeleton: boolean;
}

const ENTRY_MODE_ROUTING: Record<EntryMode, EntryModeRouting> = {
  experience: {
 ...
 */
export function getEntryModeRouting(mode: EntryMode): EntryModeRouting {

...
describe("govern CLI v0", () => {
  ...
  it("submit appends a queue event with origin=human → tag=norm_change", async () => {
    ...
    expect(code).toBe(0);
    const result = lastLogJson() as { status: string; id: string; tag: string; origin: string };
  ...
  it("submit with origin=system maps to tag=drift and submitted_by=drift-engine", async () => {
    ...
    expect(code).toBe(0);
    const result = lastLogJson() as { tag: string; origin: string };
  ...
  it("list --status pending returns only pending entries", async () => {
    ...
    const submitResp = lastLogJson() as { id: string };
    ...
    const pending = lastLogJson() as Array<{ id: string; status: string }>;
    ...
    const decided = lastLogJson() as Array<{ id: string; status: string; verdict: { verdict: string } }>;
    ...
    const all = lastLogJson() as unknown[];
  ...
  it("decide transitions entry to decided and records verdict + reason", async () => {
    ...
    const id = (lastLogJson() as { id: string }).id;
    ...
    expect(code).toBe(0);
    const result = lastLogJson() as { status: string; verdict: string; note: string };
    ...
    const decided = lastLogJson() as Array<{
      id: string;
  ...
  it("prompted-by-drift links a human submit to a drift id", async () => {
    ...
    const entries = lastLogJson() as Array<{
      id: string;

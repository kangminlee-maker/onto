...
describe("promote-principle Quality gate", () => {
  ...
  it("workload evidence 가 threshold 충족 시 quality gate 통과", () => {
    const result = executePromotePrinciple(makeProposal() as any, tmpRoot);
  ...
  it("OR mode: 하나의 축만 threshold 충족해도 통과", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) expect(result.gate_passed).toBe("quality");
  });
});
...
describe("promote-principle Frequency gate", () => {
  ...
  it("similar_to 가 기존 pending 참조 시 workload evidence 면제", () => {
    const first = executePromotePrinciple(makeProposal() as any, tmpRoot);
    expect(first.success).toBe(true);
    const firstId = first.success ? first.id : "";

    const second = executePromotePrinciple(
      makeProposal({
  ...
  it("similar_to 의 id 가 pending 에 없으면 validation 실패", () => {
    const result = executePromotePrinciple(
      makeProposal({ similar_to: ["g-nonexistent"] }) as any,
      tmpRoot,
    );
    expect(result.success).toBe(false);
    if (!result.success) expect(result.reason).toContain("존재하지 않음");
  });
});
...
describe("promote-principle Completeness gate", () => {
  ...
  it("learning_ref 누락 시 거부", () => {
    const result = executePromotePrinciple(
      makeProposal({ learning_ref: { agent_id: "", entry_marker: "" } }) as any,
      tmpRoot,
    );
    expect(result.success).toBe(false);
...
describe("promote-principle E2E (promote → list → decide)", () => {
  ...
  it("promote-principle → list(pending) → decide approve 전체 흐름", async () => {
    ...
    expect(code).toBe(0);
    const queued = lastLogJson() as { status: string; id: string; gate_passed: string };
    ...
    const pending = lastLogJson() as Array<{ id: string; payload: { promotion_kind: string } }>;

...
function makeItem(overrides: Partial<ParsedLearningItem> = {}): ParsedLearningItem {
  return {
    agent_id: "logic",
    scope: "global",
    ...
    line_number: 10,
    type: "fact" as LearningType,
    applicability_tags: ["methodology"],
    role: "guardrail" as LearningPurposeRole,
...
describe("renderHealthReport", () => {
  function makeReport(): ReturnType<typeof buildHealthReport> {
  ...
  it("labels target as Project when target=project", () => {
    const report = buildHealthReport([makeItem()], "project" as HealthTarget);

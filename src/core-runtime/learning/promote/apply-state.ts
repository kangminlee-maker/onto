 ...
 * extra metadata.
 */
export function getApplyStatePath(sessionRoot: string): string {
...
export interface InitApplyStateConfig {
  sessionId: string;
  attemptId?: string;
  pendingDecisions: PendingDecisionRef[];
  recoverabilityCheckpointPath?: string | null;
}
 ...
 */
export function initApplyState(config: InitApplyStateConfig): ApplyExecutionState {
...
export function markApplied(
  ...
  const stillPending = state.pending_decisions.filter(
    (p) => !(p.decision_id === applied.decision_id && p.decision_kind === applied.decision_kind),
  );
  if (stillPending.length === state.pending_decisions.length) {
    throw new Error(
      `apply-state.markApplied: decision ${applied.decision_kind}/${applied.decision_id} ` +
...
export function markFailed(
  ...
  const stillPending = state.pending_decisions.filter(
    (p) => !(p.decision_id === failed.decision_id && p.decision_kind === failed.decision_kind),
  );
  if (stillPending.length === state.pending_decisions.length) {
    throw new Error(
      `apply-state.markFailed: decision ${failed.decision_kind}/${failed.decision_id} ` +
...
export function transitionStatus(
  state: ApplyExecutionState,
  to: ApplyExecutionStateStatus,
): ApplyExecutionState {
  if (to === "state_persistence_failed") {
    throw new Error(
      `apply-state.transitionStatus: state_persistence_failed cannot be ` +
 ...
 */
function bump(state: ApplyExecutionState): ApplyExecutionState {
  return {
    ...state,
 ...
 */
export function persistApplyState(
  sessionRoot: string,
  state: ApplyExecutionState,
): string {
...
export function loadApplyState(
  ...
  return REGISTRY.loadFromFile<ApplyExecutionState>(

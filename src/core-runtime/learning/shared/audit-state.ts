...
export const DEFAULT_AUDIT_STATE_PATH = path.join(
  os.homedir(),
  ".onto",
  "audit-state.yaml",
);
...
export function getAuditStatePath(ontoHome?: string): string {
  if (ontoHome) {
 ...
 */
export function loadAuditState(filePath?: string): AuditState {
  const target = filePath ?? DEFAULT_AUDIT_STATE_PATH;
  if (!fs.existsSync(target)) {
    return { schema_version: "1", obligations: [] };
  }
  const json = REGISTRY.loadFromFile<AuditStateJSON>("audit_state", target);
  return {
    schema_version: "1",
...
export function saveAuditState(state: AuditState, filePath?: string): void {
  const target = filePath ?? DEFAULT_AUDIT_STATE_PATH;
  const json: AuditStateJSON = {
    schema_version: "1",
 ...
 * resolved.
 */
export function processCarryForward(
  state: AuditState,
  sessionId: string,
): AuditState {
  for (const ob of state.obligations) {
    if (ob.status !== "pending" && ob.status !== "blocked") continue;
    ...
    if (ob.hasExceededCarryForward()) {
      ob.transition(
        "expired_unattended",
        `carry_forward_count (${ob.carry_forward_count}) exceeded max ` +
          `(${ob.max_carry_forward})`,
    ...
    if (ob.status === "blocked") {
      ob.transition(
        "pending",
        `Re-entry from blocked (carry_forward ${ob.carry_forward_count})`,
        { session_id: sessionId },
      );
    }
  }
...
export function getExpiredUnattended(state: AuditState): AuditObligation[] {
  return state.obligations.filter((o) => o.status === "expired_unattended");
}

export function getActiveObligations(state: AuditState): AuditObligation[] {
  return state.obligations.filter((o) => o.isActive());
}

export function countCarriedForward(state: AuditState): number {

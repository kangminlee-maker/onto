...
import type { LearningUsageEntry } from "./usage-tracker.js";
...
function createSessionManifest(
  reviewRoot: string,
  sessionId: string,
  ...
  writeFileSync(
    join(prepDir, "learning-manifest.yaml"),
    stringify(makeManifest(manifestOverrides)),
  );
}
...
function makeUsageEntry(
  ...
  overrides: {
    ...
    tokens?: number;
    degraded?: boolean;
    tier?: { t1: number; t2: number; t3: number; t4: number };
  } = {},
): LearningUsageEntry {
  return {
    session_id,

...
import {
  ...
  figmaSourceHash,
  scanFigmaMcp,
  recordFigmaGrounding,
  type FigmaSource,
} from "./mcp-figma.js";

function makeSource(fileKey: string): FigmaSource {
...
describe("figmaSourceHash", () => {
  ...
  it("key matches figmaSourceHashKey output", () => {
    ...
    expect(figmaSourceHash(source, "2024-06-01T00:00:00Z").key).toBe(
      figmaSourceHashKey(source),
    );
  });
